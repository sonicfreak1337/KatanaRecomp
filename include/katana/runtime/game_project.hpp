...
// product, an active base-image identity makes that complete identity range
// read-only. Analysis may use its exact code/literal/table bytes, and the
// generated runtime rejects writes to the same identity-bound range. Runtime
// image identities remain snapshot candidates and are not promoted by this
// base-image contract.
