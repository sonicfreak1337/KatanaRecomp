...
std::string access_error_message(
    const MemoryAccessErrorReason reason,
    ...
    switch (reason) {
        case MemoryAccessErrorReason::Misaligned:
            output
                << "Adresse ist nicht natuerlich auf "
                << bytes
                << " Byte ausgerichtet.";
            break;
        ...
        case MemoryAccessErrorReason::CrossRegion:
            output
                << "Zugriff ueberschreitet die Region '"
                << region_name
                << "'.";
            break;
...
bool watchpoint_accepts(
    const MemoryWatchpointAccess watchpoint_access,
    const MemoryAccessOperation operation
) {
...
bool ranges_overlap(
    const std::uint32_t left_address,
...
Memory::Memory(
    ...
    const MemoryAlignmentPolicy alignment_policy
)
...
void Memory::set_alignment_policy(
    const MemoryAlignmentPolicy policy
) noexcept {
    alignment_policy_ = policy;
}

MemoryWatchpointId Memory::add_watchpoint(
    const std::uint32_t address,
    const std::size_t size,
    ...
    if (size == 0u) {
        throw std::invalid_argument(
            "Ein Watchpoint braucht eine positive Groesse."
        );
    }
    const std::uint64_t start = address;
    if (size > address_space_size - start) {
        throw std::invalid_argument(
            "Ein Watchpoint ueberschreitet den 32-Bit-Adressraum."
        );
    }
    if (!observer) {
        throw std::invalid_argument(
            "Ein Watchpoint braucht einen Observer."
        );
    }
    if (next_watchpoint_id_ ==
        std::numeric_limits<MemoryWatchpointId>::max()) {
        throw std::overflow_error(
            "Es koennen keine weiteren Watchpoints registriert werden."
        );
    }
...
bool Memory::remove_watchpoint(const MemoryWatchpointId id) {
    const auto iterator = std::find_if(
        watchpoints_.begin(),
        watchpoints_.end(),
        [id](const Watchpoint& watchpoint) {
...
std::uint8_t Memory::read_u8(const std::uint32_t address) const {
    ...
    const auto value =
        mapped.device->read_u8(region_offset(mapped.info, address));
...
std::uint16_t Memory::read_u16(const std::uint32_t address) const {
    ...
    const auto value =
        mapped.device->read_u16(region_offset(mapped.info, address));
    notify_access(MemoryAccessEvent{
        MemoryAccessOperation::Read,
        address,
        MemoryAccessWidth::Halfword,
...
std::uint32_t Memory::read_u32(const std::uint32_t address) const {
    ...
    const auto value =
        mapped.device->read_u32(region_offset(mapped.info, address));
    notify_access(MemoryAccessEvent{
        MemoryAccessOperation::Read,
        address,
        MemoryAccessWidth::Word,
...
void Memory::write_u8(
    ...
    const auto& mapped = resolve_writable(
        address,
        MemoryAccessWidth::Byte
    );
...
void Memory::notify_access(const MemoryAccessEvent& event) const {
    ...
    observers.reserve(
        watchpoints_.size() + (trace_handler_ ? 1u : 0u)
    );
    ...
    for (const auto& watchpoint : watchpoints_) {
        if (
            watchpoint_accepts(watchpoint.access, event.operation) &&
            ranges_overlap(
                watchpoint.address,
                watchpoint.size,
                event.address,
