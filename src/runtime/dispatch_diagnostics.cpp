...
bool same_event(const DispatchDiagnosticEvent& left, const DispatchDiagnosticEvent& right) noexcept {
    return left.callsite == right.callsite
        && left.source_virtual == right.source_virtual
        ...
        && left.guest_instructions == right.guest_instructions
        && left.exit_pc == right.exit_pc
...
const char* block_end_name(const BlockEndKind kind) noexcept {
    switch (kind) {
        case BlockEndKind::Fallthrough: return "fallthrough";
        case BlockEndKind::StaticBranch: return "static-branch";
        case BlockEndKind::ConditionalBranch: return "conditional-branch";
        case BlockEndKind::DynamicBranch: return "dynamic-branch";
        case BlockEndKind::Call: return "call";
        case BlockEndKind::Return: return "return";
        case BlockEndKind::Exception: return "exception";
        case BlockEndKind::InterruptSafepoint: return "interrupt-safepoint";
    }
    return "unknown";
}
...
void DispatchDiagnosticRecorder::record(DispatchDiagnosticEvent event) {
    ...
    const auto duplicate = std::find_if(events_.begin(), events_.end(), [&event](const auto& current) {
...
std::string DispatchDiagnosticRecorder::serialize_json() const {
    ...
    for (std::size_t index = 0u; index < events_.size(); ++index) {
        ...
        output << "{\"callsite\":\"" << hex32(event.callsite)
               << "\",\"source_virtual\":\"" << hex32(event.source_virtual)
               << "\",\"source_physical\":\"" << hex32(event.source_physical)
               << "\",\"virtual_target\":";
        optional_address(output, event.virtual_target);
        output << ",\"canonical_target\":";
        optional_address(output, event.canonical_target);
        output << ",\"pr\":\"" << hex32(event.pr)
               << "\",\"block_end\":\"" << block_end_name(event.block_end)
               << "\",\"origin\":\"" << dispatch_resolution_origin_name(event.origin)
               << "\",\"alias_origin\":\"" << dispatch_alias_origin_name(event.alias_origin)
               << "\",\"fallback_reason\":\""
               << dispatch_fallback_reason_name(event.fallback_reason)
               << "\",\"fallback_action\":\""
               << dispatch_fallback_action_name(event.fallback_action)
               << "\",\"guest_instructions\":" << event.guest_instructions
               << ",\"exit_pc\":\"" << hex32(event.exit_pc)
...
const char* dispatch_resolution_origin_name(const DispatchResolutionOrigin value) noexcept {
    switch (value) {
        case DispatchResolutionOrigin::StaticProof: return "static-proof";
        case DispatchResolutionOrigin::Override: return "override";
        case DispatchResolutionOrigin::TableLookup: return "table-lookup";
        case DispatchResolutionOrigin::InlineCache: return "inline-cache";
        case DispatchResolutionOrigin::Fallback: return "fallback";
    }
    return "unknown";
}
...
const char* dispatch_alias_origin_name(const DispatchAliasOrigin value) noexcept {
    switch (value) {
        case DispatchAliasOrigin::None: return "none";
        case DispatchAliasOrigin::ExactVirtual: return "exact-virtual";
        case DispatchAliasOrigin::CanonicalPhysical: return "canonical-physical";
    }
    return "unknown";
}
...
const char* dispatch_fallback_reason_name(const DispatchFallbackReason value) noexcept {
    switch (value) {
        case DispatchFallbackReason::None: return "none";
        case DispatchFallbackReason::UnknownOpcode: return "unknown-opcode";
        case DispatchFallbackReason::UnresolvedControlFlow: return "unresolved-control-flow";
        case DispatchFallbackReason::DynamicCode: return "dynamic-code";
        case DispatchFallbackReason::ManifestDenied: return "manifest-denied";
    }
    return "unknown";
}
...
const char* dispatch_fallback_action_name(const DispatchFallbackAction value) noexcept {
    switch (value) {
        case DispatchFallbackAction::None: return "none";
        case DispatchFallbackAction::Abort: return "abort";
        case DispatchFallbackAction::Diagnose: return "diagnose";
        case DispatchFallbackAction::Interpreter: return "interpreter";
        case DispatchFallbackAction::UserHook: return "user-hook";
    }
    return "unknown";
}
...
const char* dispatch_diagnostic_error_name(const DispatchDiagnosticError value) noexcept {
    switch (value) {
        case DispatchDiagnosticError::None: return "none";
        case DispatchDiagnosticError::UnknownCode: return "unknown-code";
        case DispatchDiagnosticError::UnknownTarget: return "unknown-target";
        case DispatchDiagnosticError::UnmappedMemory: return "unmapped-memory";
        case DispatchDiagnosticError::FirmwareDenied: return "firmware-denied";
        case DispatchDiagnosticError::Misaligned: return "misaligned";
        case DispatchDiagnosticError::InvalidBoundary: return "invalid-boundary";
    }
    return "unknown";
}
