...
SchedulerSafepoints::SchedulerSafepoints(EventScheduler& scheduler,
    ...
    if (event_budget == 0u || loop_quantum == 0u) {
        throw std::invalid_argument(
            "Scheduler-Safepoints brauchen Ereignisbudget und Schleifenquantum.");
    }
}

SafepointReport SchedulerSafepoints::consume(const std::uint64_t guest_cycles,
...
std::vector<SafepointReport> SchedulerSafepoints::consume_loop(std::uint64_t guest_cycles,
                                                               const ExecutionOrigin origin) {
