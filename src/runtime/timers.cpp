...
Sh4RtcClockDomain::Sh4RtcClockDomain(
    const std::uint64_t guest_cycles_per_second
...
std::uint64_t Sh4RtcClockDomain::ticks_at(
    ...
    return checked_add(
        ...
        checked_multiply(
            remainder,
            source_frequency_hz,
            "RTC-Tickphase ist uebergelaufen."
        ) / guest_cycles_per_second_,
        "RTC-Tickindex ist uebergelaufen."
    );
}

std::uint64_t Sh4RtcClockDomain::cycle_at_or_after_tick(
    const std::uint64_t tick
) const {
    const auto whole_seconds = tick / source_frequency_hz;
    const auto remainder = tick % source_frequency_hz;
    const auto partial_product = checked_multiply(
        remainder,
        guest_cycles_per_second_,
...
std::uint64_t Sh4RtcClockDomain::elapsed_ticks(
    const std::uint64_t first_cycle,
    const std::uint64_t last_cycle
) const {
...
std::uint64_t Sh4RtcClockDomain::deadline_after(
    const std::uint64_t guest_cycle,
    const std::uint64_t source_ticks
) const {
    if (source_ticks == 0u) {
        return guest_cycle;
    }
    const auto target_tick = checked_add(
        ticks_at(guest_cycle),
        source_ticks,
        "RTC-Zieltick ist uebergelaufen."
    );
    return cycle_at_or_after_tick(target_tick);
}

Sh4RtcClockDomain::PhaseObserverId Sh4RtcClockDomain::add_phase_observer(
...
bool Sh4RtcClockDomain::remove_phase_observer(
    const PhaseObserverId observer_id
) noexcept {
...
Sh4Tmu::Sh4Tmu(EventScheduler& scheduler, TmuTiming timing)
    ...
    rtc_phase_observer_ = timing_.rtc_clock->add_phase_observer(
        [this](const bool before, const std::uint64_t cycle) {
            handle_rtc_phase_reset(before, cycle);
        }
    );
    scheduler_reset_observer_ = scheduler_.add_reset_observer([this] {
...
void Sh4Tmu::synchronize(const std::size_t index) {
    ...
    const auto ticks = uses_rtc_clock(value)
...
void Sh4Tmu::schedule_underflow(const std::size_t index) {
    ...
    value.event = scheduler_.schedule_at(deadline, [this, index](const auto, const auto) {
...
void Sh4Tmu::handle_rtc_phase_reset(
    const bool before_reset,
    const std::uint64_t guest_cycle
) {
...
Sh4Rtc::Sh4Rtc(
    ...
    scheduler,
    std::make_shared<Sh4RtcClockDomain>(guest_cycles_per_second)
) {}
...
bool Sh4Rtc::running() const noexcept { return calendar_running_; }
...
bool Sh4Rtc::rtc_enabled() const noexcept { return rtc_enabled_; }
...
void Sh4Rtc::set_carry_interrupt_enabled(const bool enabled) noexcept { carry_enabled_ = enabled; }
bool Sh4Rtc::carry_flag() const noexcept { return carry_flag_; }
std::uint8_t Sh4Rtc::counter_64hz() const noexcept { return counter_64hz_; }
bool Sh4Rtc::periodic_interrupt_pending() const noexcept { return periodic_pending_; }
bool Sh4Rtc::carry_interrupt_pending() const noexcept { return carry_flag_ && carry_enabled_; }
void Sh4Rtc::acknowledge_periodic_interrupt() noexcept { periodic_pending_ = false; }
void Sh4Rtc::acknowledge_carry_interrupt() noexcept { carry_flag_ = false; }
...
void Sh4Rtc::schedule_tick() {
    ...
    event_ = scheduler_.schedule_at(deadline, [this](const auto, const auto) {
        tick();
    });
}
