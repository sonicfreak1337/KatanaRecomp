...
void validate_context(const CrashReportContext& context) {
    if (!stable_token(context.stop_code)
        || !stable_token(context.block_provenance, true)
    ...
    const bool dispatch_present = context.dispatch_callsite.has_value()
        || context.dispatch_target.has_value();
    if (dispatch_present
        && (context.dispatch_origin.empty() || context.dispatch_action.empty())) {
        throw std::invalid_argument(
...
CrashReport capture_crash_report(const CpuState& cpu, CrashReportContext context) {
    ...
    if (context.block_address.has_value()) {
        context.block_address->physical_address = canonical_physical_address(
    ...
    report.canonical_pc = report.context.canonical_pc.value_or(
        canonical_physical_address(cpu.pc)
    );
...
std::string serialize_crash_report(const CrashReport& report) {
    ...
    output << "{\"schema\":\"katana-crash-report\",\"report_version\":1"
           ...
           << ",\"virtual_pc\":\"" << hex32(report.virtual_pc) << '"'
           << ",\"canonical_pc\":\"" << hex32(report.canonical_pc) << '"'
           << ",\"cpu\":{\"r\":[";
    ...
    output << "],\"pr\":\"" << hex32(report.pr)
           << "\",\"sr\":\"" << hex32(report.sr)
           << "\",\"fpscr\":\"" << hex32(report.fpscr)
           << "\",\"spc\":\"" << hex32(report.spc)
           << "\",\"ssr\":\"" << hex32(report.ssr)
           << "\",\"sgr\":\"" << hex32(report.sgr)
           << "\",\"tea\":\"" << hex32(report.tea)
           << "\",\"expevt\":\"" << hex32(report.expevt)
    ...
    if (context.block_address.has_value() && context.block_variant.has_value()) {
        ...
        output << "{\"virtual_address\":\"" << hex32(address.virtual_address)
               ...
               << "\",\"provenance\":\"" << context.block_provenance
               << "\",\"end_kind\":\"" << block_end_name(context.block_end_kind)
    ...
    output << ",\"pr\":\"" << hex32(report.pr)
           << "\",\"origin\":\"" << context.dispatch_origin
...
const char* exception_cause_name(const ExceptionCause cause) noexcept {
    switch (cause) {
        ...
        case ExceptionCause::Trap: return "trap";
        case ExceptionCause::IllegalInstruction: return "illegal-instruction";
        case ExceptionCause::SlotIllegalInstruction: return "slot-illegal-instruction";
        case ExceptionCause::FpuDisabled: return "fpu-disabled";
        case ExceptionCause::SlotFpuDisabled: return "slot-fpu-disabled";
        case ExceptionCause::AddressErrorRead: return "address-error-read";
        case ExceptionCause::AddressErrorWrite: return "address-error-write";
        case ExceptionCause::BusErrorRead: return "bus-error-read";
        case ExceptionCause::BusErrorWrite: return "bus-error-write";
        case ExceptionCause::Interrupt: return "interrupt";
    }
    return "unknown";
}
