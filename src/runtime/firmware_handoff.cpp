...
namespace {
bool contains(const std::uint32_t start, const std::uint32_t size, const std::uint32_t address) noexcept {
    return address >= start && static_cast<std::uint64_t>(address) < static_cast<std::uint64_t>(start) + size;
}
void validate_range(const std::uint32_t start, const std::uint32_t size, const char* what) {
    if (size == 0u || static_cast<std::uint64_t>(start) + size >
            static_cast<std::uint64_t>(std::numeric_limits<std::uint32_t>::max()) + 1u) {
        throw std::invalid_argument(std::string(what) + " besitzt einen ungueltigen 32-Bit-Bereich.");
    }
}
}

void FirmwareHandoffMap::map_segment(FirmwareMapping mapping) {
    if (mapping.name.empty()) { throw std::invalid_argument("Firmwaresegment braucht einen Namen."); }
    ...
    if (duplicate != mappings_.end()) { throw std::invalid_argument("Doppelte virtuelle Firmwareabbildung."); }
    mappings_.push_back(std::move(mapping));
}

void FirmwareHandoffMap::record_copy(FirmwareCodeCopy copy) {
    if (copy.provenance.empty()) { throw std::invalid_argument("Codekopie braucht Provenienz."); }
...
void FirmwareHandoffMap::mark_copy_changed(const std::uint32_t destination_address) {
    ...
    if (!found) { throw std::out_of_range("Adresse gehoert zu keiner bekannten Firmware-Codekopie."); }
}
...
FirmwareTargetResolution FirmwareHandoffMap::resolve(const std::uint32_t virtual_address) const {
    ...
    for (const auto& copy : copies_) {
        if (contains(copy.destination_physical, copy.size, physical)) {
            return {virtual_address, physical, copy, copy.byte_verified && !copy.changed_after_copy, copy.provenance};
        }
    }
    ...
    if (mapping == mappings_.end()) { throw std::out_of_range("Firmwareziel liegt in keinem bekannten Segment."); }
    return {virtual_address, physical, std::nullopt, mapping->kind == FirmwareSegmentKind::Rom, mapping->name};
}
