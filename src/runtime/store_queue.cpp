...
Sh4StoreQueues::Sh4StoreQueues(
    Memory& memory,
    StoreQueueSink sink,
    ExecutableCodeTracker* code_tracker,
    const OperandCacheRamProfile ocram_profile
) : memory_(memory), sink_(std::move(sink)), code_tracker_(code_tracker), ocram_profile_(ocram_profile) {}
...
std::uint32_t Sh4StoreQueues::qacr(const std::size_t queue) const {
    if (queue >= qacr_.size()) { throw std::out_of_range("Ungueltige Store Queue."); }
    return qacr_[queue];
}

void Sh4StoreQueues::write_p4(
    ...
    if (offset + bytes > 32u || (address & (bytes - 1u)) != 0u) {
        throw std::invalid_argument("Store-Queue-Schreibzugriff ist falsch ausgerichtet oder kreuzt die Queue.");
    }
...
std::uint32_t Sh4StoreQueues::transfer_target(
    const std::uint32_t address,
...
bool Sh4StoreQueues::prefetch(const std::uint32_t address) {
    if (address < window_start || address > window_end) { return false; }
    ...
    transfer.target = transfer.target_address >= 0x10000000u && transfer.target_address <= 0x13FFFFFFu
        ? StoreQueueTarget::TileAccelerator : StoreQueueTarget::Ram;
    transfer.bytes = queues_[selected];
    if (sink_) {
        sink_(transfer);
    } else {
        for (std::size_t index = 0u; index < transfer.bytes.size(); ++index) {
            memory_.write_u8(transfer.target_address + static_cast<std::uint32_t>(index), transfer.bytes[index]);
        }
    }
...
const std::array<std::uint8_t, 32u>& Sh4StoreQueues::queue(const std::size_t index) const {
    if (index >= queues_.size()) { throw std::out_of_range("Ungueltige Store Queue."); }
    return queues_[index];
}
std::uint64_t Sh4StoreQueues::transfer_count() const noexcept { return transfer_count_; }

CacheMaintenanceResult Sh4StoreQueues::maintain(
    const CacheMaintenanceOperation operation,
    const std::uint32_t address,
...
void Sh4StoreQueues::set_operand_cache_ram_enabled(const bool enabled) {
    if (enabled && ocram_profile_ == OperandCacheRamProfile::Reject) {
        throw std::runtime_error("CCR-Operand-Cache-RAM ist im aktiven LLE-Profil nicht modelliert.");
    }
    operand_cache_ram_enabled_ = enabled;
}
bool Sh4StoreQueues::operand_cache_ram_enabled() const noexcept { return operand_cache_ram_enabled_; }
