...
Iso9660Entry parse_record(const std::span<const std::uint8_t> bytes, const std::size_t offset) {
    ...
    std::string name(reinterpret_cast<const char*>(bytes.data() + offset + 33u), name_length);
    return {
        normalize_name(std::move(name)), extent_le, size_le, (bytes[offset + 25u] & 0x02u) != 0u};
}

std::uint64_t sector_offset(const std::uint32_t volume_start_lba,
...
Iso9660Filesystem::Iso9660Filesystem(std::shared_ptr<const DiscSource> source,
                                     const std::uint32_t sector_size,
    ...
    if (!source_) {
        throw std::invalid_argument("ISO9660 braucht eine Disc-Quelle.");
    }
    if (sector_size_ < 2048u) {
        throw std::invalid_argument("ISO9660-Sektorgroesse ist kleiner als 2048 Byte.");
    }
    const auto descriptor =
        source_->read(sector_offset(volume_start_lba_, 16u, sector_size_), sector_size_);
    if (descriptor[0] != 1u ||
        std::string_view(reinterpret_cast<const char*>(descriptor.data() + 1u), 5u) != "CD001" ||
        descriptor[6] != 1u) {
