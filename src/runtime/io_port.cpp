...
make_register(const std::shared_ptr<Sh4IoPort>& state,
...
std::uint32_t Sh4IoPort::control_a() const noexcept { return control_a_; }
...
std::uint32_t Sh4IoPort::control_b() const noexcept { return control_b_; }

std::uint16_t Sh4IoPort::data_b() const noexcept {
    const auto outputs = output_mask(control_b_, 4u);
    return static_cast<std::uint16_t>(((data_b_latch_ & outputs) |
                                      (inputs_.port_b & ~outputs)) &
                                     0x000Fu);
}
...
void Sh4IoPort::write_control_a(const std::uint32_t value) noexcept { control_a_ = value; }
void Sh4IoPort::write_data_a(const std::uint16_t value) noexcept { data_a_latch_ = value; }
void Sh4IoPort::write_control_b(const std::uint32_t value) noexcept { control_b_ = value & 0xFFu; }
void Sh4IoPort::write_data_b(const std::uint16_t value) noexcept { data_b_latch_ = value & 0xFu; }
void Sh4IoPort::write_gpio_interrupt_control(const std::uint16_t value) noexcept {
    gpio_interrupt_control_ = value;
}
void Sh4IoPort::set_inputs(const Sh4IoPortInputs inputs) noexcept { inputs_ = inputs; }
...
std::shared_ptr<Sh4IoPort> map_sh4_io_ports(Memory& memory, const Sh4IoPortInputs inputs) {
    ...
    memory.map_region(
        "sh4-pctra", sh4_port_control_a_address,
        make_register(state, MemoryAccessWidth::Word,
    ...
    memory.map_region(
        "sh4-pdtra", sh4_port_data_a_address,
        make_register(state, MemoryAccessWidth::Halfword,
                      [state](std::uint32_t, MemoryAccessWidth) { return state->data_a(); },
                      [state](std::uint32_t, std::uint32_t value, MemoryAccessWidth) {
                          state->write_data_a(static_cast<std::uint16_t>(value));
                      }));
    memory.map_region(
        "sh4-pctrb", sh4_port_control_b_address,
        make_register(state, MemoryAccessWidth::Word,
    ...
    memory.map_region(
        "sh4-pdtrb", sh4_port_data_b_address,
        make_register(state, MemoryAccessWidth::Halfword,
    ...
    memory.map_region(
        "sh4-gpioic", sh4_gpio_interrupt_control_address,
        make_register(
            state, MemoryAccessWidth::Halfword,
