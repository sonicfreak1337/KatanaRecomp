...
std::string quote_json(const std::string_view value) {
    ...
    for (const unsigned char character : value) {
        ...
        default:
            if (character < 0x20u) {
                output << "\\u" << std::hex << std::uppercase << std::setfill('0') << std::setw(4)
...
std::string serialize_runtime_provenance_json(const ExecutableCodeTracker& code,
    ...
    std::sort(copies.begin(), copies.end(), [](const auto& left, const auto& right) {
        return std::tie(
                   left.destination_physical, left.source_physical, left.size, left.provenance) <
               std::tie(
                   right.destination_physical, right.source_physical, right.size, right.provenance);
    });
    ...
    for (auto& [key, mappings] : aliases) {
        ...
        std::sort(mappings.begin(), mappings.end(), [](const auto& left, const auto& right) {
            return std::tie(left.virtual_start, left.name) <
                   std::tie(right.virtual_start, right.name);
        });
    }
    ...
    for (std::size_t index = 0u; index < blocks.size(); ++index) {
        ...
        output << "{\"identity\":" << quote_json(portable_label(tracked.block.identity))
               ...
               << ",\"valid\":" << (tracked.valid ? "true" : "false") << ",\"incoming_links\":[";
    ...
    for (const auto& [key, mappings] : aliases) {
        ...
        output << "{\"physical_start\":\"" << hex32(key.first) << "\",\"size\":" << key.second
               << ",\"aliases\":[";
    ...
    for (std::size_t index = 0u; index < invalidations.size(); ++index) {
        ...
        output << "{\"sequence\":" << event.sequence << ",\"virtual_address\":\""
               << hex32(event.virtual_address) << "\",\"physical_address\":\""
               << hex32(event.physical_address) << "\",\"size\":" << event.size
