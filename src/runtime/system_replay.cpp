...
bool external_kind(const SystemReplayEventKind kind) noexcept {
    return kind == SystemReplayEventKind::ExternalInput
        || kind == SystemReplayEventKind::HostEvent;
}
...
void hash_event(std::uint64_t& hash, const SystemReplayEvent& event) noexcept {
    ...
    for (const unsigned char value : event.code) hash_byte(hash, value);
    hash_byte(hash, 0u);
...
const char* safepoint_kind_name(const SafepointKind kind) noexcept {
    switch (kind) {
        case SafepointKind::BlockEnd: return "block-end";
        case SafepointKind::LoopBackedge: return "loop-backedge";
        case SafepointKind::BeforeDelaySlot: return "before-delay-slot";
        case SafepointKind::AfterDelaySlot: return "after-delay-slot";
    }
    return "unknown";
}
...
bool SystemReplayLog::sealed() const noexcept { return final_guest_state_hash_.has_value(); }
const std::vector<SystemReplayEvent>& SystemReplayLog::events() const noexcept { return events_; }
std::uint64_t SystemReplayLog::dropped_events() const noexcept { return dropped_events_; }

std::uint64_t SystemReplayLog::event_hash() const noexcept {
    auto hash = fnv_offset;
    for (const auto& event : events_) hash_event(hash, event);
    return hash;
}
...
std::string SystemReplayLog::serialize_json() const {
    ...
    output << "{\"schema\":\"katana-system-replay\",\"report_version\":1"
           << ",\"replay_version\":" << system_replay_schema_version
           << ",\"status\":\"" << (dropped_events_ == 0u ? "success" : "failed")
           << "\",\"sealed\":" << (sealed() ? "true" : "false")
           << ",\"dropped_events\":" << dropped_events_
           << ",\"event_hash\":\"" << hex64(event_hash())
    ...
    for (std::size_t index = 0u; index < events_.size(); ++index) {
        ...
        output << "{\"sequence\":" << event.sequence
               ...
               << ",\"guest_cycle\":" << event.guest_cycle
               << ",\"kind\":\"" << system_replay_event_kind_name(event.kind)
               << "\",\"code\":\"" << event.code << "\",\"address\":";
...
SystemReplayMismatch::SystemReplayMismatch(
    const std::size_t event_index,
    std::string reason
) : std::runtime_error(
        "Systemreplay weicht bei Ereignis " + std::to_string(event_index) + " ab: " + reason
    ), event_index_(event_index) {}

std::size_t SystemReplayMismatch::event_index() const noexcept { return event_index_; }
...
void DeterministicSystemReplay::observe(SystemReplayEvent event) {
    if (finished_) throw std::logic_error("Abgeschlossener Systemreplay kann nicht fortgesetzt werden.");
    validate_event(event);
...
std::size_t DeterministicSystemReplay::position() const noexcept { return position_; }
bool DeterministicSystemReplay::complete() const noexcept { return finished_; }
...
SystemReplayEvent make_safepoint_replay_event(const SafepointReport& report) {
    ...
    return {
        ...
        std::string(safepoint_kind_name(report.kind)) + "-" + execution_origin_name(report.origin),
        std::nullopt,
        flags,
...
MemoryAccessObserver system_replay_mmio_observer(
    SystemReplayLog& log,
    ...
    return [&log, cycle = std::move(guest_cycle), event_code = std::move(code)](
        ...
        try {
            static_cast<void>(log.try_record({
                0u,
                cycle(),
...
std::uint64_t hash_replay_guest_state(
    const CpuState& cpu,
    const std::uint64_t scheduler_cycle,
    const std::uint64_t subsystem_hash
) noexcept {
    auto hash = fnv_offset;
    for (const auto value : cpu.r) hash_u64(hash, value);
    for (const auto value : cpu.r_bank) hash_u64(hash, value);
    for (const auto value : cpu.fr) hash_u64(hash, value);
    for (const auto value : cpu.xf) hash_u64(hash, value);
    for (const auto value : {
        cpu.pc, cpu.pr, cpu.gbr, cpu.vbr, cpu.ssr, cpu.spc, cpu.sgr, cpu.dbr,
        cpu.tra, cpu.tea, cpu.expevt, cpu.intevt, cpu.mach, cpu.macl, cpu.fpul,
        cpu.read_fpscr(), cpu.read_sr(), cpu.last_prefetch_address
    }) hash_u64(hash, value);
    for (const auto value : {
        cpu.t, cpu.s, cpu.q, cpu.m, cpu.trap_pending, cpu.exception_in_delay_slot,
        cpu.sleeping, cpu.last_prefetch_was_store_queue
...
const char* system_replay_event_kind_name(const SystemReplayEventKind kind) noexcept {
    switch (kind) {
        case SystemReplayEventKind::CpuSafepoint: return "cpu-safepoint";
        case SystemReplayEventKind::MmioRead: return "mmio-read";
        case SystemReplayEventKind::MmioWrite: return "mmio-write";
        case SystemReplayEventKind::Dma: return "dma";
        case SystemReplayEventKind::Interrupt: return "interrupt";
        case SystemReplayEventKind::Timer: return "timer";
        case SystemReplayEventKind::SchedulerCallback: return "scheduler-callback";
        case SystemReplayEventKind::Video: return "video";
        case SystemReplayEventKind::Audio: return "audio";
        case SystemReplayEventKind::ExternalInput: return "external-input";
        case SystemReplayEventKind::HostEvent: return "host-event";
    }
    return "unknown";
}
