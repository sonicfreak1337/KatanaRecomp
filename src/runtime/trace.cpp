...
RuntimeTraceRecorder::RuntimeTraceRecorder(const RuntimeTraceConfig config)
    : config_(config) {
...
const RuntimeTraceConfig& RuntimeTraceRecorder::config() const noexcept { return config_; }
const std::vector<RuntimeTraceEvent>& RuntimeTraceRecorder::events() const noexcept {
    return events_;
}
std::uint64_t RuntimeTraceRecorder::total_events() const noexcept { return total_events_; }
std::uint64_t RuntimeTraceRecorder::dropped_events() const noexcept { return dropped_events_; }

MemoryAccessObserver RuntimeTraceRecorder::memory_observer(
    const RuntimeTraceKind kind,
...
std::string RuntimeTraceRecorder::serialize_json() const {
    ...
    output << "{\"schema\":\"katana-runtime-trace\",\"report_version\":1"
           ...
           << ",\"capture_memory_values\":"
           << (config_.capture_memory_values ? "true" : "false")
    ...
    for (std::size_t index = 0u; index < events_.size(); ++index) {
        ...
        output << "{\"sequence\":" << event.sequence
               ...
               << ",\"kind\":\"" << runtime_trace_kind_name(event.kind)
               << "\",\"origin\":\"" << runtime_trace_origin_name(event.origin)
               << "\",\"pc\":\"" << hex32(event.pc) << "\",\"address\":";
        optional_hex(output, event.address);
        output << ",\"width\":";
        event.width.has_value()
            ? output << static_cast<unsigned>(*event.width)
            : output << "null";
        output << ",\"value\":";
...
const char* runtime_trace_kind_name(const RuntimeTraceKind kind) noexcept {
    switch (kind) {
        case RuntimeTraceKind::IrInstruction: return "ir-instruction";
        case RuntimeTraceKind::BlockEntry: return "block-entry";
        case RuntimeTraceKind::BlockExit: return "block-exit";
        case RuntimeTraceKind::MemoryRead: return "memory-read";
        case RuntimeTraceKind::MemoryWrite: return "memory-write";
        case RuntimeTraceKind::Watchpoint: return "watchpoint";
        case RuntimeTraceKind::Exception: return "exception";
        case RuntimeTraceKind::Scheduler: return "scheduler";
    }
    return "unknown";
}
...
const char* runtime_trace_origin_name(const RuntimeTraceOrigin origin) noexcept {
    switch (origin) {
        case RuntimeTraceOrigin::Backend: return "backend";
        case RuntimeTraceOrigin::Fallback: return "fallback";
        case RuntimeTraceOrigin::Runtime: return "runtime";
        case RuntimeTraceOrigin::External: return "external";
    }
    return "unknown";
}
