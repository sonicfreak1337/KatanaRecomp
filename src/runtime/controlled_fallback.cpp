...
DispatchFallbackReason diagnostic_reason(const FallbackReason reason) noexcept {
    switch (reason) {
    case FallbackReason::UnknownOpcode:
        return DispatchFallbackReason::UnknownOpcode;
    case FallbackReason::UnresolvedControlFlow:
        return DispatchFallbackReason::UnresolvedControlFlow;
    case FallbackReason::DynamicCode:
        return DispatchFallbackReason::DynamicCode;
    }
...
DispatchFallbackAction diagnostic_action(const FallbackPolicy policy) noexcept {
    ...
    case FallbackPolicy::Abort:
        return DispatchFallbackAction::Abort;
    case FallbackPolicy::Diagnose:
        return DispatchFallbackAction::Diagnose;
    case FallbackPolicy::Interpreter:
        return DispatchFallbackAction::Interpreter;
    case FallbackPolicy::UserHook:
        return DispatchFallbackAction::UserHook;
    }
...
std::string stable_fallback_reason(const ControlledFallbackRequest& request) {
    ...
    out << reason_name(request.reason) << " pc=0x" << std::hex << std::setw(8) << std::setfill('0')
        << request.guest_pc;
    if (request.opcode) {
        out << " opcode=0x" << std::setw(4) << *request.opcode;
    }
    if (request.target) {
        out << " target=0x" << std::setw(8) << *request.target;
    }
...
ControlledFallbackError::ControlledFallbackError(const ControlledFallbackRequest& request)
    : std::runtime_error("Kontrollierter Fallback abgebrochen: " +
                         stable_fallback_reason(request)) {}

ControlledFallback::ControlledFallback(const FallbackPolicy policy,
    ...
    if ((policy == FallbackPolicy::Interpreter || policy == FallbackPolicy::UserHook) &&
        !handler_) {
        throw std::invalid_argument(
            "Interpreter- oder Nutzerhook-Fallback benoetigt einen expliziten Handler.");
    }
}

ControlledFallbackResult ControlledFallback::enter(CpuState& cpu,
    ...
    if (next_pc != request.block_boundary) {
        diagnose(
            diagnostics_, request, cpu, policy_, next_pc, DispatchDiagnosticError::InvalidBoundary);
