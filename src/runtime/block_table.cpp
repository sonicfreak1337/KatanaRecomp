...
std::string stable_runtime_block_identity(const RuntimeBlock& block) {
    ...
    out << std::hex << std::setfill('0') << "v" << std::setw(8) << block.virtual_start
        << "-p" << std::setw(8) << block.physical_origin << std::dec
...
void RuntimeBlockTable::insert(RuntimeBlock block, const bool runtime_registered) {
    if (block.size == 0u || block.function == nullptr || block.provenance.empty()) {
        throw std::invalid_argument("Blockeintrag benoetigt Groesse, Backendfunktion und Provenienz.");
    }
    if (static_cast<std::uint64_t>(block.virtual_start) + block.size >
        static_cast<std::uint64_t>(std::numeric_limits<std::uint32_t>::max()) + 1u) {
        throw std::length_error("Blockbereich laeuft ueber den 32-Bit-Gastadressraum hinaus: " + block.provenance);
    }
    ...
    for (const auto& existing : blocks_) {
        if (existing.variant == block.variant && ranges_overlap(existing, block)) {
            throw std::invalid_argument(
                "Doppelter oder ueberlappender virtueller Block: " + existing.provenance +
                " <-> " + block.provenance
            );
        }
    }
...
const RuntimeBlock* RuntimeBlockTable::lookup(
...
std::vector<const RuntimeBlock*> RuntimeBlockTable::aliases(
    ...
    for (const auto& block : blocks_) {
        if (block.physical_origin == canonical) { result.push_back(&block); }
    }
    return result;
}

std::size_t RuntimeBlockTable::size() const noexcept { return blocks_.size(); }
void RuntimeBlockTable::clear() noexcept { blocks_.clear(); }
