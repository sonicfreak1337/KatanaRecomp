...
[[noreturn]] void
fail(const std::filesystem::path& path, const std::size_t offset, const std::string& cause) {
    throw std::runtime_error("ELF32-SH-Loaderfehler in " + path.string() + " bei Offset " +
                             std::to_string(offset) + ": " + cause);
}

void require_range(const std::span<const std::uint8_t> bytes,
...
std::string read_string(const std::span<const std::uint8_t> bytes,
...
RelocationKind elf_relocation_kind(const std::uint32_t type) noexcept {
    ...
    case kRelocationDirectory32:
        return RelocationKind::Absolute32;
    case kRelocationRelative32:
        return RelocationKind::PcRelative32;
    default:
        return RelocationKind::Unsupported;
    }
}
...
void validate_entry_point(const ExecutableImage& image,
    ...
    if (segment == nullptr) {
        fail(
            path, 24u, "Einstiegspunkt " + address + " liegt ausserhalb aller geladenen Segmente.");
    }
    if (!segment->permissions.executable) {
        fail(path,
             24u,
             "Einstiegspunkt " + address + " liegt in einem nicht ausfuehrbaren Segment.");
    }
...
ExecutableImage load_elf32_sh(const std::span<const std::uint8_t> bytes,
                              const std::filesystem::path& path) {
    ...
    if (table_size > bytes.size() ||
        program_offset > bytes.size() - static_cast<std::size_t>(table_size)) {
    ...
    for (std::size_t index = 0; index < program_count; ++index) {
        const auto header_offset =
            static_cast<std::size_t>(program_offset) + index * program_entry_size;
    ...
    if (section_count != 0u) {
        ...
        const auto section_table_size =
            static_cast<std::uint64_t>(section_entry_size) * section_count;
        ...
        for (std::size_t section_index = 0; section_index < section_count; ++section_index) {
            const auto header =
                static_cast<std::size_t>(section_offset) + section_index * section_entry_size;
            ...
            const auto string_header =
                static_cast<std::size_t>(section_offset) + string_index * section_entry_size;
            if (read_u32(bytes, string_header + 4u, path, "Stringtabellen-sh_type") !=
                kStringTableSection) {
                fail(path, string_header + 4u, "Symboltabelle verweist nicht auf SHT_STRTAB.");
            }
            const auto string_offset =
                read_u32(bytes, string_header + 16u, path, "Stringtabellen-sh_offset");
            const auto string_size =
                read_u32(bytes, string_header + 20u, path, "Stringtabellen-sh_size");
            ...
            for (std::size_t symbol_index = 0; symbol_index < symbol_records.size();
                 ++symbol_index) {
                const auto symbol =
                    static_cast<std::size_t>(symbol_offset) + symbol_index * symbol_entry_size;
        ...
        for (std::size_t section_index = 0; section_index < section_count; ++section_index) {
            ...
            if (symbol_table_index >= section_count || symbol_tables[symbol_table_index].empty()) {
                fail(path,
                     header + 24u,
                     "Relocationstabelle verweist nicht auf eine geladene Symboltabelle.");
            }
            ...
            for (std::size_t relocation_index = 0;
                ...
                if (symbol_index >= symbol_records.size()) {
                    fail(
                        path, relocation + 4u, "Relocation verweist ausserhalb der Symboltabelle.");
                }
                ...
                ImageRelocation image_relocation{
                    address, raw_type, kind, symbol.name, symbol.value};
                if (kind == RelocationKind::Absolute32 || kind == RelocationKind::PcRelative32) {
                    if (symbol.section_reference == 0u) {
                        fail(path,
                             relocation + 4u,
                             "unterstuetzte Relocation besitzt ein ungeloestes Symbol.");
                    }
                    try {
