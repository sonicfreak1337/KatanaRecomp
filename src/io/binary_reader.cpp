...
std::vector<std::uint8_t> read_binary_file(
    ...
    if (!input) {
        throw std::runtime_error(
            "Binärdatei konnte nicht geöffnet werden: " +
            path.string()
        );
    }
    ...
    if (end_position < 0) {
        throw std::runtime_error(
            "Größe der Binärdatei konnte nicht bestimmt werden."
        );
    }
    ...
    if (size > 0) {
        ...
        if (!input) {
            throw std::runtime_error(
                "Binärdatei konnte nicht vollständig gelesen werden."
            );
        }
    }
...
std::uint16_t read_u16_le(
    ...
    if (offset >= bytes.size() || bytes.size() - offset < 2) {
        throw std::out_of_range(
            "Für einen 16-Bit-Wert sind nicht genügend Bytes vorhanden."
        );
    }
