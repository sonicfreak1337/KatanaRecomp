...
constexpr std::array<FlashPartitionLayout, 5u> flash_layout{{
    {0u, 0x1A000u, 0x02000u, false},
...
FlashPartitionDiagnostic inspect_partition(
    const std::vector<std::uint8_t>& bytes,
    ...
    result.header_valid = std::equal(
        flash_magic.begin(), flash_magic.end(),
    ...
    const auto maximum_records = std::min(
        record_bits,
        (layout.size - flash_record_size - bitmap_size) / flash_record_size
    );

    struct Aggregate { std::size_t generations = 0u; std::size_t latest = 0u; std::size_t valid = 0u; };
    ...
    for (std::size_t record = 0u; record < maximum_records; ++record) {
        ...
        const bool crc_valid = flash_crc(bytes, offset) == read_u16(bytes, offset + flash_crc_offset);
...
FirmwareDiagnosticReport inspect_firmware_file(
    ...
    report.expected_size = kind == FirmwareImageKind::Bios
        ? katana::runtime::dreamcast_bios_size
        : katana::runtime::dreamcast_flash_size;
    report.size_valid = report.size == report.expected_size;
    report.sha256 = katana::io::sha256_bytes(std::string_view(
    ...
    report.ranges = {
        {0x00000u, 0x10000u, FirmwareRangeKind::Data},
...
std::string format_firmware_diagnostic_json(const FirmwareDiagnosticReport& report) {
    ...
    katana::io::write_json_report_header(
        output, "katana-firmware-diagnostic", "firmware-diagnostic",
    ...
    output << ",\"diagnostic_version\":" << firmware_diagnostic_schema_version
           ...
           << ",\"sha256\":" << katana::io::quote_json(report.sha256)
           << ",\"expected_sha256\":";
    report.expected_sha256.has_value()
        ? output << katana::io::quote_json(*report.expected_sha256)
        : output << "null";
    output << ",\"hash_valid\":" << (report.hash_valid ? "true" : "false")
           << ",\"read_only\":true"
    ...
    for (std::size_t index = 0u; index < report.partitions.size(); ++index) {
        ...
        output << "{\"id\":" << static_cast<unsigned>(partition.id)
               ...
               << ",\"header_valid\":";
        partition.block_allocated
            ? output << (partition.header_valid ? "true" : "false")
            : output << "null";
        output
               << ",\"allocated_records\":" << partition.allocated_records
        ...
        for (std::size_t block = 0u; block < partition.logical_blocks.size(); ++block) {
            ...
            output << "{\"logical_id\":";
            report.sensitive_values_included
                ? output << logical.logical_id
                : output << "null";
...
const char* firmware_range_kind_name(const FirmwareRangeKind kind) noexcept {
    switch (kind) {
        case FirmwareRangeKind::Code: return "code";
        case FirmwareRangeKind::Data: return "data";
        case FirmwareRangeKind::Unknown: return "unknown";
    }
    return "unknown";
}
