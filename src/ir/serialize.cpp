...
std::string_view address_update_name(const AddressUpdateKind value) noexcept {
    switch (value) {
    case AddressUpdateKind::None:
        return "none";
    case AddressUpdateKind::PreDecrement:
        return "pre_decrement";
    case AddressUpdateKind::PostIncrement:
...
std::string_view memory_region_name(const MemoryRegionKind value) noexcept {
    ...
    case MemoryRegionKind::Unknown:
        return "unknown";
    case MemoryRegionKind::NormalRam:
        return "normal_ram";
    case MemoryRegionKind::Volatile:
        return "volatile";
    }
    return "unknown";
}
...
std::string_view delay_role_name(const DelaySlotRole value) noexcept {
    switch (value) {
    case DelaySlotRole::None:
        return "none";
    case DelaySlotRole::Owner:
        return "owner";
    case DelaySlotRole::Slot:
        return "slot";
    }
    return "unknown";
}
...
template <typename Value> std::vector<Value> sorted_values(const std::vector<Value>& values) {
    auto result = values;
...
sorted_functions(const std::span<const Function> functions,
    ...
    for (std::size_t index = 1u; index < result.size(); ++index) {
        if (result[index - 1u]->entry_address == result[index]->entry_address) {
            throw std::invalid_argument("IR-Programm besitzt doppelte Funktionseinstiege.");
        }
    }
    return result;
}
...
const char* dynamic_target_class_name(const DynamicTargetClass value) noexcept {
    switch (value) {
    case DynamicTargetClass::NotApplicable:
        return "not-applicable";
    case DynamicTargetClass::GuardedComplete:
        return "guarded-complete";
    case DynamicTargetClass::GuardedPartial:
        return "guarded-partial";
    case DynamicTargetClass::RuntimeOnly:
        return "runtime-only";
...
void emit_text_instruction(std::ostringstream& output, const Instruction& value) {
    output << "    instruction " << hex32(value.source_address)
           ...
           << " source=r" << static_cast<unsigned>(value.source_register) << " branch=r"
           << static_cast<unsigned>(value.branch_register) << " immediate=" << value.immediate
           << " displacement=" << value.displacement
           << " special=" << special_register_name(value.special_register) << " forwarded="
           << (value.forwarded_value_register
                   ? "r" + std::to_string(*value.forwarded_value_register)
                   : "null")
           ...
           << " dynamic_target_class=" << dynamic_target_class_name(value.dynamic_target_class)
           << " resolved_targets=";
    ...
    output << '\n'
           << "      delay role=" << delay_role_name(value.delay_slot.role) << " counterpart="
           << (value.delay_slot.counterpart_address ? hex32(*value.delay_slot.counterpart_address)
                                                    : "null")
...
void emit_json_instruction(std::ostringstream& output, const Instruction& value) {
    output << '{' << "\"address\":\"" << hex32(value.source_address) << "\","
           ...
           << "\"displacement\":" << value.displacement << ',' << "\"special_register\":\""
           << special_register_name(value.special_register) << "\","
    ...
    if (value.effective_address)
        output << '"' << hex32(*value.effective_address) << '"';
    else
        output << "null";
    ...
    output << ",\"branch_register_relative\":"
           << (value.branch_register_relative ? "true" : "false") << ",\"dynamic_target_class\":\""
           << dynamic_target_class_name(value.dynamic_target_class) << "\",\"resolved_targets\":";
    ...
    output << "},\"memory_effects\":{"
           ...
           << static_cast<unsigned>(value.memory_effects.updated_register_count) << ",\"region\":\""
           << memory_region_name(value.memory_effects.region) << "\""
