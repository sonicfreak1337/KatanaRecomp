...
const Instruction& controlling_instruction(const BasicBlock& block) {
    ...
    if (
        last.delay_slot.role == DelaySlotRole::Slot &&
...
void verify_memory_effects(
    ...
    if (!has_access && effects.region != MemoryRegionKind::Unknown) {
        add_issue(issues, instruction.source_address,
            "Speicherregion ist ohne Speicherzugriff gesetzt.");
    }
}
...
std::vector<VerificationIssue> verify_function(const Function& function) {
    ...
    for (const auto& block : function.blocks) {
        ...
        for (std::size_t index = 0u; index < block.instructions.size(); ++index) {
            ...
            auto expected_memory_effects = instruction_memory_effects(
            ...
            if (
                requires_effective_address(instruction.operation) &&
            ...
            if (
                requires_special_register(instruction.operation) &&
    ...
    for (const auto& block : function.blocks) {
        ...
        switch (control.operation) {
            ...
            case Operation::CallRegister:
                ...
                expected_callees.insert(
                    expected_callees.end(),
        ...
        if (actual_successors != expected_successors) {
            add_issue(issues, block.start_address,
                "Blocknachfolger passen nicht zur Terminaloperation.");
        }
        const bool expected_indirect =
    ...
    if (
        actual_callees != function.direct_callees ||
        actual_callees != expected_callees
    ) {
        add_issue(issues, function.entry_address,
            "Direkte Callee-Metadaten sind nicht kanonisch oder veraltet.");
    }
    if (
        actual_indirect_sites != function.indirect_call_sites ||
...
std::vector<VerificationIssue> verify_program(
