...
[[nodiscard]] std::uint64_t invocation_units(
    ...
    // CMake can place the sole source in a response file. The compiler
    // launcher itself is nevertheless one real compile-edge admission.
...
[[nodiscard]] bool write_new_event(
    ...
    try {
        ...
        valid = valid && ::fsync(descriptor) == 0;
...
int run_host_build_tool_launcher(
    ...
    const std::span<const char* const> arguments) noexcept {
    try {
        if (event_root.empty() || tool.empty()) return 125;
        ...
        const auto root_attributes =
            GetFileAttributesW(event_root.c_str());
        if (root_attributes == INVALID_FILE_ATTRIBUTES ||
            (root_attributes & FILE_ATTRIBUTE_REPARSE_POINT) != 0u)
            return 125;
#endif
        const auto units = invocation_units(kind, arguments);
        const auto base = unique_event_path(event_root, kind);
        const auto started =
            std::filesystem::path(base.string() + ".started");
        if (!write_new_event(started, kind, units)) return 125;
        const auto result = run_tool(tool, arguments);
        const auto terminal = std::filesystem::path(
            base.string() +
            (result == 0 ? ".committed" : ".failed"));
        std::error_code rename_error;
        std::filesystem::rename(started, terminal, rename_error);
        if (rename_error) return 125;
        return result;
    } catch (...) {
        return 125;
    }
