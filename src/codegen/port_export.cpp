...
std::string handwritten_main(const std::string& entry_namespace,
    ...
    return "#include \"katana_port.hpp\"\n"
           ...
           "                if (!scanout->video_blank && !first_guest_frame_reported) {\n"
...
std::string runtime_dispatch_adapter(const std::string& entry_namespace,
    ...
    output << "katana::runtime::BlockExit dispatch_dynamic_interpreter(\n"
              ...
              "        katana::runtime::execute_dynamic_sh4_block(cpu, *active_services, 64u);\n"
    ...
    output << "    katana::runtime::DispatchDiagnosticRecorder diagnostics;\n"
           ...
           << "            std::size_t size = 0u;\n"
           << "            for (std::size_t count = 0u; count < 64u && size + 2u <= bytes.size();\n"
           << "                 ++count) {\n"
           << "                const auto opcode = static_cast<std::uint16_t>(bytes[size]) |\n"
           << "                    static_cast<std::uint16_t>(bytes[size + 1u] << 8u);\n"
           << "                const auto decoded = katana::sh4::decode(opcode);\n"
           << "                if (!decoded.is_known()) return candidate;\n"
           << "                size += 2u; ++candidate.instructions;\n"
           << "                if (decoded.changes_control_flow()) {\n"
           << "                    if (decoded.has_delay_slot) {\n"
           << "                        if (size + 2u > bytes.size()) return candidate;\n"
           << "                        const auto slot_opcode = static_cast<std::uint16_t>(bytes[size]) |\n"
           << "                            static_cast<std::uint16_t>(bytes[size + 1u] << 8u);\n"
