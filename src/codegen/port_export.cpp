...
void ensure_safe_port_directory(
    ...
    for (const auto& component : relative) {
        ...
        if (component == "..")
            throw std::runtime_error(
                "Portverzeichnis verlaesst sein Ausgabeziel.");
        current /= component;
        ...
        if (status_error == std::errc::no_such_file_or_directory ||
            ...
            if (!std::filesystem::create_directory(
                ...
                throw std::filesystem::filesystem_error(
                    "Portverzeichnis konnte nicht erstellt werden.",
                    current,
                    status_error);
            status =
                std::filesystem::symlink_status(current, status_error);
        ...
        if (status_error ||
            ...
            unsafe_port_path_link(current, status))
            throw std::runtime_error(
                "Portverzeichnis ist kein sicherer regulaerer Ordner.");
    }
}
...
void validate_game_project_image_contract(
    ...
    for (const auto& table : definition.jump_tables) {
        ...
        if (segment == nullptr || !segment->permissions.readable)
            throw std::invalid_argument(
                "Game-Project-Jump-Table ist nicht lesbar im "
...
bool is_exact_return_nop_callback(
    const katana::ir::BasicBlock& block) noexcept {
    using katana::ir::DelaySlotRole;
    using katana::ir::Operation;

    if (block.instructions.size() != 2u) return false;
    const auto& terminal = block.instructions[0];
    const auto& delay = block.instructions[1];
    return terminal.source_address == block.start_address &&
           terminal.original_opcode == 0x000Bu &&
           terminal.original_operation == Operation::Return &&
           terminal.operation == Operation::Return &&
           terminal.delay_slot.role == DelaySlotRole::Owner &&
           terminal.delay_slot.counterpart_address == delay.source_address &&
           delay.source_address == block.start_address + 2u &&
           delay.original_opcode == 0x0009u &&
           delay.original_operation == Operation::Nop &&
           delay.operation == Operation::Nop &&
           delay.delay_slot.role == DelaySlotRole::Slot &&
           delay.delay_slot.counterpart_address == terminal.source_address;
}
...
void require_guarded_aot_program_entries(
    ...
    for (const auto& entry : entries) {
        ...
        if (divergent_blocks.contains(entry.guest_address))
            throw std::runtime_error(
                "IR-Basic-Block besitzt abweichende Funktionsbesitzer bei " +
                guarded_aot_address(entry.guest_address) + " (" +
                std::string(phase) + ").");
...
struct MemoryFillLoopBatchProof {
    ...
    struct Descriptor {
        ...
        std::uint32_t decrement_instruction_address = 0u;
        std::uint8_t source_register = 0u;
        std::uint8_t value_register = 0u;
        std::uint8_t counter_register = 0u;
    } descriptor;
};
...
struct CompositeCallbackBatchProof {
    ...
    struct Descriptor {
        ...
        std::uint32_t kernel_body_address = 0u;
        ...
        std::uint32_t kernel_body_size = 0u;
        ...
        std::uint64_t kernel_body_guest_cycles = 0u;
...
memory_fill_loop_batch_proofs(const std::span<const katana::ir::Function> program) {
    ...
    const auto exact_delay_roles = [](
            ...
            std::initializer_list<katana::ir::DelaySlotRole> expected) {
        if (block.instructions.size() != expected.size()) return false;
        return std::equal(expected.begin(), expected.end(), block.instructions.begin(),
                          [](const auto role, const auto& instruction) {
    ...
    // Direct scalar-copy kernels are common compiler output on SH-4. Keeping
    // this proof block-local makes the optimization independent of symbols,
    // function names and game-specific addresses while retaining an exact
    // scalar fallback for every unproven state.
    for (const auto& function : program) {
        for (const auto& block : function.blocks) {
            if (duplicate_block_addresses.contains(block.start_address) ||
                block.instructions.size() != 5u || block.has_indirect_successor ||
                block.start_address > std::numeric_limits<std::uint32_t>::max() - 10u ||
                !exact_delay_roles(block,
                                   {katana::ir::DelaySlotRole::None,
                                    katana::ir::DelaySlotRole::None,
                                    katana::ir::DelaySlotRole::None,
            ...
            for (std::size_t index = 0u; index < block.instructions.size(); ++index) {
                if (block.instructions[index].source_address !=
                    block.start_address + static_cast<std::uint32_t>(index * 2u)) {
                    contiguous = false;
                    break;
                }
            }
            ...
            const auto& load = block.instructions[0];
            const auto& decrement = block.instructions[1];
            const auto& store = block.instructions[2];
            const auto& branch = block.instructions[3];
            const auto& destination_increment = block.instructions[4];
            std::uint8_t copy_width = 0u;
            katana::ir::OperandWidth copy_operand_width =
                katana::ir::OperandWidth::None;
            Operation expected_store = Operation::Unknown;
            switch (load.operation) {
            case Operation::LoadByteSignedPostIncrement:
                copy_width = 1u;
                copy_operand_width = katana::ir::OperandWidth::Bits8;
                expected_store = Operation::StoreByte;
                break;
            case Operation::LoadWordSignedPostIncrement:
                copy_width = 2u;
                copy_operand_width = katana::ir::OperandWidth::Bits16;
                expected_store = Operation::StoreWord;
                break;
            case Operation::LoadLongPostIncrement:
                copy_width = 4u;
                copy_operand_width = katana::ir::OperandWidth::Bits32;
                expected_store = Operation::StoreLong;
                break;
            default:
                break;
            }
            if (copy_width == 0u || load.forwarded_value_register.has_value() ||
                load.memory_effects.access != katana::ir::MemoryAccessKind::Read ||
                load.memory_effects.width != copy_operand_width ||
                load.memory_effects.access_count != 1u ||
                load.memory_effects.region == katana::ir::MemoryRegionKind::Volatile ||
                decrement.operation != Operation::DecrementAndTest ||
                store.operation != expected_store ||
                store.memory_effects.access != katana::ir::MemoryAccessKind::Write ||
                store.memory_effects.width != copy_operand_width ||
                store.memory_effects.access_count != 1u ||
                store.memory_effects.region == katana::ir::MemoryRegionKind::Volatile ||
                store.source_register != load.destination_register ||
                branch.operation != Operation::BranchIfFalse ||
                !branch.target_address.has_value() ||
                *branch.target_address != block.start_address ||
                ...
                destination_increment.immediate != copy_width ||
                !exact_successors(
                    block, {block.start_address, block.start_address + 10u}))
                continue;

            const std::array registers{
                load.source_register,
                load.destination_register,
                decrement.destination_register,
                store.destination_register,
            };
            auto unique_registers = registers;
            std::sort(unique_registers.begin(), unique_registers.end());
            ...
            std::uint64_t round_cycles = 0u;
            for (const auto& instruction : block.instructions)
                round_cycles +=
                    katana::sh4::instruction_timing(instruction.original_opcode)
                        .guest_cycles;
            const auto load_timing =
                katana::sh4::instruction_timing(load.original_opcode);
            const auto store_timing =
                katana::sh4::instruction_timing(store.original_opcode);
            constexpr auto byte_max =
                static_cast<std::uint64_t>(std::numeric_limits<std::uint8_t>::max());
            if (!load_timing.requires_cycle_flush || !store_timing.requires_cycle_flush ||
                load_timing.guest_cycles == 0u || store_timing.guest_cycles == 0u ||
                store_timing.guest_cycles > byte_max || round_cycles == 0u ||
                round_cycles > 4096u || block.instructions.size() > byte_max)
                continue;

            MemoryFillLoopBatchProof::Descriptor descriptor;
            descriptor.kind = MemoryFillLoopBatchProof::Kind::DirectMemoryCopy;
            descriptor.guard_address = block.start_address;
            descriptor.body_address = block.start_address;
            descriptor.exit_address = block.start_address + 10u;
            descriptor.store_instruction_address = store.source_address;
            descriptor.increment_instruction_address =
                destination_increment.source_address;
            descriptor.branch_instruction_address = branch.source_address;
            descriptor.guard_size = 10u;
            descriptor.body_size = 10u;
            descriptor.cursor_register = store.destination_register;
            descriptor.fill_register = load.destination_register;
            descriptor.limit_register = decrement.destination_register;
            descriptor.limit_pointer_register = load.source_register;
            descriptor.store_width = copy_width;
            descriptor.step = copy_width;
            descriptor.round_instruction_count =
            ...
            descriptor.decrement_instruction_address = decrement.source_address;
            descriptor.source_register = load.source_register;
            descriptor.value_register = load.destination_register;
            descriptor.counter_register = decrement.destination_register;
            result.push_back({descriptor});
        }
    }
    std::sort(result.begin(), result.end(), [](const auto& left, const auto& right) {
        return std::tuple{left.descriptor.guard_address,
                          left.descriptor.body_address,
                          left.descriptor.kind} <
               std::tuple{right.descriptor.guard_address,
                          right.descriptor.body_address,
...
std::vector<CompositeCallbackBatchProof> composite_callback_batch_proofs(
    ...
    struct KernelProof {
        const katana::ir::Function* function = nullptr;
        const katana::ir::BasicBlock* entry = nullptr;
        const katana::ir::BasicBlock* body = nullptr;
        const katana::ir::BasicBlock* return_block = nullptr;
        std::uint64_t entry_guest_cycles = 0u;
        std::uint64_t body_guest_cycles = 0u;
    ...
    for (const auto& candidate : function_candidates) {
        const bool runtime_provenance = std::any_of(
        ...
        const bool explicit_override = std::find(
    ...
    const auto same_owner_functions = [](const auto& left_owners,
                                         const auto& right_owners) {
        std::vector<const katana::ir::Function*> left;
        std::vector<const katana::ir::Function*> right;
        left.reserve(left_owners.size());
        right.reserve(right_owners.size());
        for (const auto& owner : left_owners) left.push_back(owner.function);
        for (const auto& owner : right_owners) right.push_back(owner.function);
    ...
    for (const auto& [kernel_address, entry_owners] : block_owners) {
        if (!provenance_strong_candidates.contains(kernel_address) ||
            entry_owners.empty())
            continue;
        const auto& canonical_owner = entry_owners.front();
        const auto& function = *canonical_owner.function;
        const auto& entry = *canonical_owner.block;
        if (std::any_of(
                entry_owners.begin(), entry_owners.end(), [&](const auto& owner) {
                    return !equivalent_ir_block(entry, *owner.block);
                }))
            continue;

        const katana::ir::BasicBlock* loop_body = nullptr;
        bool split_kernel = false;
        if (exact_contiguous_block(entry, 6u) &&
            !entry.has_indirect_successor &&
            exact_delay_roles(entry,
                              {katana::ir::DelaySlotRole::None,
                               katana::ir::DelaySlotRole::None,
                               katana::ir::DelaySlotRole::None,
                               katana::ir::DelaySlotRole::None,
            ...
            if (!exact_contiguous_block(entry, 1u) ||
                entry.has_indirect_successor ||
                !exact_delay_roles(entry, {katana::ir::DelaySlotRole::None}) ||
                entry.start_address >
                    std::numeric_limits<std::uint32_t>::max() - 2u)
                continue;
            const auto body_address = entry.start_address + 2u;
            if (!exact_successors(entry, {body_address}))
                continue;
            const auto body_found = block_owners.find(body_address);
            if (body_found == block_owners.end() || body_found->second.empty() ||
                !same_owner_functions(entry_owners, body_found->second))
                continue;
            const auto& body = *body_found->second.front().block;
            if (std::any_of(
                    body_found->second.begin(), body_found->second.end(),
                    [&](const auto& owner) {
                        return !equivalent_ir_block(body, *owner.block);
                    }) ||
                !exact_contiguous_block(body, 5u) ||
                body.has_indirect_successor ||
        ...
        const auto& shift = entry.instructions[0];
        const auto& load = loop_body->instructions[split_kernel ? 0u : 1u];
        const auto& decrement =
            loop_body->instructions[split_kernel ? 1u : 2u];
        const auto& store = loop_body->instructions[split_kernel ? 2u : 3u];
        const auto& branch = loop_body->instructions[split_kernel ? 3u : 4u];
        const auto& delay_increment =
            loop_body->instructions[split_kernel ? 4u : 5u];
        ...
        const auto entry_cycles = block_guest_cycles(entry);
        const auto body_cycles = split_kernel
            ? block_guest_cycles(*loop_body)
            : std::optional<std::uint64_t>{0u};
        const auto return_cycles = block_guest_cycles(return_block);
        if (!entry_cycles || !body_cycles || !return_cycles ||
            *entry_cycles >
                std::numeric_limits<std::uint64_t>::max() - *body_cycles ||
            *entry_cycles + *body_cycles >
                std::numeric_limits<std::uint64_t>::max() - *return_cycles)
            continue;
        kernels.emplace(
            entry.start_address,
            KernelProof{&function,
                        &entry,
                        split_kernel ? loop_body : nullptr,
                        &return_block,
                        *entry_cycles,
                        *body_cycles,
                        *entry_cycles + *body_cycles + *return_cycles});
    }
    ...
    for (const auto& function : program) {
        for (const auto& call_block : function.blocks) {
            ...
            if (source_setup.operation != Operation::MovRegister ||
                ...
                continue;
            std::vector<std::uint32_t> callback_candidates;
            const auto runtime_resolution =
                resolution_by_callsite.find(call.source_address);
            if (runtime_resolution != resolution_by_callsite.end() &&
                runtime_resolution->second->register_index ==
                    call.branch_register) {
                callback_candidates =
                    runtime_resolution->second->analysis_candidates;
            } else {
                const auto singleton =
                    singleton_resolution_by_callsite.find(call.source_address);
                if (singleton == singleton_resolution_by_callsite.end() ||
                    singleton->second.resolution->register_index !=
                        call.branch_register)
                    continue;
                callback_candidates.push_back(singleton->second.candidate);
            }
            ...
            constexpr auto byte_max =
                static_cast<std::uint64_t>(std::numeric_limits<std::uint8_t>::max());
            for (const auto candidate : callback_candidates) {
                ...
                const auto current_instructions =
                    kernel->second.entry->instructions.size() +
                    (kernel->second.body != nullptr
                ...
                descriptor.exit_address = exit_address;
                descriptor.kernel_address = kernel->second.entry->start_address;
                descriptor.kernel_body_address = kernel->second.body != nullptr
                    ? kernel->second.body->start_address
                    : 0u;
                descriptor.kernel_return_address =
                    kernel->second.return_block->start_address;
                const auto& copy_body = kernel->second.body != nullptr
                    ? *kernel->second.body
                    : *kernel->second.entry;
                const auto load_index =
                    kernel->second.body != nullptr ? 0u : 1u;
                const auto store_index =
                    kernel->second.body != nullptr ? 2u : 3u;
                descriptor.source_load_instruction_address =
                    copy_body.instructions[load_index].source_address;
                descriptor.target_store_instruction_address =
                    copy_body.instructions[store_index].source_address;
                ...
                descriptor.kernel_size = static_cast<std::uint32_t>(
                    kernel->second.entry->instructions.size() * 2u);
                descriptor.kernel_body_size = kernel->second.body != nullptr
                ...
                descriptor.kernel_guest_cycles =
                    kernel->second.entry_guest_cycles;
                descriptor.kernel_body_guest_cycles =
                    kernel->second.body_guest_cycles;
...
std::string generated_header(const std::string& entry_namespace) {
    return "#pragma once\n\n"
           ...
           "};\n"
           "enum class MemoryLoopBatchKind : std::uint8_t {\n"
           "    Fill,\n"
           "    DirectMemoryCopy,\n"
           "};\n"
           "struct MemoryFillLoopBatchDescriptor {\n"
           "    MemoryLoopBatchKind kind = MemoryLoopBatchKind::Fill;\n"
           "    std::uint32_t guard_address = 0u;\n"
           "    std::uint32_t body_address = 0u;\n"
           "    std::uint32_t exit_address = 0u;\n"
           "    std::uint32_t store_instruction_address = 0u;\n"
           ...
           "    std::string_view kernel_body_provenance;\n"
...
std::string handwritten_main(
    ...
    identity_contract << "};\n"
                      ...
                      << "struct NativeMmioWaitBackedge {\n"
                         "    std::uint32_t loop_header = 0u;\n"
                         "    std::uint32_t backedge = 0u;\n"
                         "};\n"
                      << "constexpr std::array<NativeMmioWaitBackedge, "
                      << mmio_wait_loops.size()
                      << "u> native_mmio_wait_backedges{{\n";
    for (const auto& proof : mmio_wait_loops) {
        identity_contract
            << "    {" << proof.descriptor.loop_header << "u, "
            << proof.descriptor.backedge_instruction_address << "u},\n";
    }
    identity_contract << "}};\n";
    ...
    return "#include \"katana_port.hpp\"\n"
           ...
           "    }\n"
           "}\n"
           "bool direct_memory_copy_batch_rejected(const std::string_view stage) {\n"
           "    static const bool enabled = [] {\n"
           "        const auto* value = std::getenv(\n"
           "            \"KATANA_PORT_DIRECT_MEMORY_COPY_TRACE\");\n"
           "        return value != nullptr && std::string_view(value) == \"1\";\n"
           "    }();\n"
           "    if (enabled) {\n"
           "        static std::unordered_set<std::string> reported;\n"
           "        if (reported.insert(std::string(stage)).second)\n"
           "            std::cerr << \"KATANA_DIRECT_MEMORY_COPY_REJECT stage=\"\n"
           "                      << stage << '\\n';\n"
           "    }\n"
           "    return false;\n"
           "}\n"
           "void trace_direct_memory_copy_batch_admission(\n"
           ...
           "    if (enabled && !reported) {\n"
           "        reported = true;\n"
           "        std::cerr << \"KATANA_DIRECT_MEMORY_COPY_ADMIT iterations=\"\n"
           "                  << admitted << '\\n';\n"
           "    }\n"
           "}\n"
           ...
           "    katana::runtime::NativeVideoBackend host_video_backend =\n"
           "        katana::runtime::NativeVideoBackend::Unknown;\n"
           "    std::uint64_t host_video_fallbacks = 0u;\n"
           "    katana::runtime::NativeVideoPresentationTelemetry\n"
           "        host_video_presentation;\n"
           "    double post_entry_host_seconds = 0.0;\n"
           "    bool host_video_hardware_accelerated = false;\n"
           ...
           "                         katana::runtime::HostWorkloadLimiter* "
           "workload_limiter = nullptr,\n"
           "                         std::function<void()> product_entry_evidence_callback = {},\n"
           "                         std::function<void()> product_progress_evidence_callback = {},\n"
           ...
           "              std::move(product_progress_evidence_callback)),\n"
           "          game_entry_callback_(std::move(game_entry_callback)),\n"
           "          replay_log_(replay_log),\n"
           "          workload_limiter_(workload_limiter),\n"
           ...
           "          runtime_aot_chaining_enabled_([] {\n"
           "              const auto* value = std::getenv(\"KATANA_RUNTIME_AOT_CHAINING\");\n"
           "              return value != nullptr && std::string_view(value) == \"1\";\n"
           "          }()),\n"
           ...
           "            memory_fill_loop_batching_enabled_ = false;\n"
           "#endif\n"
           "        direct_memory_copy_batching_enabled_ =\n"
           "            memory_fill_loop_batching_enabled_;\n"
           "        if (const auto* disabled = std::getenv(\n"
           "                \"KATANA_PORT_DISABLE_DIRECT_MEMORY_COPY_BATCH\");\n"
           "            disabled != nullptr && std::string_view(disabled) == \"1\")\n"
           "            direct_memory_copy_batching_enabled_ = false;\n"
           ...
           "        const bool forced_host_poll =\n"
           "            lifecycle_poll_ && ++host_poll_calls_since_callback_ >=\n"
           "                host_poll_call_quantum;\n"
           "        if (lifecycle_poll_ &&\n"
           "            (eager_host_poll_ || forced_host_poll || cached_lifecycle_ !=\n"
           "                katana::runtime::PlatformLifecycleState::Running ||\n"
           ...
           "            host_poll_calls_since_callback_ = 0u;\n"
           ...
           "        }\n"
           "        if (workload_limiter_ != nullptr) {\n"
           "            if (cached_lifecycle_ !=\n"
           "                    katana::runtime::PlatformLifecycleState::Running) {\n"
           "                workload_limiter_->reset();\n"
           "            } else\n"
           "                workload_limiter_->limit_if_due();\n"
           "        }\n"
           ...
           "        if (product_progress_evidence_callback_) {\n"
           "            try {\n"
           "                product_progress_evidence_callback_();\n"
           "            } catch (...) {\n"
           ...
           "               << '\\n';\n"
           "    }\n"
           "    void report_runtime_aot_chain_statistics(std::ostream& output) const {\n"
           "        output << \"KATANA_RUNTIME_AOT_CHAIN_STATS contracts=\"\n"
           "               << runtime_aot_chain_contracts_.size()\n"
           "               << \" chainable=\" << runtime_aot_chainable_contracts_\n"
           "               << \" collisions=\" << runtime_aot_chain_contract_collisions_\n"
           "               << \" attempts=\" << runtime_aot_chain_attempts_\n"
           "               << \" admissions=\" << runtime_aot_chain_admissions_\n"
           "               << \" mapping_admissions=\"\n"
           "               << runtime_aot_chain_mapping_admissions_\n"
           "               << \" reject_context=\" << runtime_aot_chain_context_rejections_\n"
           "               << \" reject_target=\" << runtime_aot_chain_target_rejections_\n"
           "               << \" reject_contract=\" << runtime_aot_chain_contract_rejections_\n"
           "               << \" reject_identity=\" << runtime_aot_chain_identity_rejections_\n"
           "               << \" reject_validation=\"\n"
           "               << runtime_aot_chain_validation_rejections_ << '\\n';\n"
           "    }\n"
           ...
           "            &found->second;\n"
           "    }\n"
           "    void reserve_runtime_aot_chain_contracts(\n"
           "            const std::size_t count) override {\n"
           "        runtime_aot_chain_contracts_.reserve(count);\n"
           "    }\n"
           "    void register_runtime_aot_chain_contract(\n"
           "            const std::uint32_t source_address, const std::uint32_t size,\n"
           "            const katana::runtime::ExecutableBlockTimingClass timing_class,\n"
           "            const std::uint64_t maximum_guest_cycles,\n"
           "            const bool chainable) override {\n"
           "        const RuntimeAotChainContract candidate{\n"
           "            size, timing_class, maximum_guest_cycles, chainable, false};\n"
           "        const auto [entry, inserted] =\n"
           "            runtime_aot_chain_contracts_.try_emplace(\n"
           "                source_address, candidate);\n"
           "        if (inserted) {\n"
           "            if (chainable) ++runtime_aot_chainable_contracts_;\n"
           "            return;\n"
           "        }\n"
           "        auto& known = entry->second;\n"
           "        if (!known.ambiguous && known.size == candidate.size &&\n"
           "            known.timing_class == candidate.timing_class &&\n"
           "            known.maximum_guest_cycles == candidate.maximum_guest_cycles &&\n"
           "            known.chainable == candidate.chainable)\n"
           "            return;\n"
           "        if (!known.ambiguous) {\n"
           "            if (known.chainable) --runtime_aot_chainable_contracts_;\n"
           "            ++runtime_aot_chain_contract_collisions_;\n"
           "        }\n"
           "        known.chainable = false;\n"
           "        known.ambiguous = true;\n"
           "    }\n"
           "    void bind_runtime_dispatch_context(\n"
           "            katana::runtime::RuntimeBlockTable* const table,\n"
           "            katana::runtime::DemandBlockMaterializer* const materializer)\n"
           "            noexcept override {\n"
           "        if ((table == nullptr) != (materializer == nullptr) ||\n"
           "            (table != nullptr && state_.runtime_blocks.get() != table)) {\n"
           "            runtime_dispatch_table_ = nullptr;\n"
           "            runtime_dispatch_materializer_ = nullptr;\n"
           "            ++runtime_aot_chain_context_rejections_;\n"
           "            return;\n"
           "        }\n"
           "        runtime_dispatch_table_ = table;\n"
           "        runtime_dispatch_materializer_ = materializer;\n"
           "    }\n"
           ...
           "        prepare_runtime_aot_chain_context(\n"
           "            virtual_start, physical_start, size, variant, runtime_registered);\n"
           ...
           "                [&](const auto& boundary) {\n"
           "                    return boundary.loop_header == target_address &&\n"
           "                        boundary.backedge == source_address;\n"
           "                }))\n"
           ...
           "                if (!runtime_aot_chaining_enabled_)\n"
           "                    return reject(Rejection::TargetNotRegistered);\n"
           "                ++runtime_aot_chain_attempts_;\n"
           "                const auto reject_runtime = [&](\n"
           "                        std::uint64_t& counter,\n"
           "                        const Rejection reason) noexcept {\n"
           "                    ++counter;\n"
           "                    return reject(reason);\n"
           "                };\n"
           "                if (runtime_dispatch_table_ == nullptr ||\n"
           "                    runtime_dispatch_materializer_ == nullptr ||\n"
           "                    state_.runtime_blocks.get() != runtime_dispatch_table_)\n"
           "                    return reject_runtime(\n"
           "                        runtime_aot_chain_context_rejections_,\n"
           "                        Rejection::MissingRuntimeContract);\n"
           "                const auto resolution = resolve_runtime_aot_chain_entry(\n"
           "                    address, physical, *active_block_variant_);\n"
           "                if (!resolution.valid)\n"
           "                    return reject(resolution.rejection);\n"
           "                const auto& contract = *resolution.contract;\n"
           "                if (contract.ambiguous ||\n"
           "                    contract.size != resolution.target_size)\n"
           "                    return reject_runtime(\n"
           "                        runtime_aot_chain_identity_rejections_,\n"
           "                        Rejection::TargetNotNativeEntrySafe);\n"
           "                if (!contract.chainable)\n"
           "                    return reject_runtime(\n"
           "                        runtime_aot_chain_contract_rejections_,\n"
           "                        Rejection::TargetNotNativeEntrySafe);\n"
           "                if (contract.timing_class !=\n"
           "                        katana::runtime::ExecutableBlockTimingClass::PureCpu &&\n"
           "                    contract.timing_class !=\n"
           "                        katana::runtime::ExecutableBlockTimingClass::LinearRamOnly &&\n"
           "                    contract.timing_class !=\n"
           "                        katana::runtime::ExecutableBlockTimingClass::\n"
           "                            RequiresCycleFlush)\n"
           "                    return reject_runtime(\n"
           "                        runtime_aot_chain_contract_rejections_,\n"
           "                        Rejection::TimingNotDeferrable);\n"
           ...
           "                    product_budget_arm_failed_ || !cpu_.address_space ||\n"
           ...
           "                    return reject_runtime(\n"
           "                        runtime_aot_chain_identity_rejections_,\n"
           "                        Rejection::VariantOrGeneration);\n"
           "                if (contract.maximum_guest_cycles >\n"
           "                        local_block_chain_guest_cycle_budget ||\n"
           "                    pending_guest_cycles >\n"
           "                        local_block_chain_guest_cycle_budget -\n"
           "                            contract.maximum_guest_cycles)\n"
           ...
           "                    pending_guest_cycles + contract.maximum_guest_cycles;\n"
           "                if (const auto remaining =\n"
           "                        state_.scheduler->remaining_guest_cycles();\n"
           ...
           "                    event && *event <= current_cycle + prospective_guest_cycles)\n"
           ...
           "                    }\n"
           "                }\n"
           "                if (!runtime_dispatch_materializer_->validate_for_dispatch(\n"
           "                        cpu_, resolution.validation_handle,\n"
           "                        resolution.validation_virtual,\n"
           "                        resolution.validation_physical))\n"
           "                    return reject_runtime(\n"
           "                        runtime_aot_chain_validation_rejections_,\n"
           "                        Rejection::CodeGeneration);\n"
           "                cpu_.active_block_virtual_start = address;\n"
           "                cpu_.active_block_physical_start = physical;\n"
           "                cpu_.active_block_size = resolution.target_size;\n"
           "                if (resolution.same_mapping)\n"
           "                    ++runtime_aot_chain_mapping_admissions_;\n"
           "                ++runtime_aot_chain_admissions_;\n"
           "                return true;\n"
           "            }\n"
           "            if (!registration->chainable)\n"
           "                return reject(Rejection::TargetNotNativeEntrySafe);\n"
           "            // RequiresCycleFlush remains non-deferrable at the central\n"
           "            // dispatcher. A chainable registration in that class is a\n"
           "            // separately emitted proof that every possible boundary is\n"
           "            // an exact guarded linear-RAM read; generated code flushes\n"
           "            // before taking any failed guard's generic path.\n"
           ...
           "            descriptor.kernel_body_address != 0u ||\n"
           ...
           "            descriptor.kernel_body_size != 0u ||\n"
           ...
           "            descriptor.kernel_body_guest_cycles != 0u ||\n"
           ...
           "        const bool split_kernel = descriptor.kernel_body_size != 0u;\n"
           "        if (!composite_callback_batching_enabled_ ||\n"
           "            descriptor.kind != " +
           entry_namespace +
           "::CompositeCallbackBatchKind::MemoryCopy ||\n"
           ...
           "            descriptor.kernel_address >\n"
           "                std::numeric_limits<std::uint32_t>::max() - 16u ||\n"
           "            descriptor.kernel_return_address !=\n"
           "                descriptor.kernel_address + descriptor.kernel_size +\n"
           "                    descriptor.kernel_body_size ||\n"
           "            (split_kernel\n"
           "                 ? descriptor.kernel_size != 2u ||\n"
           "                       descriptor.kernel_body_address !=\n"
           "                           descriptor.kernel_address + 2u ||\n"
           "                       descriptor.kernel_body_size != 10u ||\n"
           "                       descriptor.source_load_instruction_address !=\n"
           "                           descriptor.kernel_body_address ||\n"
           "                       descriptor.target_store_instruction_address !=\n"
           "                           descriptor.kernel_body_address + 4u\n"
           "                 : descriptor.kernel_body_address != 0u ||\n"
           ...
           "        };\n"
           "        const auto live_target = cpu_.r[descriptor.callback_register];\n"
           "        const auto kernel_entry_end_kind = split_kernel\n"
           "            ? katana::runtime::BlockEndKind::Fallthrough\n"
           "            : katana::runtime::BlockEndKind::ConditionalBranch;\n"
           "        if (cpu_.pc != live_target || selected_block.size != descriptor.kernel_size ||\n"
           "            selected_block.end_kind != kernel_entry_end_kind)\n"
           ...
           "        const auto kernel =\n"
           "            executable_blocks_.find(descriptor.kernel_address);\n"
           "        const auto kernel_body = split_kernel\n"
           "            ? executable_blocks_.find(descriptor.kernel_body_address)\n"
           "            : executable_blocks_.end();\n"
           ...
           "            kernel == executable_blocks_.end() ||\n"
           "            (split_kernel &&\n"
           "             kernel_body == executable_blocks_.end()) ||\n"
           ...
           "                    descriptor.kernel_address) ||\n"
           "            (split_kernel &&\n"
           "             kernel_body->second.physical_origin !=\n"
           "                 katana::runtime::canonical_physical_address(\n"
           "                     descriptor.kernel_body_address)) ||\n"
           ...
           "            kernel->second.size != descriptor.kernel_size ||\n"
           "            (split_kernel &&\n"
           "             kernel_body->second.size != descriptor.kernel_body_size) ||\n"
           ...
           "                           katana::runtime::ExecutableBlockTimingClass::RequiresCycleFlush) ||\n"
           "            (split_kernel &&\n"
           "             kernel_body->second.timing_class !=\n"
           "                 katana::runtime::ExecutableBlockTimingClass::LinearRamOnly &&\n"
           "             kernel_body->second.timing_class !=\n"
           "                 katana::runtime::ExecutableBlockTimingClass::RequiresCycleFlush) ||\n"
           ...
           "                descriptor.kernel_guest_cycles ||\n"
           "            (split_kernel &&\n"
           "             kernel_body->second.maximum_guest_cycles !=\n"
           "                 descriptor.kernel_body_guest_cycles) ||\n"
           ...
           "                               descriptor.kernel_provenance) ||\n"
           "            (split_kernel &&\n"
           "             !provenance_matches(kernel_body->second.identity,\n"
           "                                 descriptor.kernel_body_provenance)) ||\n"
           ...
           "                                      kernel->second) ||\n"
           "            (split_kernel &&\n"
           "             !proves_instruction_block(descriptor.kernel_body_address,\n"
           "                                       kernel_body->second)) ||\n"
           ...
           "                registration.physical_origin);\n"
           "            const auto end = begin + registration.size;\n"
           ...
           "        return true;\n"
           "    }\n"
           "    bool try_direct_memory_copy_loop_batch(\n"
           "            const katana::runtime::ValidatedBlockExecution& selected_block,\n"
           "            const " +
           entry_namespace +
           ...
           "        constexpr std::uint64_t quantum = 131'072u;\n"
           "        const auto reject = [](const std::string_view stage) {\n"
           "            return direct_memory_copy_batch_rejected(stage);\n"
           "        };\n"
           "        const auto runtime_state_allows_batch = [&] {\n"
           "            return cpu_.memory.watchpoint_count() == 0u &&\n"
           "                !cpu_.memory.has_trace_handler() &&\n"
           ...
           "        };\n"
           "        if (!direct_memory_copy_batching_enabled_ ||\n"
           "            descriptor.kind != " +
           entry_namespace +
           "::MemoryLoopBatchKind::DirectMemoryCopy ||\n"
           "            descriptor.guard_address != descriptor.body_address ||\n"
           "            descriptor.exit_address != descriptor.body_address + 10u ||\n"
           "            descriptor.source_load_instruction_address !=\n"
           "                descriptor.body_address ||\n"
           "            descriptor.decrement_instruction_address !=\n"
           "                descriptor.body_address + 2u ||\n"
           "            descriptor.store_instruction_address !=\n"
           "                descriptor.body_address + 4u ||\n"
           "            descriptor.branch_instruction_address !=\n"
           "                descriptor.body_address + 6u ||\n"
           "            descriptor.increment_instruction_address !=\n"
           "                descriptor.body_address + 8u ||\n"
           "            descriptor.guard_size != 10u || descriptor.body_size != 10u ||\n"
           "            (descriptor.store_width != 1u &&\n"
           "             descriptor.store_width != 2u &&\n"
           "             descriptor.store_width != 4u) ||\n"
           "            descriptor.step != descriptor.store_width ||\n"
           "            descriptor.round_instruction_count != 5u ||\n"
           "            descriptor.guard_instruction_count != 0u ||\n"
           ...
           "            descriptor.source_register >= cpu_.r.size() ||\n"
           "            descriptor.value_register >= cpu_.r.size() ||\n"
           "            descriptor.counter_register >= cpu_.r.size() ||\n"
           "            descriptor.cursor_register >= cpu_.r.size() ||\n"
           "            descriptor.source_register !=\n"
           "                descriptor.limit_pointer_register ||\n"
           "            descriptor.value_register != descriptor.fill_register ||\n"
           "            descriptor.counter_register != descriptor.limit_register)\n"
           "            return reject(\"descriptor\");\n"
           "        const std::array registers{\n"
           "            descriptor.source_register, descriptor.value_register,\n"
           "            descriptor.counter_register, descriptor.cursor_register};\n"
           "        auto sorted_registers = registers;\n"
           "        std::sort(sorted_registers.begin(), sorted_registers.end());\n"
           "        if (std::adjacent_find(sorted_registers.begin(),\n"
           "                               sorted_registers.end()) !=\n"
           "            sorted_registers.end())\n"
           ...
           "        };\n"
           "        if (cpu_.pc != descriptor.body_address ||\n"
           "            selected_block.runtime_registered || selected_block.aot_template ||\n"
           ...
           "                katana::runtime::BlockEndKind::ConditionalBranch ||\n"
           "            !provenance_matches(selected_block.provenance,\n"
           "                                descriptor.body_provenance))\n"
           "            return reject(\"selected-block\");\n"
           "        // Even one iteration must use the proven transaction path here.\n"
           "        // A scalar staged write exits before the loop tail so the generated\n"
           "        // resume block would otherwise require another central dispatch.\n"
           "        if (!runtime_state_allows_batch())\n"
           "            return reject(\"runtime-state\");\n"
           "        if (has_acceptable_pending_interrupt())\n"
           "            return reject(\"interrupt-pending\");\n"
           "        const ScopedCpuActiveBlockProvenance active_block_provenance(\n"
           "            cpu_, selected_block.virtual_start,\n"
           "            selected_block.physical_origin, selected_block.size);\n"
           "        if (cpu_.pending_guest_cycles != 0u)\n"
           ...
           "            return reject(\"preflush\");\n"
           "        if (has_acceptable_pending_interrupt())\n"
           "            return reject(\"preflush-interrupt-pending\");\n"
           "\n"
           "        const auto block = executable_blocks_.find(descriptor.body_address);\n"
           "        if (block == executable_blocks_.end() ||\n"
           "            block->second.physical_origin != selected_block.physical_origin ||\n"
           "            block->second.size != descriptor.body_size ||\n"
           "            block->second.timing_class !=\n"
           "                katana::runtime::ExecutableBlockTimingClass::RequiresCycleFlush ||\n"
           "            block->second.maximum_guest_cycles !=\n"
           "                descriptor.round_guest_cycles ||\n"
           "            !state_.code_tracker->dispatchable(block->second.identity) ||\n"
           "            !selected_block.generation_guard_reusable ||\n"
           "            selected_block.generation_guard.kind !=\n"
           "                katana::runtime::BlockDispatchGenerationGuardKind::StaticAot ||\n"
           "            !state_.runtime_blocks->static_dispatch_generation_guard_current(\n"
           "                selected_block.generation_guard))\n"
           "            return reject(\"registered-block\");\n"
           "        try {\n"
           "            if (state_.address_space->instruction_translation_path(\n"
           "                    descriptor.body_address, cpu_.privileged_mode()) !=\n"
           "                    katana::runtime::InstructionTranslationPath::Direct)\n"
           "                return reject(\"instruction-path\");\n"
           "            const auto inspected = state_.address_space->inspect_translation(\n"
           "                descriptor.body_address,\n"
           "                katana::runtime::TranslationAccess::Instruction,\n"
           "                cpu_.privileged_mode());\n"
           "            if (katana::runtime::canonical_physical_address(\n"
           "                    inspected.physical_address) !=\n"
           "                    block->second.physical_origin ||\n"
           "                !inspected.no_mmu_fastpath || inspected.utlb_slot != 0xFFu ||\n"
           "                inspected.mmu_generation !=\n"
           "                    active_block_variant_->mmu_generation ||\n"
           "                !state_.address_space->prove_instruction_mapping(\n"
           "                    descriptor.body_address, block->second.physical_origin,\n"
           "                    block->second.size, cpu_.privileged_mode()))\n"
           "                return reject(\"instruction-mapping\");\n"
           "            const auto variant = katana::runtime::block_variant_key(\n"
           "                state_.address_space->guard_for(\n"
           "                    descriptor.body_address, cpu_.read_fpscr(),\n"
           "                    cpu_.privileged_mode()),\n"
           "                active_block_variant_->runtime_generation);\n"
           "            if (variant != *active_block_variant_)\n"
           "                return reject(\"instruction-variant\");\n"
           "        } catch (...) {\n"
           "            return reject(\"instruction-exception\");\n"
           "        }\n"
           "\n"
           "        const auto initial_counter = cpu_.r[descriptor.counter_register];\n"
           "        const auto remaining_iterations = initial_counter == 0u\n"
           "            ? 0x1'0000'0000ull\n"
           "            : static_cast<std::uint64_t>(initial_counter);\n"
           "        auto admitted = std::min(\n"
           "            remaining_iterations, quantum / descriptor.round_guest_cycles);\n"
           "        const auto scheduler_cycle = state_.scheduler->current_cycle();\n"
           "        if (const auto event = state_.scheduler->next_event_cycle()) {\n"
           "            if (*event <= scheduler_cycle)\n"
           "                return reject(\"scheduler-event-due\");\n"
           "            admitted = std::min(\n"
           "                admitted,\n"
           "                (*event - scheduler_cycle - 1u) /\n"
           "                    descriptor.round_guest_cycles);\n"
           "        }\n"
           "        if (const auto budget = state_.scheduler->remaining_guest_cycles()) {\n"
           "            if (*budget == 0u)\n"
           "                return reject(\"scheduler-budget-empty\");\n"
           "            admitted = std::min(\n"
           "                admitted,\n"
           "                (*budget - 1u) / descriptor.round_guest_cycles);\n"
           "        }\n"
           "        if (admitted == 0u ||\n"
           "            admitted > std::numeric_limits<std::size_t>::max())\n"
           "            return reject(\"empty-batch\");\n"
           "        if (admitted > std::numeric_limits<std::size_t>::max() /\n"
           "                           descriptor.store_width)\n"
           "            return reject(\"copy-size-overflow\");\n"
           "        const auto copy_size = static_cast<std::size_t>(admitted) *\n"
           "            descriptor.store_width;\n"
           "        const auto source_address = cpu_.r[descriptor.source_register];\n"
           "        const auto target_address = cpu_.r[descriptor.cursor_register];\n"
           "        if ((source_address & (descriptor.store_width - 1u)) != 0u ||\n"
           "            (target_address & (descriptor.store_width - 1u)) != 0u)\n"
           "            return reject(\"alignment\");\n"
           "        if (copy_size > 0x1'0000'0000ull - source_address ||\n"
           "            copy_size > 0x1'0000'0000ull - target_address)\n"
           "            return reject(\"address-overflow\");\n"
           "        const auto source_read = prove_main_ram_translation(\n"
           "            cpu_, state_, source_address, copy_size,\n"
           "            katana::runtime::TranslationAccess::Read);\n"
           "        const auto target_write = prove_contiguous_translation(\n"
           "            cpu_, state_, target_address, copy_size,\n"
           "            katana::runtime::TranslationAccess::Write);\n"
           "        if (!source_read || !target_write ||\n"
           "            !source_read->no_mmu_fastpath ||\n"
           "            !target_write->no_mmu_fastpath ||\n"
           "            source_read->utlb_slot != 0xFFu ||\n"
           "            target_write->utlb_slot != 0xFFu ||\n"
           "            source_read->mmu_generation !=\n"
           "                active_block_variant_->mmu_generation ||\n"
           "            target_write->mmu_generation !=\n"
           "                active_block_variant_->mmu_generation)\n"
           "            return reject(\"ram-translation\");\n"
           "        const auto source_backing = dreamcast_main_ram_backing_offset(\n"
           "            source_read->physical_address, copy_size);\n"
           "        const auto target_backing = dreamcast_main_ram_backing_offset(\n"
           "            target_write->physical_address, copy_size);\n"
           "        const bool target_is_main_ram = target_backing &&\n"
           "            cpu_.memory.maps_device(\n"
           "                target_write->physical_address, copy_size,\n"
           "                state_.main_ram.get(), false) &&\n"
           "            cpu_.memory.is_writable_linear_range(\n"
           "                target_write->physical_address, copy_size, false);\n"
           "        const auto translated_target_end =\n"
           "            static_cast<std::uint64_t>(target_write->physical_address) +\n"
           "                copy_size;\n"
           "        const auto target_within_vram_alias = [&](const auto& bases) {\n"
           "            return std::any_of(\n"
           "                bases.begin(), bases.end(), [&](const std::uint32_t base) {\n"
           "                    const auto begin = static_cast<std::uint64_t>(base);\n"
           "                    return target_write->physical_address >= begin &&\n"
           "                        translated_target_end <=\n"
           "                            begin + katana::runtime::dreamcast_vram_size;\n"
           "                });\n"
           "        };\n"
           "        const bool target_is_vram = descriptor.store_width == 4u &&\n"
           "            (target_within_vram_alias(\n"
           "                 katana::runtime::dreamcast_vram_64bit_physical_bases) ||\n"
           "             target_within_vram_alias(\n"
           "                 katana::runtime::dreamcast_vram_32bit_physical_bases));\n"
           "        if (!source_backing ||\n"
           "            (!target_is_main_ram && !target_is_vram))\n"
           "            return reject(\"memory-region\");\n"
           "        const auto target_physical = target_is_main_ram\n"
           "            ? static_cast<std::uint32_t>(\n"
           "                  katana::runtime::dreamcast_main_ram_area_bases.front() +\n"
           ...
           "            : target_write->physical_address;\n"
           "        const auto target_physical_end =\n"
           "            static_cast<std::uint64_t>(target_physical) + copy_size;\n"
           "        const auto loop_physical_begin =\n"
           "            static_cast<std::uint64_t>(block->second.physical_origin);\n"
           "        const auto loop_physical_end = loop_physical_begin +\n"
           "            descriptor.body_size;\n"
           "        if (target_is_main_ram &&\n"
           "            static_cast<std::uint64_t>(target_physical) <\n"
           "                loop_physical_end &&\n"
           "            loop_physical_begin < target_physical_end)\n"
           "            return reject(\"active-loop-code-overlap\");\n"
           "        // A complete-project AOT map intentionally marks many main-RAM\n"
           "        // pages executable.  Rejecting every such destination turns a\n"
           "        // proven copy loop into one central dispatch per word.  The\n"
           "        // stable prevalidated-write observer below atomically invalidates\n"
           "        // every changed target block before the next dispatch.  Only the\n"
           "        // loop body executing this synthetic batch must remain immutable;\n"
           "        // its exact physical extent is rejected above.\n"
           "\n"
           "        if (admitted > std::numeric_limits<std::uint64_t>::max() /\n"
           "                           descriptor.round_guest_cycles ||\n"
           "            admitted > std::numeric_limits<std::uint64_t>::max() /\n"
           "                           descriptor.round_instruction_count ||\n"
           "            admitted > std::numeric_limits<std::uint64_t>::max() / 2u)\n"
           "            return reject(\"accounting-overflow\");\n"
           "        const auto batch_guest_cycles =\n"
           ...
           "                    cpu_.retired_guest_instructions)\n"
           "            return reject(\"cpu-counter-overflow\");\n"
           "        const auto main_ram_bytes = state_.main_ram->bytes();\n"
           "        if (*source_backing > main_ram_bytes.size() ||\n"
           "            copy_size > main_ram_bytes.size() - *source_backing)\n"
           "            return reject(\"ram-backing\");\n"
           "        const auto source_bytes = main_ram_bytes.subspan(\n"
           "            *source_backing, copy_size);\n"
           "        std::vector<std::uint8_t> forward_overlap_bytes;\n"
           "        std::span<const std::uint8_t> copied_bytes = source_bytes;\n"
           "        if (target_is_main_ram &&\n"
           "            *source_backing < *target_backing &&\n"
           "            *target_backing < *source_backing + copy_size) {\n"
           "            const auto seed_size = static_cast<std::size_t>(\n"
           "                *target_backing - *source_backing);\n"
           "            try {\n"
           "                forward_overlap_bytes.resize(copy_size);\n"
           "            } catch (...) {\n"
           "                return reject(\"overlap-staging\");\n"
           "            }\n"
           "            std::copy_n(source_bytes.begin(), seed_size,\n"
           "                        forward_overlap_bytes.begin());\n"
           "            for (std::size_t filled = seed_size; filled < copy_size;) {\n"
           "                const auto chunk = std::min(filled, copy_size - filled);\n"
           "                std::copy_n(forward_overlap_bytes.begin(), chunk,\n"
           "                            forward_overlap_bytes.begin() +\n"
           "                                static_cast<std::ptrdiff_t>(filled));\n"
           "                filled += chunk;\n"
           "            }\n"
           "            copied_bytes = std::span<const std::uint8_t>(\n"
           "                forward_overlap_bytes);\n"
           "        }\n"
           "        std::uint32_t last_value = 0u;\n"
           "        const auto last_value_offset =\n"
           "            copy_size - descriptor.store_width;\n"
           "        for (std::size_t byte = 0u; byte < descriptor.store_width; ++byte)\n"
           "            last_value |= static_cast<std::uint32_t>(\n"
           "                copied_bytes[last_value_offset + byte]) << (byte * 8u);\n"
           "        if (descriptor.store_width == 1u && (last_value & 0x80u) != 0u)\n"
           "            last_value |= 0xFFFFFF00u;\n"
           "        else if (descriptor.store_width == 2u &&\n"
           "                 (last_value & 0x8000u) != 0u)\n"
           "            last_value |= 0xFFFF0000u;\n"
           "        std::optional<katana::runtime::Memory::PreparedLinearTransactionBatch>\n"
           "            prepared_copy;\n"
           "        std::optional<katana::runtime::Memory::PreparedLinearU32Copy>\n"
           "            prepared_vram_copy;\n"
           "        if (target_is_main_ram) {\n"
           "            const katana::runtime::LinearMemoryTransactionWrite write{\n"
           "                target_write->physical_address, copied_bytes};\n"
           "            prepared_copy = cpu_.memory.prepare_linear_transaction_batch(\n"
           ...
           "                return reject(\"memory-prepare\");\n"
           "        } else {\n"
           "            auto candidate_vram_copy =\n"
           "                cpu_.memory.prepare_prevalidated_linear_u32_copy(\n"
           "                    target_write->raw_physical_address, copied_bytes,\n"
           "                    katana::runtime::CodeWriteSource::Cpu, admitted,\n"
           "                    admitted * 2u);\n"
           "            if (!candidate_vram_copy)\n"
           "                return reject(\"vram-prepare\");\n"
           "            prepared_vram_copy.emplace(\n"
           "                std::move(*candidate_vram_copy));\n"
           "        }\n"
           "        accept_batch_guest_cycles_before_commit(\n"
           "            batch_guest_cycles, AtomicBatchCommitKind::MemoryFill);\n"
           "        if (target_is_main_ram) {\n"
           "            if (!cpu_.memory.account_prevalidated_unobserved_accesses(\n"
           "                    admitted, admitted * 2u))\n"
           "                std::terminate();\n"
           "            cpu_.memory.commit_prepared_linear_transaction_batch(\n"
           "                std::move(*prepared_copy));\n"
           "        } else {\n"
           "            cpu_.memory.commit_prepared_linear_u32_copy(\n"
           "                std::move(*prepared_vram_copy));\n"
           "        }\n"
           "\n"
           "        const auto final_source = static_cast<std::uint32_t>(\n"
           "            source_address + copy_size);\n"
           "        const auto final_target = static_cast<std::uint32_t>(\n"
           "            target_address + copy_size);\n"
           "        const auto final_counter = static_cast<std::uint32_t>(\n"
           "            initial_counter - admitted);\n"
           "        const bool complete = admitted == remaining_iterations;\n"
           "        cpu_.r[descriptor.source_register] = final_source;\n"
           "        cpu_.r[descriptor.value_register] = last_value;\n"
           "        cpu_.r[descriptor.counter_register] = final_counter;\n"
           "        cpu_.r[descriptor.cursor_register] = final_target;\n"
           "        cpu_.t = final_counter == 0u;\n"
           "        cpu_.pc = complete ? descriptor.exit_address\n"
           "                           : descriptor.body_address;\n"
           "        cpu_.active_instruction_pc = descriptor.branch_instruction_address;\n"
           "        cpu_.active_instruction_physical_pc =\n"
           "            block->second.physical_origin + 6u;\n"
           "        cpu_.attempted_guest_instructions += batch_instructions;\n"
           "        cpu_.retired_guest_instructions += batch_instructions;\n"
           "        trace_direct_memory_copy_batch_admission(admitted);\n"
           "        return true;\n"
           "    }\n"
           ...
           entry_namespace +
           "::MemoryLoopBatchKind::DirectMemoryCopy)\n"
           "            return try_direct_memory_copy_loop_batch(\n"
           "                selected_block, descriptor);\n"
           "        constexpr std::uint64_t quantum = 131'072u;\n"
           ...
           "    std::function<void()> product_progress_evidence_callback_;\n"
           "    std::function<void()> game_entry_callback_;\n"
           "    katana::runtime::SystemReplayLog* replay_log_ = nullptr;\n"
           "    katana::runtime::HostWorkloadLimiter* workload_limiter_ = nullptr;\n"
           ...
           "    static constexpr std::uint32_t host_poll_call_quantum = 65'536u;\n"
           "    std::uint64_t next_lifecycle_poll_guest_cycle_ = 0u;\n"
           "    std::uint64_t last_lifecycle_poll_guest_cycle_ = 0u;\n"
           "    std::uint32_t host_poll_calls_since_callback_ = 0u;\n"
           ...
           "    };\n"
           "    struct RuntimeAotChainContract {\n"
           "        std::uint32_t size = 0u;\n"
           "        katana::runtime::ExecutableBlockTimingClass timing_class =\n"
           "            katana::runtime::ExecutableBlockTimingClass::NeverChain;\n"
           "        std::uint64_t maximum_guest_cycles = 0u;\n"
           ...
           "    };\n"
           "    struct ActiveRuntimeAotChainContext {\n"
           "        katana::runtime::RuntimeBlockHandle handle;\n"
           "        std::uint32_t virtual_start = 0u;\n"
           "        std::uint32_t physical_start = 0u;\n"
           "        std::uint32_t size = 0u;\n"
           "        katana::runtime::BlockVariantKey variant;\n"
           "    };\n"
           "    struct RuntimeAotChainResolution {\n"
           "        katana::runtime::RuntimeBlockHandle validation_handle;\n"
           "        const RuntimeAotChainContract* contract = nullptr;\n"
           "        std::uint32_t validation_virtual = 0u;\n"
           "        std::uint32_t validation_physical = 0u;\n"
           "        std::uint32_t target_size = 0u;\n"
           "        katana::runtime::ExecutableChainRejectionReason rejection =\n"
           "            katana::runtime::ExecutableChainRejectionReason::\n"
           "                TargetNotRegistered;\n"
           "        bool same_mapping = false;\n"
           "        bool valid = false;\n"
           "    };\n"
           "    void prepare_runtime_aot_chain_context(\n"
           "            const std::uint32_t virtual_start,\n"
           "            const std::uint32_t physical_start,\n"
           "            const std::uint32_t size,\n"
           "            const katana::runtime::BlockVariantKey& variant,\n"
           "            const bool runtime_registered) noexcept {\n"
           "        active_runtime_aot_chain_context_.reset();\n"
           "        if (!runtime_registered || runtime_dispatch_table_ == nullptr ||\n"
           "            runtime_dispatch_materializer_ == nullptr ||\n"
           "            state_.runtime_blocks.get() != runtime_dispatch_table_)\n"
           "            return;\n"
           "        const auto handle = runtime_dispatch_table_->lookup(\n"
           "            virtual_start, variant);\n"
           "        if (!handle) return;\n"
           "        const auto resolved = runtime_dispatch_table_->resolve(*handle);\n"
           "        if (!resolved || !resolved->get().runtime_registered ||\n"
           "            !resolved->get().aot_template ||\n"
           "            resolved->get().aot_template->validation_mode !=\n"
           "                katana::runtime::NativeAotTemplateValidationMode::\n"
           "                    SourceModule ||\n"
           "            resolved->get().virtual_start != virtual_start ||\n"
           "            resolved->get().physical_origin !=\n"
           "                katana::runtime::canonical_physical_address(physical_start) ||\n"
           "            resolved->get().size != size ||\n"
           "            resolved->get().variant != variant ||\n"
           "            resolved->get().function == nullptr)\n"
           "            return;\n"
           "        active_runtime_aot_chain_context_ = ActiveRuntimeAotChainContext{\n"
           "            *handle, virtual_start,\n"
           "            katana::runtime::canonical_physical_address(physical_start),\n"
           "            size, variant};\n"
           "    }\n"
           "    RuntimeAotChainResolution resolve_runtime_aot_chain_entry(\n"
           "            const std::uint32_t address, const std::uint32_t physical,\n"
           "            const katana::runtime::BlockVariantKey& variant) const noexcept {\n"
           "        using Rejection =\n"
           "            katana::runtime::ExecutableChainRejectionReason;\n"
           "        const auto reject = [&](std::uint64_t& counter,\n"
           "                                const Rejection reason) noexcept {\n"
           "            ++counter;\n"
           "            RuntimeAotChainResolution result;\n"
           "            result.rejection = reason;\n"
           "            return result;\n"
           "        };\n"
           "        if (const auto handle = runtime_dispatch_table_->lookup(\n"
           "                address, variant)) {\n"
           "            const auto resolved = runtime_dispatch_table_->resolve(*handle);\n"
           "            if (!resolved || !resolved->get().runtime_registered ||\n"
           "                !resolved->get().aot_template ||\n"
           "                resolved->get().virtual_start != address ||\n"
           "                resolved->get().physical_origin != physical ||\n"
           "                resolved->get().function == nullptr)\n"
           "                return reject(runtime_aot_chain_target_rejections_,\n"
           "                              Rejection::TargetNotRegistered);\n"
           "            const auto& block = resolved->get();\n"
           "            const auto& mapping = block.aot_template->mapping;\n"
           "            if (block.virtual_start < mapping.runtime_start)\n"
           "                return reject(runtime_aot_chain_identity_rejections_,\n"
           "                              Rejection::TargetNotNativeEntrySafe);\n"
           "            const auto offset = static_cast<std::uint64_t>(\n"
           "                block.virtual_start - mapping.runtime_start);\n"
           "            if (offset >= mapping.extent ||\n"
           "                block.size > mapping.extent - offset ||\n"
           "                offset > std::numeric_limits<std::uint32_t>::max() -\n"
           "                    mapping.source_start ||\n"
           "                block.end_kind ==\n"
           "                    katana::runtime::BlockEndKind::ExceptionReturn ||\n"
           "                block.end_kind == katana::runtime::BlockEndKind::Sleep ||\n"
           "                block.end_kind == katana::runtime::BlockEndKind::Exception ||\n"
           "                block.fastpath.kind !=\n"
           "                    katana::runtime::RuntimeBlockFastpathKind::None)\n"
           "                return reject(runtime_aot_chain_identity_rejections_,\n"
           "                              Rejection::TargetNotNativeEntrySafe);\n"
           "            const auto source = static_cast<std::uint32_t>(\n"
           "                mapping.source_start + offset);\n"
           "            const auto contract = runtime_aot_chain_contracts_.find(source);\n"
           "            if (contract == runtime_aot_chain_contracts_.end())\n"
           "                return reject(runtime_aot_chain_contract_rejections_,\n"
           "                              Rejection::TargetNotRegistered);\n"
           "            return {*handle, &contract->second, address, physical, block.size,\n"
           "                    Rejection::None, false, true};\n"
           "        }\n"
           "        if (!active_runtime_aot_chain_context_)\n"
           "            return reject(runtime_aot_chain_target_rejections_,\n"
           "                          Rejection::TargetNotRegistered);\n"
           "        const auto& active = *active_runtime_aot_chain_context_;\n"
           "        const auto resolved = runtime_dispatch_table_->resolve(active.handle);\n"
           "        if (!resolved || !resolved->get().runtime_registered ||\n"
           "            !resolved->get().aot_template ||\n"
           "            resolved->get().aot_template->validation_mode !=\n"
           "                katana::runtime::NativeAotTemplateValidationMode::\n"
           "                    SourceModule ||\n"
           "            resolved->get().virtual_start != active.virtual_start ||\n"
           "            resolved->get().physical_origin != active.physical_start ||\n"
           "            resolved->get().size != active.size ||\n"
           "            resolved->get().variant != active.variant ||\n"
           "            active.variant != variant || resolved->get().function == nullptr)\n"
           "            return reject(runtime_aot_chain_target_rejections_,\n"
           "                          Rejection::TargetNotRegistered);\n"
           "        const auto& mapping = resolved->get().aot_template->mapping;\n"
           "        if (active.virtual_start < mapping.runtime_start ||\n"
           "            address < mapping.runtime_start)\n"
           "            return reject(runtime_aot_chain_identity_rejections_,\n"
           "                          Rejection::TargetNotNativeEntrySafe);\n"
           "        const auto active_offset = static_cast<std::uint64_t>(\n"
           "            active.virtual_start - mapping.runtime_start);\n"
           "        const auto target_offset = static_cast<std::uint64_t>(\n"
           "            address - mapping.runtime_start);\n"
           "        if (active_offset >= mapping.extent ||\n"
           "            active.size > mapping.extent - active_offset ||\n"
           "            target_offset >= mapping.extent ||\n"
           "            target_offset >\n"
           "                std::numeric_limits<std::uint32_t>::max() -\n"
           "                    mapping.source_start ||\n"
           "            active_offset > active.physical_start)\n"
           "            return reject(runtime_aot_chain_identity_rejections_,\n"
           "                          Rejection::TargetNotNativeEntrySafe);\n"
           "        const auto validation_physical =\n"
           "            active.physical_start -\n"
           "            static_cast<std::uint32_t>(active_offset);\n"
           "        const auto target_physical = static_cast<std::uint64_t>(\n"
           "            validation_physical) + target_offset;\n"
           "        if (target_physical > std::numeric_limits<std::uint32_t>::max() ||\n"
           "            static_cast<std::uint32_t>(target_physical) != physical)\n"
           "            return reject(runtime_aot_chain_identity_rejections_,\n"
           "                          Rejection::AddressTranslation);\n"
           "        const auto source = static_cast<std::uint32_t>(\n"
           "            mapping.source_start + target_offset);\n"
           "        const auto contract = runtime_aot_chain_contracts_.find(source);\n"
           "        if (contract == runtime_aot_chain_contracts_.end())\n"
           "            return reject(runtime_aot_chain_contract_rejections_,\n"
           "                          Rejection::TargetNotRegistered);\n"
           "        if (contract->second.size > mapping.extent - target_offset)\n"
           "            return reject(runtime_aot_chain_identity_rejections_,\n"
           "                          Rejection::TargetNotNativeEntrySafe);\n"
           "        return {active.handle, &contract->second, active.virtual_start,\n"
           "                active.physical_start, contract->second.size,\n"
           "                Rejection::None, true, true};\n"
           "    }\n"
           ...
           "executable_blocks_;\n"
           "    std::unordered_map<std::uint32_t, RuntimeAotChainContract>\n"
           "        runtime_aot_chain_contracts_;\n"
           "    std::vector<std::unique_ptr<ExecutableChainPage>> executable_chain_pages_{\n"
           "        executable_chain_page_count};\n"
           "    katana::runtime::RuntimeBlockTable* runtime_dispatch_table_ = nullptr;\n"
           "    katana::runtime::DemandBlockMaterializer*\n"
           "        runtime_dispatch_materializer_ = nullptr;\n"
           "    std::uint64_t runtime_aot_chainable_contracts_ = 0u;\n"
           "    std::uint64_t runtime_aot_chain_contract_collisions_ = 0u;\n"
           "    mutable std::uint64_t runtime_aot_chain_attempts_ = 0u;\n"
           "    mutable std::uint64_t runtime_aot_chain_admissions_ = 0u;\n"
           "    mutable std::uint64_t runtime_aot_chain_mapping_admissions_ = 0u;\n"
           "    mutable std::uint64_t runtime_aot_chain_context_rejections_ = 0u;\n"
           "    mutable std::uint64_t runtime_aot_chain_target_rejections_ = 0u;\n"
           "    mutable std::uint64_t runtime_aot_chain_contract_rejections_ = 0u;\n"
           "    mutable std::uint64_t runtime_aot_chain_identity_rejections_ = 0u;\n"
           "    mutable std::uint64_t runtime_aot_chain_validation_rejections_ = 0u;\n"
           "    std::optional<katana::runtime::BlockVariantKey> active_block_variant_;\n"
           "    std::optional<ActiveRuntimeAotChainContext>\n"
           "        active_runtime_aot_chain_context_;\n"
           ...
           "    bool runtime_aot_chaining_enabled_ = false;\n"
           ...
           "    bool direct_memory_copy_batching_enabled_ = false;\n"
           ...
           "            if (tick.frame_count > std::numeric_limits<std::size_t>::max() / 2u)\n"
           "                throw std::invalid_argument(\"AICA-Audioausgabe besitzt eine ungueltige Geometrie.\");\n"
           "            audio_samples.resize(tick.frame_count * 2u);\n"
           "            state.aica_registers->render_audio_into(\n"
           "                audio_samples, 44'100u);\n"
           "            audio.submit(audio_samples, 44'100u);\n"
           "            record_runtime_probe_event(\n"
           "                replay, *state.scheduler,\n"
           ...
           "        if (workload_limiter)\n"
           "            std::cerr << \"KATANA_HOST_WORKLOAD_LIMITER \"\n"
           "                      << workload_limiter->serialize_status_json()\n"
           "                      << '\\n';\n"
           ...
           "                  << \" host_video_backend=\"\n"
           "                  << katana::runtime::native_video_backend_name(\n"
           "                         terminal_telemetry.host_video_backend)\n"
           "                  << \" host_video_hardware_accelerated=\"\n"
           "                  << terminal_telemetry.host_video_hardware_accelerated\n"
           "                  << \" host_video_fallback_count=\"\n"
           "                  << terminal_telemetry.host_video_fallbacks\n"
           "                  << \" host_video_submitted_frames=\"\n"
           "                  << terminal_telemetry.host_video_presentation."
           "submitted_frames\n"
           "                  << \" host_video_presented_frames=\"\n"
           "                  << terminal_telemetry.host_video_presentation."
           "presented_frames\n"
           "                  << \" host_video_occluded_frames=\"\n"
           "                  << terminal_telemetry.host_video_presentation."
           "occluded_frames\n"
           "                  << \" host_video_not_presentable_frames=\"\n"
           "                  << terminal_telemetry.host_video_presentation."
           "not_presentable_frames\n"
           "                  << \" host_video_backend_failures=\"\n"
           "                  << terminal_telemetry.host_video_presentation."
           "backend_failures\n"
           "                  << \" host_video_last_outcome=\"\n"
           "                  << katana::runtime::"
           "native_video_presentation_outcome_name(\n"
           "                         terminal_telemetry.host_video_presentation."
           "last_outcome)\n"
           "                  << \" host_video_currently_occluded=\"\n"
           "                  << terminal_telemetry.host_video_presentation."
           "currently_occluded\n"
           "                  << \" first_problem=\" << first_problem << '\\n';\n"
           "    };\n"
           "    try {\n"
           "        katana::ProgressReporter runtime_progress(\n"
           "            [](const katana::ProgressEvent& event) {\n"
           "                std::osyncstream output(std::cerr);\n"
           "                output << \"KATANA_RUNTIME_PROGRESS operation=\"\n"
           ...
           "                    output << \" parent=\" << *event.parent_scope_id;\n"
           "                if (event.total)\n"
           "                    output << \" total=\" << *event.total;\n"
           "                output << '\\n';\n"
           "            },\n"
           ...
           "source, destination, runtime_progress);\n"
           "                auto packed = katana::runtime::PackedDiscSource::open(\n"
           "                    destination, runtime_progress);\n"
           ...
           "            }\n"
           "        }\n"
           "        auto runtime_startup = runtime_progress.begin(\n"
           "            katana::ProgressOperation::RuntimeStartup,\n"
           "            katana::ProgressUnit::Steps, 3u, \"runtime-startup\");\n"
           "        const auto runtime_startup_progress =\n"
           "            runtime_startup.child_reporter();\n"
           ...
           "        runtime_startup.complete();\n"
           "        std::cerr << \"KATANA_RUNTIME_PARALLEL jobs=\"\n"
           ...
           "        }\n"
           "        if (video != nullptr) {\n"
           "            terminal_telemetry.host_video_backend = video->backend();\n"
           "            terminal_telemetry.host_video_hardware_accelerated =\n"
           "                video->hardware_accelerated();\n"
           "            terminal_telemetry.host_video_fallbacks =\n"
           "                video->backend_fallbacks();\n"
           "            terminal_telemetry.host_video_presentation =\n"
           "                video->presentation_telemetry();\n"
           "        }\n"
           "        katana::runtime::GuestFrameEvidenceTracker guest_frame_evidence;\n"
           "        katana::runtime::GuestFrameVisibilityClassifier\n"
           "            visible_game_frame_classifier;\n"
           "        katana::runtime::GuestFrameVisibilityObservation\n"
           "            last_game_frame_visibility;\n"
           "        struct PendingVisibleGameFrame {\n"
           "            bool available = false;\n"
           "            std::uint64_t digest = 0u;\n"
           "            std::uint64_t presented_cycle = 0u;\n"
           "            katana::runtime::GuestFrameVisibilityObservation observation;\n"
           ...
           "            std::uint64_t pvr_ta_frames_current_generation = 0u;\n"
           "            std::uint64_t pvr_ta_frames_lifetime = 0u;\n"
           "            std::uint64_t pvr_ta_reset_generation = 0u;\n"
           "            std::uint64_t pvr_ta_resets_total = 0u;\n"
           ...
           "        std::optional<katana::runtime::NativeVideoBackend>\n"
           "            reported_video_backend;\n"
           "        std::optional<katana::runtime::NativeVideoPresentationOutcome>\n"
           "            reported_video_outcome;\n"
           "        std::uint64_t reported_video_fallbacks = 0u;\n"
           ...
           "        std::uint64_t last_presented_frame_digest = 0u;\n"
           "        std::uint64_t last_presented_changed_pixels = 0u;\n"
           "        bool last_presented_evidence_collected = false;\n"
           ...
           "        std::uint64_t first_visible_game_frame_presented_cycle = 0u;\n"
           "        std::uint64_t first_ip_bin_visible_frame_cycle = 0u;\n"
           "        std::uint64_t first_visible_game_frame_digest = 0u;\n"
           "        std::uint64_t first_visible_game_frame_changed_pixels = 0u;\n"
           "        std::uint64_t first_visible_game_frame_proof_changed_pixels = 0u;\n"
           "        std::uint64_t first_visible_game_frame_required_changed_pixels = 0u;\n"
           "        std::uint32_t first_visible_game_frame_changed_tiles = 0u;\n"
           "        std::uint32_t first_visible_game_frame_changed_interior_tiles = 0u;\n"
           "        std::uint32_t first_visible_game_frame_color_classes = 0u;\n"
           "        std::uint32_t first_visible_game_frame_luminance_classes = 0u;\n"
           ...
           "        const auto configured_host_cpu_percent = [&] {\n"
           "            if (!lifecycle_test.empty()) return std::uint32_t{100u};\n"
           "            const auto* text = std::getenv(\"KATANA_HOST_CPU_PERCENT\");\n"
           "            // Native ports run unthrottled by default. Hosts that need\n"
           "            // a thermal or background-work cap can opt in via the env.\n"
           ...
           "                value == 0u || value > 100u)\n"
           "                throw std::runtime_error(\n"
           "                    \"KATANA_HOST_CPU_PERCENT ist ungueltig.\");\n"
           "            return static_cast<std::uint32_t>(value);\n"
           "        }();\n"
           "        const auto configured_process_cpu_percent = [&] {\n"
           "            if (!lifecycle_test.empty()) return std::uint32_t{100u};\n"
           "            const auto* text =\n"
           "                std::getenv(\"KATANA_HOST_PROCESS_CPU_PERCENT\");\n"
           ...
           "                value == 0u || value > 100u)\n"
           "                throw std::runtime_error(\n"
           "                    \"KATANA_HOST_PROCESS_CPU_PERCENT ist ungueltig.\");\n"
           "            return static_cast<std::uint32_t>(value);\n"
           "        }();\n"
           "        workload_limiter.emplace(\n"
           "            katana::runtime::HostWorkloadLimiterConfig{\n"
           "                configured_host_cpu_percent,\n"
           "                100'000'000u,\n"
           "                2'000'000u,\n"
           ...
           "            current.pvr_ta_frames_current_generation =\n"
           "                ta.frames_current_generation;\n"
           "            current.pvr_ta_frames_lifetime = ta.frames_lifetime;\n"
           "            current.pvr_ta_reset_generation = ta.reset_generation;\n"
           "            current.pvr_ta_resets_total = ta.resets_total;\n"
           ...
           "            delta.pvr_ta_frames_current_generation =\n"
           "                current.pvr_ta_frames_current_generation;\n"
           "            delta.pvr_ta_frames_lifetime = saturating_counter_delta(\n"
           "                current.pvr_ta_frames_lifetime,\n"
           "                product_entry_evidence_baseline.\n"
           "                    pvr_ta_frames_lifetime);\n"
           "            delta.pvr_ta_reset_generation =\n"
           "                current.pvr_ta_reset_generation;\n"
           "            delta.pvr_ta_resets_total = saturating_counter_delta(\n"
           "                current.pvr_ta_resets_total,\n"
           "                product_entry_evidence_baseline.pvr_ta_resets_total);\n"
           ...
           "            };\n"
           "        const auto promote_pending_visible_game_frame = [&] {\n"
           "            if (!pending_visible_game_frame.available ||\n"
           "                first_visible_game_frame)\n"
           "                return;\n"
           "            const auto& promoted =\n"
           "                pending_visible_game_frame.observation;\n"
           "            first_visible_game_frame = true;\n"
           "            state.pvr_renderer->complete_guest_frame_evidence();\n"
           "            first_visible_game_frame_cycle =\n"
           "                state.scheduler->current_cycle();\n"
           "            first_visible_game_frame_presented_cycle =\n"
           "                pending_visible_game_frame.presented_cycle;\n"
           "            first_visible_game_frame_digest =\n"
           "                pending_visible_game_frame.digest;\n"
           "            first_visible_game_frame_changed_pixels =\n"
           "                promoted.changed_pixels;\n"
           "            first_visible_game_frame_proof_changed_pixels =\n"
           "                promoted.proof_changed_pixels;\n"
           "            first_visible_game_frame_required_changed_pixels =\n"
           "                promoted.required_changed_pixels;\n"
           "            first_visible_game_frame_changed_tiles =\n"
           "                promoted.changed_tiles;\n"
           "            first_visible_game_frame_changed_interior_tiles =\n"
           "                promoted.changed_interior_tiles;\n"
           "            first_visible_game_frame_color_classes =\n"
           "                promoted.changed_color_classes;\n"
           "            first_visible_game_frame_luminance_classes =\n"
           "                promoted.changed_luminance_classes;\n"
           "            pending_visible_game_frame.available = false;\n"
           ...
           "                      << \" changed_pixels=\"\n"
           "                      << first_visible_game_frame_changed_pixels\n"
           "                      << \" proof_changed_pixels=\"\n"
           "                      << first_visible_game_frame_proof_changed_pixels\n"
           "                      << \" required_changed_pixels=\"\n"
           "                      << first_visible_game_frame_required_changed_pixels\n"
           "                      << \" changed_tiles=\"\n"
           "                      << first_visible_game_frame_changed_tiles\n"
           "                      << \" changed_interior_tiles=\"\n"
           "                      << first_visible_game_frame_changed_interior_tiles\n"
           "                      << \" changed_color_classes=\"\n"
           "                      << first_visible_game_frame_color_classes\n"
           "                      << \" changed_luminance_classes=\"\n"
           "                      << first_visible_game_frame_luminance_classes\n"
           "                      << \" presented_cycle=\"\n"
           "                      << first_visible_game_frame_presented_cycle\n"
           "                      << '\\n';\n"
           ...
           "            auto result = katana::runtime::pump_guest_frame_proof(\n"
           "                *state.pvr_renderer, video.get(),\n"
           "                !first_visible_game_frame);\n"
           "            if (video != nullptr) {\n"
           "                const auto backend = video->backend();\n"
           "                const auto fallbacks = video->backend_fallbacks();\n"
           "                const auto presentation =\n"
           "                    video->presentation_telemetry();\n"
           "                terminal_telemetry.host_video_backend = backend;\n"
           "                terminal_telemetry.host_video_hardware_accelerated =\n"
           "                    video->hardware_accelerated();\n"
           "                terminal_telemetry.host_video_fallbacks = fallbacks;\n"
           "                terminal_telemetry.host_video_presentation =\n"
           "                    presentation;\n"
           "                if (!reported_video_backend ||\n"
           "                    *reported_video_backend != backend ||\n"
           "                    !reported_video_outcome ||\n"
           "                    *reported_video_outcome != "
           "presentation.last_outcome ||\n"
           "                    reported_video_fallbacks != fallbacks) {\n"
           "                    std::cout << \"KATANA_HOST_VIDEO_BACKEND backend=\"\n"
           "                              << katana::runtime::native_video_backend_name(\n"
           "                                     backend)\n"
           "                              << \" hardware_accelerated=\"\n"
           "                              << (video->hardware_accelerated() ? 1 : 0)\n"
           "                              << \" fallback_count=\" << fallbacks\n"
           "                              << \" submitted_frames=\"\n"
           "                              << presentation.submitted_frames\n"
           "                              << \" presented_frames=\"\n"
           "                              << presentation.presented_frames\n"
           "                              << \" occluded_frames=\"\n"
           "                              << presentation.occluded_frames\n"
           "                              << \" not_presentable_frames=\"\n"
           "                              << presentation.not_presentable_frames\n"
           "                              << \" backend_failures=\"\n"
           "                              << presentation.backend_failures\n"
           "                              << \" last_outcome=\"\n"
           "                              << katana::runtime::\n"
           "                                     native_video_presentation_outcome_name(\n"
           "                                         presentation.last_outcome)\n"
           ...
           "                    reported_video_backend = backend;\n"
           "                    reported_video_outcome = presentation.last_outcome;\n"
           "                    reported_video_fallbacks = fallbacks;\n"
           "                }\n"
           "            }\n"
           ...
           "                visible_progress = visible_game_frame_classifier.observe(\n"
           "                    *result.presented_frame,\n"
           ...
           "                product_entry_evidence_ready &&\n"
           "                result.proven_frame_presented)\n"
           "                last_game_frame_visibility = visible_progress;\n"
           "            const bool visibly_structured_proven_frame =\n"
           "                result.proven_frame_presented &&\n"
           "                result.presented_source ==\n"
           "                    katana::runtime::GuestFramePresentedSource::GuestProof &&\n"
           "                result.presented_changed_pixels != 0u &&\n"
           "                katana::runtime::has_relevant_guest_frame_content(\n"
           "                    result.presented_frame_evidence);\n"
           "            const bool visibly_changed_proven_frame =\n"
           "                visibly_structured_proven_frame &&\n"
           ...
           "            }\n"
           "            if (visibly_changed_proven_frame &&\n"
           "                !pending_visible_game_frame.available &&\n"
           "                !first_visible_game_frame) {\n"
           "                pending_visible_game_frame.available = true;\n"
           "                pending_visible_game_frame.digest =\n"
           "                    result.presented_frame_evidence.digest;\n"
           "                pending_visible_game_frame.presented_cycle =\n"
           "                    state.scheduler->current_cycle();\n"
           "                pending_visible_game_frame.observation =\n"
           "                    visible_progress;\n"
           "            }\n"
           "            if (game_code_progressed)\n"
           "                promote_pending_visible_game_frame();\n"
           "            if (visibly_structured_proven_frame &&\n"
           "                       (runtime_observer == nullptr ||\n"
           ...
           "                                 kind, controller});\n"
           "                };\n"
           "                if (lifecycle_test == \"close-before-running\") {\n"
           "                    inject(katana::runtime::HostRuntimeEventKind::Shutdown);\n"
           "                } else if (lifecycle_test == \"running-close\") {\n"
           ...
           "                    *state.pvr_registers, state.vram->bytes()));\n"
           ...
           "                      << last_presented_pixel_count\n"
           "                      << \" host_last_presented_frame_digest=\"\n"
           "                      << last_presented_frame_digest\n"
           "                      << \" host_last_presented_evidence_collected=\"\n"
           "                      << static_cast<unsigned>(\n"
           "                             last_presented_evidence_collected)\n"
           "                      << \" host_last_presented_changed_pixels=\"\n"
           "                      << last_presented_changed_pixels\n"
           ...
           "                      << \" pvr_ta_frames_current_generation=\"\n"
           "                      << post_entry.pvr_ta_frames_current_generation\n"
           "                      << \" post_entry_pvr_ta_frames_lifetime=\"\n"
           "                      << post_entry.pvr_ta_frames_lifetime\n"
           "                      << \" pvr_ta_reset_generation=\"\n"
           "                      << post_entry.pvr_ta_reset_generation\n"
           "                      << \" post_entry_pvr_ta_resets_total=\"\n"
           "                      << post_entry.pvr_ta_resets_total\n"
           ...
           "            services.report_runtime_aot_chain_statistics(std::cerr);\n"
           "        };\n"
           "        " +
           entry_namespace +
           ...
           "            throw;\n"
           "        }\n"
           "        if (host.state() == katana::runtime::HostRuntimeState::Running &&\n"
           ...
           "        std::ostringstream product_terminal_summary;\n"
           "        const auto emit_product_terminal_summary = [&] {\n"
           "            if (terminal_telemetry.terminal_summary_emitted)\n"
           "                throw std::logic_error(\n"
           "                    \"product-terminal-summary-already-emitted\");\n"
           ...
           "            product_terminal_summary << (comparable_product_gate\n"
           "                              ? \"KATANA_PRODUCT_GATE\"\n"
           "                              : \"KATANA_BRINGUP_RUN\")\n"
           ...
           "                             pvr_ta_frames_current_generation\n"
           "                      << \" post_entry_pvr_ta_frames_lifetime=\"\n"
           "                      << post_entry_evidence.pvr_ta_frames_lifetime\n"
           "                      << \" pvr_ta_reset_generation=\"\n"
           "                      << post_entry_evidence.pvr_ta_reset_generation\n"
           "                      << \" post_entry_pvr_ta_resets_total=\"\n"
           "                      << post_entry_evidence.pvr_ta_resets_total\n"
           "                      << \" post_entry_pvr_yuv_macroblocks=\"\n"
           ...
           "                      << \" first_visible_game_frame_luminance_classes=\"\n"
           "                      << first_visible_game_frame_luminance_classes\n"
           "                      << \" visible_frame_baseline_available=\"\n"
           "                      << static_cast<unsigned>(\n"
           "                             last_game_frame_visibility.baseline_available)\n"
           "                      << \" visible_frame_geometry_matches=\"\n"
           "                      << static_cast<unsigned>(\n"
           "                             last_game_frame_visibility.geometry_matches)\n"
           "                      << \" visible_frame_digest_changed=\"\n"
           "                      << static_cast<unsigned>(\n"
           "                             last_game_frame_visibility.digest_changed)\n"
           "                      << \" visible_frame_changed_pixels=\"\n"
           "                      << last_game_frame_visibility.changed_pixels\n"
           "                      << \" visible_frame_required_changed_pixels=\"\n"
           "                      << last_game_frame_visibility.required_changed_pixels\n"
           "                      << \" visible_frame_required_changed_pixels_per_tile=\"\n"
           "                      << last_game_frame_visibility.required_changed_pixels_per_tile\n"
           "                      << \" visible_frame_required_changed_pixels_per_class=\"\n"
           "                      << last_game_frame_visibility.required_changed_pixels_per_class\n"
           "                      << \" visible_frame_changed_tiles=\"\n"
           "                      << last_game_frame_visibility.changed_tiles\n"
           "                      << \" visible_frame_changed_interior_tiles=\"\n"
           "                      << last_game_frame_visibility.changed_interior_tiles\n"
           "                      << \" visible_frame_color_classes=\"\n"
           "                      << last_game_frame_visibility.relevant_color_classes\n"
           "                      << \" visible_frame_luminance_classes=\"\n"
           "                      << last_game_frame_visibility.relevant_luminance_classes\n"
           "                      << \" visible_frame_changed_color_classes=\"\n"
           "                      << last_game_frame_visibility.changed_color_classes\n"
           "                      << \" visible_frame_changed_luminance_classes=\"\n"
           "                      << last_game_frame_visibility.changed_luminance_classes\n"
           "                      << \" first_ip_bin_visible_frame_cycle=\"\n"
           "                      << first_ip_bin_visible_frame_cycle\n"
           "                      << \" first_problem=\" << first_problem << '\\n';\n"
           ...
           "        }\n"
           "        // silent_failures is already a fail-closed product result with\n"
           "        // first_problem=runtime-contract. Throwing here would replace a\n"
           "        // controlled host close with a misleading runtime-exception and\n"
           "        // bypass the buffered terminal summary.\n"
           ...
           "                  << services.fallback_count()\n"
           "                  << \" host_cpu_target_percent=\"\n"
           "                  << workload_limiter->target_cpu_percent()\n"
           "                  << \" host_process_cpu_target_percent=\"\n"
           "                  << workload_limiter->target_process_cpu_percent()\n"
           "                  << \" host_process_cpu_capacity=\"\n"
           "                  << workload_limiter->process_cpu_capacity()\n"
           "                  << \" host_cpu_limit_wait_calls=\"\n"
           "                  << workload_limiter->wait_calls()\n"
           "                  << \" host_cpu_limit_wait_ns=\"\n"
           "                  << workload_limiter->wait_time_ns()\n"
           "                  << \" host_cpu_measured_wall_ns=\"\n"
           "                  << workload_limiter->measured_wall_time_ns()\n"
           "                  << \" host_cpu_measured_thread_cpu_ns=\"\n"
           "                  << workload_limiter->measured_thread_cpu_time_ns()\n"
           "                  << \" host_cpu_measured_percent_milli=\"\n"
           "                  << workload_limiter->measured_cpu_percent_milli()\n"
           "                  << \" host_cpu_measured_process_cpu_ns=\"\n"
           "                  << workload_limiter->measured_process_cpu_time_ns()\n"
           "                  << \" host_cpu_measured_process_percent_milli=\"\n"
           "                  << workload_limiter->"
           "measured_process_cpu_percent_milli()\n"
           "                  << '\\n';\n"
           "        std::cout << \"KATANA_HOST_WORKLOAD_LIMITER \"\n"
           "                  << workload_limiter->serialize_status_json() << '\\n';\n"
...
std::vector<ProjectArtifact> runtime_dispatch_artifacts(
    ...
    struct DispatchBlock {
        ...
        bool guarded_cycle_flush_chainable;
    };
    ...
    std::unordered_map<std::uint32_t, EmittedBlock> emitted_blocks;
    emitted_blocks.reserve(block_count);
    std::vector<std::tuple<std::uint32_t, std::uint32_t, std::uint32_t>>
        divergent_block_owners;
    std::unordered_set<std::uint32_t> dispatch_entry_addresses;
    dispatch_entry_addresses.reserve(block_count);
    std::unordered_set<std::uint32_t> requested_resume_entries(
        native_aot_resume_entries.begin(), native_aot_resume_entries.end());
    std::unordered_set<std::uint32_t> emitted_resume_entries;
    emitted_resume_entries.reserve(requested_resume_entries.size());
    ...
    for (const auto& function : program) {
        for (const auto& block : function.blocks) {
            const auto [existing, inserted] =
                emitted_blocks.emplace(
                    block.start_address,
                    EmittedBlock{&block, function.entry_address});
            if (!inserted) {
                if (!equivalent_ir_block(*existing->second.block, block))
                    divergent_block_owners.emplace_back(
                        block.start_address,
            ...
            const auto append_dispatch_entry = [&](const std::uint32_t address) {
                if (address < block.start_address || address >= end ||
                    !dispatch_entry_addresses.insert(address).second)
                    throw std::runtime_error(
                        "Nativer AOT-Resume-Entry kollidiert mit einem DispatchBlock.");
                auto timing_class =
                    katana::runtime::ExecutableBlockTimingClass::PureCpu;
                std::uint64_t maximum_guest_cycles = 0u;
                bool has_guarded_linear_ram_access = false;
                bool guarded_cycle_flush_chainable = true;
                std::unordered_set<std::uint32_t> timed_instructions;
                for (const auto& instruction : block.instructions) {
                    if (instruction.source_address < address ||
                        !timed_instructions.insert(instruction.source_address).second)
                        continue;
                    const auto timing =
                        katana::sh4::instruction_timing(instruction.original_opcode);
                    maximum_guest_cycles += timing.guest_cycles;
                    if (timing.timing_class ==
                        katana::sh4::InstructionTimingClass::DeviceBusBoundary) {
                        timing_class =
                            katana::runtime::ExecutableBlockTimingClass::NeverChain;
                        guarded_cycle_flush_chainable = false;
                        ...
                        if (has_proven_linear_ram_access(instruction)) {
                            if (timing_class ==
                                ...
                                timing_class =
                                    katana::runtime::ExecutableBlockTimingClass::LinearRamOnly;
                        } else if (detail::native_aot_has_guarded_linear_ram_access(
                                       instruction)) {
                            has_guarded_linear_ram_access = true;
                            timing_class = katana::runtime::
                                ExecutableBlockTimingClass::RequiresCycleFlush;
                        } else {
                            timing_class = katana::runtime::
                                ExecutableBlockTimingClass::RequiresCycleFlush;
                            guarded_cycle_flush_chainable = false;
            ...
            const bool immutable_boot_block =
    ...
    if (!divergent_block_owners.empty()) {
        std::sort(divergent_block_owners.begin(),
                  divergent_block_owners.end());
        divergent_block_owners.erase(
            std::unique(divergent_block_owners.begin(),
                        divergent_block_owners.end()),
            divergent_block_owners.end());
        std::ostringstream reason;
        reason << "IR-Basic-Bloecke besitzen abweichende Funktionsbesitzer:";
        constexpr std::size_t maximum_reported_conflicts = 128u;
        const auto reported = std::min(
            divergent_block_owners.size(),
            maximum_reported_conflicts);
        for (std::size_t index = 0u; index < reported; ++index) {
            const auto [block, first_owner, other_owner] =
                divergent_block_owners[index];
            reason << " {block=" << guarded_aot_address(block)
                   << ",first=" << guarded_aot_address(first_owner)
                   << ",other=" << guarded_aot_address(other_owner) << '}';
        }
        if (reported != divergent_block_owners.size())
            reason << " (" << (divergent_block_owners.size() - reported)
                   << " weitere)";
        throw std::runtime_error(reason.str());
    }
    if (emitted_resume_entries.size() != requested_resume_entries.size())
        throw std::runtime_error(
            "Nicht jeder native AOT-Resume-Entry erhielt einen statischen DispatchBlock.");
    ...
    static_return_nop_callbacks.reserve(emitted_blocks.size());
    for (const auto& [address, emitted] : emitted_blocks) {
        if (!is_exact_return_nop_callback(*emitted.block)) continue;
        const auto& terminal = emitted.block->instructions[0];
        const auto& delay = emitted.block->instructions[1];
        ...
        if (terminal_timing.guest_cycles == 0u ||
            ...
            throw std::runtime_error(
                "Exakter RTS/NOP-Callback besitzt keinen rein lokalen Timingvertrag.");
        ...
        for (const auto& runtime_image :
            ...
            for (const auto& block : dispatch_blocks) {
                ...
                if (!emission.block_identities.empty() &&
                    emission.block_identities.back().source_offset >= offset)
                    throw std::runtime_error(
                        "Game-Project-Runtime-Image besitzt einen "
                        "mehrdeutigen AOT-Einstieg.");
    ...
    std::set<std::string> latent_source_binding_ids;
    std::vector<std::pair<std::uint64_t, std::uint64_t>>
        latent_source_binding_extents;
    std::unordered_set<std::uint32_t> latent_aot_source_blocks;
    std::size_t latent_source_binding_count = 0u;
    ...
    const auto valid_source_binding_id = [](const std::string_view id) {
    ...
    for (const auto& module : latent_modules) {
        ...
        for (const auto& binding : module.source_bindings) {
            if (!valid_source_binding_id(binding.id) ||
                ...
                binding.disc_byte_offset >
                    std::numeric_limits<std::uint64_t>::max() -
                        binding.byte_size ||
                latent_source_binding_count >= 1024u ||
                binding.byte_size >
                    256u * 1024u * 1024u -
                        latent_source_binding_bytes)
                throw std::runtime_error(
                    "Latentes AOT-Modul besitzt eine ungueltige "
                    "Disc-Source-Bindung.");
            const auto binding_end =
                binding.disc_byte_offset + binding.byte_size;
            if (std::any_of(
                    latent_source_binding_extents.begin(),
                    latent_source_binding_extents.end(),
                    [&](const auto& extent) {
                        return binding.disc_byte_offset < extent.second &&
                               extent.first < binding_end;
                    }))
                throw std::runtime_error(
                    "Latente AOT-Disc-Source-Bindungen ueberlappen.");
            latent_source_binding_extents.emplace_back(
                binding.disc_byte_offset, binding_end);
            ++latent_source_binding_count;
            latent_source_binding_bytes += binding.byte_size;
        }
        std::optional<std::uint32_t> previous_identity_offset;
        ...
        for (const auto& identity : module.block_identities) {
            ...
            if ((identity.source_offset & 1u) != 0u ||
                ...
                (previous_identity_offset.has_value() &&
                 identity.source_offset <= *previous_identity_offset) ||
            ...
            for (const auto& block : function.blocks) {
                if (block.start_address < module.source_address)
                    throw std::runtime_error(
                        ...
                        "Source-Range.");
                module_dispatch_entries.insert(block.start_address);
                for (const auto resume :
                     detail::native_aot_internal_resume_entries(block))
                    module_dispatch_entries.insert(resume);
            }
        }
        if (module_dispatch_entries.size() !=
            module.block_identities.size())
            throw std::runtime_error(
                "Latentes AOT-Modul besitzt ungebundene oder fehlende "
                "Block-/Resume-Identitaeten.");
        for (const auto source_address : module_dispatch_entries) {
            const auto source_offset =
                source_address - module.source_address;
            ...
            if (identity == module.block_identities.end() ||
                ...
                dispatch->address != source_address ||
                identity->size != dispatch->size)
                throw std::runtime_error(
                    "Latentes AOT-Modul verlor die exakte "
                    "DispatchBlock-/Resume-Identitaet.");
            latent_aot_source_blocks.insert(source_address);
        }
    }
    ...
    internal_header
        ...
        << "[[nodiscard]] bool try_static_return_nop_callback(\n"
           "    katana::runtime::CpuState& cpu, std::uint32_t target) noexcept;\n"
    ...
    for (std::size_t shard = 0u; shard < shard_count; ++shard) {
        ...
        for (auto index = begin; index < end; ++index) {
            const auto& block = dispatch_blocks[index];
            const auto address = symbol(block.address);
            const auto static_fastpath =
                static_fastpath_bindings.find(block.address);
            const bool has_static_fastpath_binding =
                static_fastpath != static_fastpath_bindings.end() &&
                static_fastpath->second.size() == 1u;
            const bool entry_chainable_mmio_wait =
                has_static_fastpath_binding &&
                static_fastpath->second.front().kind ==
                    StaticFastpathBindingKind::MmioWait;
            const bool entry_chainable =
                (std::string_view(block.timing_class) == "PureCpu" ||
                 std::string_view(block.timing_class) == "LinearRamOnly" ||
                 block.guarded_cycle_flush_chainable ||
                 entry_chainable_mmio_wait) &&
                ...
                (!has_static_fastpath_binding || entry_chainable_mmio_wait) &&
                !external_hook_entries.contains(block.address);
            if (template_source_blocks.contains(block.address)) {
                shard_output
                    << "    services.register_runtime_aot_chain_contract(0x"
                    << address << "u, " << block.size
                    << "u, katana::runtime::ExecutableBlockTimingClass::"
                    << block.timing_class << ", " << block.maximum_guest_cycles
                    << "u, " << (entry_chainable ? "true" : "false") << ");\n";
                continue;
            }
    ...
    output << "struct StaticReturnNopCallbackProof {\n"
              "    std::uint32_t source_address;\n"
              "    std::uint64_t return_guest_cycles;\n"
              "    std::uint64_t delay_guest_cycles;\n"
              "};\n"
    ...
    output << "}};\n"
           ...
           << "thread_local bool static_aot_escape_site_detail_enabled = false;\n"
           ...
           << " || enabled;\n"
           << "}\n"
           << "bool static_aot_escape_site_details_enabled() noexcept {\n"
           << "    static const bool enabled = [] {\n"
           << "        const auto* value =\n"
              "            std::getenv(\"KATANA_STATIC_AOT_ESCAPE_SITES\");\n"
           << "        return value != nullptr && std::string_view(value) == \"1\";\n"
           << "    }();\n"
           << "    return enabled || detailed_dispatch_diagnostics_enabled();\n"
           << "}\n"
           << "enum class DispatchChainBoundary { NestedCall, ProgramRoot };\n"
           << "class DispatchChainExitStateFrame final {\n"
           << "  public:\n"
           << "    explicit DispatchChainExitStateFrame(const bool restore) noexcept\n"
           << "        : restore_(restore), source_(active_exit_source),\n"
           << "          kind_(active_exit_kind), site_class_(active_exit_site_class),\n"
           << "          tail_dispatch_(tail_dispatch_completed) {}\n"
           << "    ~DispatchChainExitStateFrame() noexcept {\n"
           << "        if (!restore_) return;\n"
           << "        active_exit_source = source_; active_exit_kind = kind_;\n"
           << "        active_exit_site_class = site_class_;\n"
           << "        tail_dispatch_completed = tail_dispatch_;\n"
           << "    }\n"
           << "    DispatchChainExitStateFrame(const DispatchChainExitStateFrame&) = delete;\n"
           << "    DispatchChainExitStateFrame& operator=(\n"
           << "        const DispatchChainExitStateFrame&) = delete;\n"
           << "  private:\n"
           << "    bool restore_ = false;\n"
           << "    katana::runtime::BlockAddress source_{};\n"
           << "    katana::runtime::BlockEndKind kind_ =\n"
           << "        katana::runtime::BlockEndKind::Fallthrough;\n"
           << "    katana::runtime::DynamicDispatchSiteClass site_class_ =\n"
           << "        katana::runtime::DynamicDispatchSiteClass::NotDynamic;\n"
           << "    bool tail_dispatch_ = false;\n"
           << "};\n"
           ...
           << "        active_services = &services; active_table = &table;\n"
              ...
              "        active_observations = &observations;\n"
               ...
               "        services.bind_runtime_dispatch_context(&table, &materializer);\n"
               "        executed_dispatch_blocks = 0u;\n"
           << "        static_aot_escape_site_detail_enabled =\n"
              "            static_aot_escape_site_details_enabled();\n"
           << "        static_aot_escape_counts.fill(0u);\n"
           << "        static_aot_classified_dispatches = 0u;\n"
           << "        if (static_aot_escape_site_detail_enabled) {\n"
           << "            static_aot_escape_site_counts.assign("
           << (dispatch_blocks.size() - template_source_blocks.size() + 1u)
           << "u, {});\n"
           ...
           << "            static_aot_escape_site_addresses.clear();\n"
           << "        }\n"
           << "    }\n"
           << "    ~ServiceScope() {\n"
              "        if (active_services != nullptr)\n"
              "            active_services->bind_runtime_dispatch_context(nullptr, nullptr);\n"
              "        active_services = nullptr; active_table = nullptr;\n"
              ...
              "        static_aot_escape_site_detail_enabled = false; }\n"
           << "};\n";
    if (diagnostic_interpreter)
    ...
    if (!diagnostic_interpreter) {
        for (const auto* proof : composite_callback_emission) {
            ...
            output << "    {" << kind << ", 0x"
                   ...
                   << symbol(descriptor.kernel_body_address) << "u, 0x"
                   ...
                   << descriptor.kernel_body_guest_cycles << "u, "
                   ...
                   << "\", ";
            if (descriptor.kernel_body_size != 0u)
                output << "\"generated-block-"
                       << symbol(descriptor.kernel_body_address) << "\", ";
            else
                output << "\"\", ";
    ...
    if (!diagnostic_interpreter) {
        for (const auto& proof : memory_fill_loops) {
            ...
            const auto kind = descriptor.kind ==
                    MemoryFillLoopBatchProof::Kind::DirectMemoryCopy
                ? "MemoryLoopBatchKind::DirectMemoryCopy"
                : "MemoryLoopBatchKind::Fill";
            output << "    {" << kind << ", 0x"
                   << symbol(descriptor.guard_address) << "u, 0x"
                   ...
                   << descriptor.round_guest_cycles << "u, 0x"
                   << symbol(descriptor.source_load_instruction_address) << "u, 0x"
                   << symbol(descriptor.decrement_instruction_address) << "u, "
                   << static_cast<unsigned>(descriptor.source_register) << "u, "
                   << static_cast<unsigned>(descriptor.value_register) << "u, "
                   << static_cast<unsigned>(descriptor.counter_register)
                   << "u, \"generated-block-"
    ...
    output
        << "std::uint64_t configured_block_budget() {\n"
           ...
           "    DispatchChainExitStateFrame dispatch_chain_exit_state(\n"
           "        boundary == DispatchChainBoundary::NestedCall);\n"
    ...
    output <<
           ...
           "            if (cpu.pc == program_return_sentinel)\n"
           "                return;\n"
    ...
    output << "    static_blocks.reserve("
           << dispatch_blocks.size() - template_source_blocks.size()
           << "u);\n"
    ...
    output << "    static_cast<void>(table.register_static_contextual_bulk(\n"
              "        std::move(static_blocks)));\n"
           << "    katana::runtime::RuntimeBlockTable fixed_source_table;\n"
           << "    static_cast<void>(fixed_source_table.register_static_contextual_bulk(\n"
              "        std::move(fixed_source_blocks)));\n"
           << "    services.reserve_runtime_aot_chain_contracts("
           << template_source_blocks.size() << "u);\n";
    for (std::size_t shard = 0u; shard < shard_count; ++shard)
    ...
    output << "    last_materialization_status = {};\n"
           ...
           << "    }\n"
           << "    if (static_aot_escape_site_detail_enabled) {\n"
           << "        constexpr auto partition_reason = static_cast<std::size_t>(\n"
              "            StaticAotEscapeReason::PartitionOrSymbol);\n"
           << "        std::vector<StaticAotEscapeSiteCounter> partition_sites;\n"
           << "        std::uint64_t partition_total = 0u;\n"
           << "        for (std::size_t site = 0u;\n"
              "             site < static_aot_escape_site_counts.size(); ++site) {\n"
           << "            const auto count =\n"
              "                static_aot_escape_site_counts[site][partition_reason];\n"
           << "            if (count == 0u) continue;\n"
           << "            partition_total += count;\n"
           << "            partition_sites.push_back({\n"
           << "                static_cast<std::uint64_t>(site),\n"
           << "                static_aot_escape_site_addresses[site],\n"
              "                StaticAotEscapeReason::PartitionOrSymbol, count});\n"
           << "        }\n"
           << "        std::sort(partition_sites.begin(), partition_sites.end(),\n"
           << "                  [](const auto& left, const auto& right) {\n"
           << "                      return left.count != right.count\n"
           << "                          ? left.count > right.count\n"
           << "                          : left.site_id < right.site_id;\n"
           << "                  });\n"
           << "        constexpr std::size_t reported_partition_sites = 64u;\n"
           << "        std::cerr << \"KATANA_STATIC_AOT_PARTITION_SITES total=\"\n"
           << "                  << partition_total << \" nonzero=\"\n"
              "                  << partition_sites.size();\n"
           << "        const auto reported = std::min(\n"
              "            reported_partition_sites, partition_sites.size());\n"
           << "        for (std::size_t index = 0u; index < reported; ++index) {\n"
           << "            const auto& site = partition_sites[index];\n"
           << "            std::cerr << \" rank\" << index << \"_site_id=\"\n"
              "                      << site.site_id << \" rank\" << index\n"
              "                      << \"_guest_address=\" << site.guest_address\n"
              "                      << \" rank\" << index << \"_count=\"\n"
              "                      << site.count;\n"
           << "        }\n"
           << "        std::cerr << '\\n';\n"
...
std::string root_cmake(const bool diagnostic_partial,
                       const bool native_definition_present,
                       const bool native_hardware_closure_complete,
                       const std::size_t native_hardware_gap_count) {
    return "cmake_minimum_required(VERSION 3.25)\n"
           "project(KatanaPort LANGUAGES CXX)\n"
           "set(KATANA_PORT_EXPECTED_PROJECT_VERSION \"" +
           std::string(katana::build_contract::project_version) +
           "\")\n"
           "set(KATANA_PORT_EXPECTED_RUNTIME_ABI_VERSION \"" +
           std::to_string(katana::build_contract::runtime_abi_version) +
           "\")\n"
           "set(KATANA_PORT_EXPECTED_BLOCK_ABI_VERSION \"" +
           std::to_string(katana::build_contract::block_abi_version) +
           "\")\n"
           "set(KATANA_PORT_EXPECTED_PLATFORM_SERVICES_ABI_VERSION \"" +
           std::to_string(
               katana::build_contract::platform_services_abi_version) +
           "\")\n"
           "set(KATANA_PORT_EXPECTED_PROJECT_CONTRACT_VERSION \"" +
           std::to_string(
           ...
           "set(KATANA_RUNTIME_BUILD_TARGETS \"\" CACHE FILEPATH "
           "\"KatanaRecomp build-tree runtime target export\")\n"
           "set(KATANA_PORT_DIAGNOSTIC_RUNTIME " +
           std::string(diagnostic_partial ? "ON" : "OFF") +
           ")\n"
           "set(KATANA_NATIVE_PORT_DEFINITION_PRESENT " +
           std::string(native_definition_present ? "ON" : "OFF") +
           ")\n"
           "set(KATANA_NATIVE_PORT_HARDWARE_CLOSURE_COMPLETE " +
           std::string(native_hardware_closure_complete ? "ON" : "OFF") +
           ")\n"
           "set(KATANA_NATIVE_PORT_HARDWARE_GAP_COUNT \"" +
           std::to_string(native_hardware_gap_count) +
           "\")\n"
           // The independent definition/link contract exists now, but the
           // generated title bootstrap and direct hook calls are the next
           // vertical task. Never let the legacy Dreamcast main.cpp become an
           // accidental native-product fallback while that codegen is absent.
           "set(KATANA_NATIVE_PRODUCT_BOOTSTRAP_EMITTED OFF)\n"
           "set(KATANA_PORT_RUNTIME_PROFILE \"" +
           std::string(diagnostic_partial ? "diagnostic-interpreter"
                                          : "native-port") +
           "\" CACHE STRING \"Runtime profile: native-port or "
           "diagnostic-interpreter\")\n"
           "set_property(CACHE KATANA_PORT_RUNTIME_PROFILE PROPERTY STRINGS "
           "native-port diagnostic-interpreter)\n"
           ...
           "  endif()\n"
           "elseif(NOT KATANA_PORT_RUNTIME_PROFILE STREQUAL \"native-port\")\n"
           "  message(FATAL_ERROR \"Product exports require native-port; "
           "the historical device runtime is not a generated-port profile\")\n"
           "elseif(NOT KATANA_NATIVE_PORT_DEFINITION_PRESENT)\n"
           "  message(FATAL_ERROR \"KATANA_MISSING_NATIVE_PORT_DEFINITION\")\n"
           "elseif(NOT KATANA_NATIVE_PRODUCT_BOOTSTRAP_EMITTED)\n"
           "  message(FATAL_ERROR \"KATANA_NATIVE_BOOTSTRAP_CODEGEN_PENDING\")\n"
           "endif()\n"
           ...
           "endif()\n"
           "if(KATANA_PORT_BUILD_PROFILE STREQUAL \"gate\" AND\n"
           "   NOT KATANA_PORT_DIAGNOSTIC_RUNTIME AND\n"
           "   NOT KATANA_NATIVE_PORT_HARDWARE_CLOSURE_COMPLETE)\n"
           "  message(FATAL_ERROR "
           "\"KATANA_NATIVE_HARDWARE_CLOSURE_INCOMPLETE "
           "gaps=${KATANA_NATIVE_PORT_HARDWARE_GAP_COUNT}\")\n"
           "endif()\n"
           "if(KATANA_RUNTIME_ROOT STREQUAL \"\" AND\n"
           "   KATANA_RUNTIME_PREFIX STREQUAL \"\" AND\n"
           "   KATANA_RUNTIME_BUILD_TARGETS STREQUAL \"\")\n"
           "  set(KATANA_RUNTIME_ENV_BINDING_COUNT 0)\n"
           "  if(NOT \"$ENV{KATANA_RUNTIME_ROOT}\" STREQUAL \"\")\n"
           "    math(EXPR KATANA_RUNTIME_ENV_BINDING_COUNT "
           "\"${KATANA_RUNTIME_ENV_BINDING_COUNT} + 1\")\n"
           "  endif()\n"
           "  if(NOT \"$ENV{KATANA_RUNTIME_PREFIX}\" STREQUAL \"\")\n"
           "    math(EXPR KATANA_RUNTIME_ENV_BINDING_COUNT "
           "\"${KATANA_RUNTIME_ENV_BINDING_COUNT} + 1\")\n"
           "  endif()\n"
           "  if(NOT \"$ENV{KATANA_RUNTIME_BUILD_TARGETS}\" STREQUAL \"\")\n"
           "    math(EXPR KATANA_RUNTIME_ENV_BINDING_COUNT "
           "\"${KATANA_RUNTIME_ENV_BINDING_COUNT} + 1\")\n"
           "  endif()\n"
           "  if(KATANA_RUNTIME_ENV_BINDING_COUNT GREATER 1)\n"
           "    message(FATAL_ERROR "
           "\"Configure exactly one Katana runtime environment binding\")\n"
           "  elseif(NOT \"$ENV{KATANA_RUNTIME_PREFIX}\" STREQUAL \"\")\n"
           "    file(TO_CMAKE_PATH \"$ENV{KATANA_RUNTIME_PREFIX}\" "
           "KATANA_RUNTIME_PREFIX)\n"
           "  elseif(NOT \"$ENV{KATANA_RUNTIME_BUILD_TARGETS}\" STREQUAL \"\")\n"
           "    file(TO_CMAKE_PATH \"$ENV{KATANA_RUNTIME_BUILD_TARGETS}\" "
           "KATANA_RUNTIME_BUILD_TARGETS)\n"
           "  elseif(NOT \"$ENV{KATANA_RUNTIME_ROOT}\" STREQUAL \"\")\n"
           "    file(TO_CMAKE_PATH \"$ENV{KATANA_RUNTIME_ROOT}\" "
           "KATANA_RUNTIME_ROOT)\n"
           "  endif()\n"
           "endif()\n"
           "set(KATANA_RUNTIME_BINDING_COUNT 0)\n"
           "foreach(KATANA_RUNTIME_BINDING IN ITEMS "
           "KATANA_RUNTIME_ROOT KATANA_RUNTIME_PREFIX "
           "KATANA_RUNTIME_BUILD_TARGETS)\n"
           "  if(NOT \"${${KATANA_RUNTIME_BINDING}}\" STREQUAL \"\")\n"
           "    math(EXPR KATANA_RUNTIME_BINDING_COUNT "
           "\"${KATANA_RUNTIME_BINDING_COUNT} + 1\")\n"
           "  endif()\n"
           "endforeach()\n"
           "if(KATANA_RUNTIME_BINDING_COUNT GREATER 1)\n"
           "  message(FATAL_ERROR \"Configure exactly one Katana runtime binding\")\n"
           "endif()\n"
           ...
           "endif()\n"
           "if(NOT KATANA_RUNTIME_BUILD_TARGETS STREQUAL \"\")\n"
           "  if(NOT IS_ABSOLUTE \"${KATANA_RUNTIME_BUILD_TARGETS}\" OR\n"
           "     NOT EXISTS \"${KATANA_RUNTIME_BUILD_TARGETS}\" OR\n"
           "     IS_DIRECTORY \"${KATANA_RUNTIME_BUILD_TARGETS}\")\n"
           "    message(FATAL_ERROR \"Katana build-tree runtime export is invalid\")\n"
           "  endif()\n"
           "  find_package(Threads REQUIRED)\n"
           "  include(\"${KATANA_RUNTIME_BUILD_TARGETS}\")\n"
           "  if(TARGET \"${KATANA_PORT_NAMESPACED_RUNTIME_TARGET}\")\n"
           "    set(KATANA_PORT_RUNTIME_TARGET "
           "\"${KATANA_PORT_NAMESPACED_RUNTIME_TARGET}\")\n"
           "  else()\n"
           "    message(FATAL_ERROR \"Katana build-tree export has no requested "
           "runtime target\")\n"
           "  endif()\n"
           "elseif(NOT KATANA_RUNTIME_PREFIX STREQUAL \"\")\n"
           ...
           "  endif()\n"
           "endif()\n"
           "function(katana_require_runtime_contract target)\n"
           "  get_target_property(KATANA_PORT_RESOLVED_RUNTIME_TARGET "
           "\"${target}\" ALIASED_TARGET)\n"
           "  if(NOT KATANA_PORT_RESOLVED_RUNTIME_TARGET)\n"
           "    set(KATANA_PORT_RESOLVED_RUNTIME_TARGET \"${target}\")\n"
           "  endif()\n"
           "  get_target_property(KATANA_PORT_RUNTIME_IMPORTED\n"
           "                      \"${KATANA_PORT_RESOLVED_RUNTIME_TARGET}\" IMPORTED)\n"
           "  if(KATANA_PORT_RUNTIME_IMPORTED)\n"
           "    get_target_property(KATANA_PORT_RUNTIME_CONFIGURATIONS\n"
           "                        \"${KATANA_PORT_RESOLVED_RUNTIME_TARGET}\"\n"
           "                        IMPORTED_CONFIGURATIONS)\n"
           "    set(KATANA_PORT_OPTIMIZED_RUNTIME_CONFIGURATIONS \"\")\n"
           "    foreach(KATANA_PORT_PREFERRED_CONFIG\n"
           "            IN ITEMS RELWITHDEBINFO RELEASE MINSIZEREL)\n"
           "      foreach(KATANA_PORT_AVAILABLE_CONFIG\n"
           "              IN LISTS KATANA_PORT_RUNTIME_CONFIGURATIONS)\n"
           "        string(TOUPPER \"${KATANA_PORT_AVAILABLE_CONFIG}\"\n"
           "               KATANA_PORT_AVAILABLE_CONFIG_UPPER)\n"
           "        if(KATANA_PORT_AVAILABLE_CONFIG_UPPER STREQUAL\n"
           "           KATANA_PORT_PREFERRED_CONFIG)\n"
           "          list(APPEND KATANA_PORT_OPTIMIZED_RUNTIME_CONFIGURATIONS\n"
           "               \"${KATANA_PORT_AVAILABLE_CONFIG}\")\n"
           "        endif()\n"
           "      endforeach()\n"
           "    endforeach()\n"
           "    if(NOT KATANA_PORT_OPTIMIZED_RUNTIME_CONFIGURATIONS)\n"
           "      message(FATAL_ERROR\n"
           "              \"Katana runtime import has no optimized configuration\")\n"
           "    endif()\n"
           "    set_property(TARGET \"${KATANA_PORT_RESOLVED_RUNTIME_TARGET}\"\n"
           "                 PROPERTY MAP_IMPORTED_CONFIG_RELWITHDEBINFO\n"
           "                 \"${KATANA_PORT_OPTIMIZED_RUNTIME_CONFIGURATIONS}\")\n"
           "    set_property(TARGET \"${KATANA_PORT_RESOLVED_RUNTIME_TARGET}\"\n"
           "                 PROPERTY MAP_IMPORTED_CONFIG_RELEASE\n"
           "                 \"${KATANA_PORT_OPTIMIZED_RUNTIME_CONFIGURATIONS}\")\n"
           "  endif()\n"
           "  get_target_property(KATANA_PORT_ACTUAL_PROJECT_VERSION "
           "\"${KATANA_PORT_RESOLVED_RUNTIME_TARGET}\" KATANA_PROJECT_VERSION)\n"
           "  get_target_property(KATANA_PORT_ACTUAL_RUNTIME_ABI_VERSION "
           "\"${KATANA_PORT_RESOLVED_RUNTIME_TARGET}\" KATANA_RUNTIME_ABI_VERSION)\n"
           "  get_target_property(KATANA_PORT_ACTUAL_BLOCK_ABI_VERSION "
           "\"${KATANA_PORT_RESOLVED_RUNTIME_TARGET}\" KATANA_BLOCK_ABI_VERSION)\n"
           "  get_target_property(KATANA_PORT_ACTUAL_PLATFORM_SERVICES_ABI_VERSION "
           "\"${KATANA_PORT_RESOLVED_RUNTIME_TARGET}\" "
           "KATANA_PLATFORM_SERVICES_ABI_VERSION)\n"
           "  get_target_property(KATANA_PORT_ACTUAL_PROJECT_CONTRACT_VERSION "
           "\"${KATANA_PORT_RESOLVED_RUNTIME_TARGET}\" "
           "KATANA_PORT_PROJECT_CONTRACT_VERSION)\n"
           "  if(NOT KATANA_PORT_ACTUAL_PROJECT_VERSION STREQUAL "
           "KATANA_PORT_EXPECTED_PROJECT_VERSION OR\n"
           "     NOT KATANA_PORT_ACTUAL_RUNTIME_ABI_VERSION STREQUAL "
           "KATANA_PORT_EXPECTED_RUNTIME_ABI_VERSION OR\n"
           "     NOT KATANA_PORT_ACTUAL_BLOCK_ABI_VERSION STREQUAL "
           "KATANA_PORT_EXPECTED_BLOCK_ABI_VERSION OR\n"
           "     NOT KATANA_PORT_ACTUAL_PLATFORM_SERVICES_ABI_VERSION STREQUAL "
           "KATANA_PORT_EXPECTED_PLATFORM_SERVICES_ABI_VERSION OR\n"
           "     NOT KATANA_PORT_ACTUAL_PROJECT_CONTRACT_VERSION STREQUAL "
           "KATANA_PORT_EXPECTED_PROJECT_CONTRACT_VERSION)\n"
           "    message(FATAL_ERROR\n"
           "      \"KatanaRecomp runtime contract mismatch: expected \"\n"
           "      \"project=${KATANA_PORT_EXPECTED_PROJECT_VERSION},\"\n"
           "      \"runtime=${KATANA_PORT_EXPECTED_RUNTIME_ABI_VERSION},\"\n"
           "      \"block=${KATANA_PORT_EXPECTED_BLOCK_ABI_VERSION},\"\n"
           "      \"platform=${KATANA_PORT_EXPECTED_PLATFORM_SERVICES_ABI_VERSION},\"\n"
           "      \"port=${KATANA_PORT_EXPECTED_PROJECT_CONTRACT_VERSION}; actual \"\n"
           "      \"project=${KATANA_PORT_ACTUAL_PROJECT_VERSION},\"\n"
           "      \"runtime=${KATANA_PORT_ACTUAL_RUNTIME_ABI_VERSION},\"\n"
           "      \"block=${KATANA_PORT_ACTUAL_BLOCK_ABI_VERSION},\"\n"
           "      \"platform=${KATANA_PORT_ACTUAL_PLATFORM_SERVICES_ABI_VERSION},\"\n"
           "      \"port=${KATANA_PORT_ACTUAL_PROJECT_CONTRACT_VERSION}\")\n"
           "  endif()\n"
           "endfunction()\n"
...
std::string product_gate_runner() {
    return "param(\n"
           ...
           "    [ValidateRange(1, 840)][int]$WatchdogSeconds = 120\n"
           ")\n"
...
std::string native_port_link_audit(
    const katana::runtime::NativePortDefinition* const definition) {
    std::string script = R"cmake(cmake_minimum_required(VERSION 3.25)
...
set(KATANA_NATIVE_PORT_REQUIRED_TOKENS
@KATANA_NATIVE_PORT_REQUIRED_TOKENS@
)
set(KATANA_NATIVE_PORT_MISSING_TOKENS)
foreach(KATANA_NATIVE_PORT_TOKEN IN LISTS
    ...
    if(KATANA_NATIVE_PORT_TOKEN_OFFSET EQUAL -1)
        list(APPEND
             KATANA_NATIVE_PORT_MISSING_TOKENS
             "${KATANA_NATIVE_PORT_TOKEN}")
    endif()
endforeach()
if(KATANA_NATIVE_PORT_MISSING_TOKENS)
    list(JOIN KATANA_NATIVE_PORT_MISSING_TOKENS "," KATANA_NATIVE_PORT_REPORT)
    message(FATAL_ERROR
        "KATANA_NATIVE_PORT_LINK_VIOLATION missing=${KATANA_NATIVE_PORT_REPORT}")
endif()
...
set(KATANA_NATIVE_PORT_FORBIDDEN_TOKENS
    ...
    disc.cpp
    disc_install.cpp
    disc_load_transaction.cpp
    dma.cpp
    pvr.cpp
    dreamcast_boot.cpp
    dreamcast_memory.cpp
    firmware_handoff.cpp
    game_entry_handoff.cpp
    game_entry_platform_state.cpp
    game_project.cpp
    gdrom_controller.cpp
    holly_dma.cpp
    host_video.cpp
    host_video_d3d11.cpp
    interrupt.cpp
    io_port.cpp
    maple.cpp
    maple_mmio.cpp
...
message(STATUS
    ...
    required << "    katana_native_port_runtime\n"
             << "    katana_aot_runtime\n"
             << "    native_port_link_contract\n"
             << "    katana_native_bootstrap_dispatch\n"
             << "    katana_native_invoke_original\n"
             << "    katana_native_invoke_callback\n";
    if (definition != nullptr) {
        const auto append_symbol = [&](const std::string_view symbol) {
    ...
    constexpr std::string_view marker{
        "@KATANA_NATIVE_PORT_REQUIRED_TOKENS@"};
    const auto marker_offset = script.find(marker);
    if (marker_offset == std::string::npos)
        throw std::logic_error("native-port-link-audit-marker-missing");
    script.replace(marker_offset, marker.size(), required.str());
    return script;
}
...
std::string port_cmake(const std::string& target_name) {
    return "add_executable(" + target_name +
           ...
           "  target_link_options(" +
           target_name +
           " PRIVATE \"/STACK:${KATANA_PORT_HOST_STACK_RESERVE_BYTES}\")\n"
           "endif()\n"
...
port_metadata(const PortExportOptions& options,
    ...
    output << ",\"contract_version\":" << port_project_contract_version
           ...
           << ",\"guarded_contextual_provenance_replay_capsule_budget\":"
           << analysis.guarded_code_inventory_walk.contextual_provenance_replay_capsule_budget
           << ",\"guarded_contextual_provenance_replay_capsule_limited_functions\":"
           << analysis.guarded_code_inventory_walk.contextual_provenance_replay_capsule_limited_functions
           << ",\"guarded_contextual_provenance_replay_key_byte_budget\":"
           << analysis.guarded_code_inventory_walk.contextual_provenance_replay_key_byte_budget
           << ",\"guarded_contextual_provenance_replay_key_byte_limited_functions\":"
           << analysis.guarded_code_inventory_walk.contextual_provenance_replay_key_byte_limited_functions
...
void write_port_file(const std::filesystem::path& root,
    ...
    for (const auto& component : relative) {
        ...
        if (!error &&
            ...
            unsafe_port_path_link(candidate, status)) {
    ...
    std::error_code path_error;
    auto path_status =
        std::filesystem::symlink_status(path, path_error);
    const bool path_missing =
        path_error == std::errc::no_such_file_or_directory ||
        (!path_error &&
         path_status.type() ==
             std::filesystem::file_type::not_found);
    if (!path_missing &&
        (path_error ||
         ...
         unsafe_port_path_link(path, path_status)))
        throw std::runtime_error(
            "Port-Bootstrapdatei ist kein sicherer regulaerer Pfad.");
    if (!path_missing && !replace_existing) return;
    ensure_safe_port_directory(
        root, path.parent_path().lexically_relative(root));
    path_error.clear();
    path_status =
        std::filesystem::symlink_status(path, path_error);
    const bool path_still_missing =
        path_error == std::errc::no_such_file_or_directory ||
        (!path_error &&
         path_status.type() ==
             std::filesystem::file_type::not_found);
    if (!path_still_missing &&
        (path_error ||
         ...
         unsafe_port_path_link(path, path_status)))
        throw std::runtime_error(
            "Port-Bootstrapdatei wurde vor dem Schreiben unsicher.");
...
void require_unique_loaded_module_template_ids(
    ...
    if (game_project != nullptr) {
        for (const auto& native_template :
             game_project->runtime_code_templates) {
            if (native_template.destination !=
                katana::runtime::NativeAotTemplateDestination::LoadedModule)
                continue;
            if (!template_ids.insert(native_template.source_module_id).second)
                throw std::runtime_error(
                    "Externe LoadedModule-Template-ID ist nicht eindeutig.");
        }
    }
    for (const auto& module : latent_modules) {
        if (module.id.empty() || !template_ids.insert(module.id).second)
            throw std::runtime_error(
                "LoadedModule-Template-ID kollidiert zwischen "
                "GameProject und Latent-AOT.");
    }
}
...
void preserve_local_port_user_data(const std::filesystem::path& previous_root,
    ...
    const auto require_safe_user_data_tree =
        [&](const std::filesystem::path& root,
            ...
            if (status_error ||
                ...
                unsafe_port_path_link(root, root_status))
                throw std::runtime_error(
                    "Lokale Portdaten sind kein sicherer regulaerer Ordner.");
            for (std::filesystem::recursive_directory_iterator iterator(root),
                ...
                if (status_error ||
                    unsafe_port_path_link(iterator->path(), status))
                    throw std::runtime_error(
                        "Lokale Portdaten enthalten einen unsicheren "
                        "Dateisystemeintrag.");
                if (require_empty &&
                    !std::filesystem::is_directory(status))
                    throw std::runtime_error(
                        "Frisch publizierter Port besitzt unerwartet "
                        "lokale Datendateien.");
            }
        };
    ...
    require_safe_user_data_tree(previous, false);

    const auto published = published_root / "user-data";
    status_error.clear();
    const auto published_status =
        std::filesystem::symlink_status(published, status_error);
    const bool published_missing =
        status_error == std::errc::no_such_file_or_directory ||
        (!status_error &&
         published_status.type() ==
             std::filesystem::file_type::not_found);
    if (!published_missing) {
        if (status_error)
            throw std::filesystem::filesystem_error(
                "Frisch publizierte lokale Daten konnten nicht geprueft "
                "werden.",
                published,
                status_error);
        require_safe_user_data_tree(published, true);
    }
    status_error.clear();
    static_cast<void>(
        std::filesystem::remove_all(published, status_error));
    if (status_error)
        throw std::filesystem::filesystem_error(
...
struct NativePortHardwareClosure final {
    struct Gap final {
...
NativePortHardwareClosure evaluate_native_port_hardware_closure(
    ...
    std::map<std::uint32_t, const katana::runtime::NativePortHardwareResolution*>
        resolutions;
    for (const auto& resolution : definition->hardware_resolutions)
        resolutions.emplace(resolution.instruction_address, &resolution);
    std::set<std::uint32_t> consumed_resolutions;
    std::set<std::uint32_t> known_hardware_instructions;
    ...
    const auto covering_required_hook =
        [&](const std::uint32_t instruction_address,
            const std::uint32_t hook_address) {
            const auto hook = std::find_if(
                definition->hooks.begin(),
                definition->hooks.end(),
                [&](const auto& candidate) {
                    return candidate.guest_address == hook_address;
                });
            return hook != definition->hooks.end() &&
                   hook->requirement ==
                       katana::runtime::NativePortHookRequirement::Required &&
                   hook->original_policy ==
                       katana::runtime::NativePortHookOriginalPolicy::
                           ReplacesOriginal &&
                   instruction_address >= hook->guest_address &&
    ...
    const auto admit = [&](const std::uint32_t instruction_address,
        ...
        if (!covering_required_hook(
                instruction_address, resolution.hook_guest_address)) {
            result.gaps.push_back(
                {instruction_address, "hardware-hook-proof-invalid"});
            return;
        }
    ...
    for (const auto& diagnostic : audit.instruction_diagnostics)
        hook_required_instructions.insert(diagnostic.address);
    result.unknown_instruction_sites =
        audit.instruction_diagnostics.size();
    ...
    for (const auto& site :
        ...
        if (!hook_required_instructions.contains(site.instruction_address))
            admit(site.instruction_address,
                  ...
                  site.access_mask,
                  site.width_mask,
                  "dynamic-memory-access-requires-hook");
    }
    for (const auto& [address, resolution] : resolutions) {
        static_cast<void>(resolution);
        if (!consumed_resolutions.contains(address))
            result.gaps.push_back(
                {address, "stale-hardware-resolution"});
    }
    // The boot-image audit cannot silently stand in for separately materialized
    // runtime/overlay modules. Until each emitted module contributes its own
    // exact hardware-access audit, native product admission remains fail-closed.
    ...
    std::sort(
        ...
        result.gaps.end(),
        [](const auto& left, const auto& right) {
            return std::tie(left.instruction_address, left.reason) <
                   std::tie(right.instruction_address, right.reason);
        });
    result.complete = result.gaps.empty();
    return result;
}

std::string native_port_hardware_closure_json(
    const NativePortHardwareClosure& closure) {
    std::ostringstream output;
    katana::io::write_json_report_header(
        output,
        "katana.native-port-hardware-closure.v1",
    ...
    output << ",\"definition_present\":"
           ...
           << closure.proven_native_memory << ",\"gaps\":[";
    for (std::size_t index = 0u; index < closure.gaps.size(); ++index) {
        if (index != 0u) output << ',';
        output << "{\"instruction_address\":"
               << closure.gaps[index].instruction_address
               << ",\"reason\":"
               << katana::io::quote_json(closure.gaps[index].reason) << '}';
    }
    output << "]}";
    return output.str();
}
...
static PortExportResult export_dreamcast_port_project_impl(
    ...
    if (prepared.discover_latent_aot || !options.latent_aot_entry_hints.empty()) {
        ...
        for (std::size_t index = 0u;
             index < latent_aot.analysis_candidate_duration_ms.size();
             ++index) {
            report_progress(
                options,
                "latent-aot-module-analysis-ms:" +
                    std::to_string(index) + ':' +
                    std::to_string(
                        latent_aot.analysis_candidate_duration_ms[index]));
        }
    } else {
        auto latent_progress = options.progress.begin(
            katana::ProgressOperation::LatentAotAnalysis,
            katana::ProgressUnit::Modules,
            std::nullopt,
            "latent-aot-discovery");
        latent_progress.skipped();
    }
    ...
    if (native_port_definition != nullptr) {
        ...
        if (!disc_context.has_value())
            throw std::invalid_argument(
                "Native-Port-Export braucht eine validierte Disc- oder "
                "Boot-Artefaktidentitaet.");
        if (native_port_definition->executable.content_identity !=
            disc_context->recipe.content_identity)
            throw std::invalid_argument(
                "Native-Port-Contentidentitaet passt nicht zur "
                "Installationsquelle.");
        const auto* const boot_segment = prepared.image.find_segment(
            prepared.boot_address, prepared.boot_size);
        if (boot_segment == nullptr ||
            boot_segment->local_source_name !=
                native_port_definition->executable.executable_name)
            throw std::invalid_argument(
    ...
    auto partition_preparation_progress = options.progress.begin(
    ...
    if (!std::is_sorted(native_aot_resume_entries.begin(),
                        native_aot_resume_entries.end()) ||
        std::adjacent_find(native_aot_resume_entries.begin(),
                           native_aot_resume_entries.end()) !=
            native_aot_resume_entries.end())
        throw std::invalid_argument(
            "Native AOT-Resume-Entries muessen sortiert und eindeutig sein.");
    if (!native_aot_resume_entries.empty() && options.game_project == nullptr)
        throw std::invalid_argument(
            "Native AOT-Resume-Entries brauchen ein identitaetsgebundenes Game-Project.");
    const auto boot_end =
        static_cast<std::uint64_t>(prepared.boot_address) + prepared.boot_size;
    ...
    if (prepared.precomputed_hardware_audit == nullptr)
        computed_hardware_audit.emplace(
    ...
    const auto emit_partition = [&](const TranslationUnitPartition& partition) {
        ...
        const auto append_native_block_owner = [&](const std::uint32_t target) {
            ...
            partition_native_block_owners.push_back({target, owner->second});
        ...
        for (const auto& function : functions) {
            ...
            for (const auto& block : function.blocks) {
                if (external_hook_entries.contains(block.start_address) ||
                    ...
                    partition_architectural_boundaries.push_back(
                        block.start_address);
                const auto block_resume_candidates =
                    detail::native_aot_internal_resume_entries(block);
                for (const auto resume : native_aot_resume_entries) {
                    if (std::binary_search(block_resume_candidates.begin(),
                                           block_resume_candidates.end(),
                                           resume))
                        partition_architectural_boundaries.push_back(resume);
                }
                for (const auto successor : block.successors)
                ...
                for (const auto& instruction : block.instructions) {
                    ...
                    if (instruction.target_address.has_value())
                        append_native_block_owner(*instruction.target_address);
                    for (const auto target : instruction.resolved_targets) {
                        append_native_block_owner(target);
                    }
                ...
                if (terminal != nullptr) {
                    using Operation = katana::ir::Operation;
                    const auto fallthrough = terminal->source_address +
                        (terminal->delay_slot.role ==
                                 katana::ir::DelaySlotRole::Owner
                             ? 4u
                             : 2u);
                    switch (terminal->operation) {
                    ...
                    case Operation::CallRegister:
                        append_native_block_owner(fallthrough);
                        break;
    ...
    for (std::size_t worker = 0u; worker < codegen_jobs; ++worker) {
        workers.push_back(std::async(std::launch::async, [&] {
            for (;;) {
                ...
                active_partitions.fetch_add(
                    1u, std::memory_order_relaxed);
                const auto started =
                    started_partitions.fetch_add(
                        1u, std::memory_order_relaxed) +
                    1u;
                {
                    katana::ProgressCounterSnapshot counters;
    ...
    if (!metadata_cache_hit) {
        ...
        if (partition_cache) {
            const auto store_metadata_cache =
            ...
            store_metadata_cache("cfg.dot", control_flow_graph_dot);
            store_metadata_cache("callgraph.json", call_graph_json);
            store_metadata_cache("callgraph.dot", call_graph_dot);
        }
    }
    ...
    artifacts.push_back(
        {"verify-native-port-link.cmake",
         native_port_link_audit(native_port_definition)});
    ...
    artifacts.push_back(
        {"metadata/native-hardware-closure.json",
         native_port_hardware_closure_json(native_hardware_closure)});
    ...
    if (!root_error &&
        ...
        unsafe_port_path_link(absolute_root, root_status)) {
    ...
    write_port_file(canonical_root,
                    ...
                    true);
    ensure_safe_port_directory(canonical_root, "runtime");
    ensure_safe_port_directory(canonical_root, "user-data/content");
    report_progress(options, "artifact-write");
    auto artifact_progress = options.progress.begin(
        katana::ProgressOperation::ArtifactWrite,
        katana::ProgressUnit::Files,
        std::nullopt,
        "port-artifacts");
    ProjectWriteOptions write_options;
    write_options.parallel_jobs = codegen_jobs;
    write_options.progress =
        artifact_progress.child_reporter();
    const auto write = write_codegen_project(
        canonical_root / "generated",
        std::move(artifacts),
        write_options);
    ...
    write_port_file(
        canonical_root,
        "CMakeLists.txt",
        root_cmake(options.diagnostic_partial,
                   native_hardware_closure.definition_present,
    ...
    if (latent_aot.analysis_cache_positive_hits != 0u ||
        latent_aot.analysis_cache_negative_hits != 0u)
        result.checkpoints.insert(
            result.checkpoints.begin() + 2u,
            "latent-aot-analysis-cache-hit");
    if (options.game_project != nullptr)
        result.checkpoints.insert(
            result.checkpoints.begin() + 2u,
    ...
    if (cache_enabled) {
        ...
        try {
            if (auto stored = cache->load_bounded(
                ...
                auto parsed = parse_boot_analysis_cache(
                    cache_key,
                    std::span(
                ...
                katana::analysis::DreamcastHardwareAudit
                    rebound_hardware_audit;
                if (parsed.state == BootAnalysisCacheState::Hit &&
                    validate_boot_analysis_cache_source_binding(
                        parsed.artifact,
                        image,
                        overrides,
                        &rebound_hardware_audit)) {
                    auto result = PreparedBootAnalysisRun{};
                    result.artifact =
                        std::move(parsed.artifact);
                    result.hardware_audit =
                        std::move(rebound_hardware_audit);
                    ...
                    const auto cached_phase =
                        [&options](
                            ...
                            auto scope = options.progress.begin(
                                operation,
                                unit,
                                std::nullopt,
                                std::string(label));
            ...
            // The cache is an optimization. Unsafe or concurrently replaced
            // entries become a fail-closed miss.
            cache_entry_absent = false;
        }
    }
    ...
    if (cache != nullptr && cache_entry_absent &&
        ...
        try {
            const auto serialized =
                serialize_boot_analysis_cache(
                    cache_key, result.artifact);
            cache->store_bounded(
                cache_key,
                artifact_name,
                std::string_view(
            ...
            // Fresh analysis remains authoritative if cache publication loses
            // a race or the local cache tree is unsafe.
        }
    }
    return result;
}
...
PortExportResult export_dreamcast_port_project(
    ...
    auto provenance_progress = options.progress.begin(
        katana::ProgressOperation::InputProvenance,
        katana::ProgressUnit::Files,
        inputs.size(),
        "validated-disc-provenance");
    provenance_progress.cached();
    const auto project_identity =
        katana::platform::dreamcast_disc_project_identity(disc);
    auto report = export_dreamcast_port_project_impl(
        {image,
         analysis,
         program,
         inputs,
...
PortExportResult export_dreamcast_port_project_from_boot_artifact(
    ...
    auto artifact_load_progress = options.progress.begin(
        katana::ProgressOperation::DiscLoad,
        katana::ProgressUnit::Files,
        1u,
        "boot-executable-artifact");
    const auto artifact =
        katana::platform::load_dreamcast_boot_executable_artifact(
            artifact_manifest_path);
    artifact_load_progress.complete();
    ...
    auto boot_image_progress = options.progress.begin(
        ...
        katana::ProgressUnit::Steps,
        1u,
        "boot-artifact-executable-image");
    auto image =
    ...
    inputs.push_back(katana::io::capture_input_provenance(
        "boot-executable-manifest",
        artifact.manifest_path,
        {},
        options.progress));
    inputs.push_back(katana::io::capture_input_provenance(
        "boot-executable-private",
        artifact.executable_path,
        {},
        options.progress));
    inputs.push_back(katana::io::capture_input_provenance(
        "disc-install-recipe",
        artifact.install_recipe_path,
        {},
        options.progress));
