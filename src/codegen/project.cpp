...
std::string cmake_project(const std::vector<std::filesystem::path>& sources) {
    ...
    output << "cmake_minimum_required(VERSION 3.25)\n"
           ...
           << "     NOT TARGET KatanaRecomp::runtime_core AND NOT TARGET katana_runtime_core)\n"
           << "    find_package(KatanaRecomp CONFIG QUIET)\n"
           << "  endif()\n"
           << "  if(TARGET KatanaRecomp::runtime_core)\n"
           << "    set(KATANA_GENERATED_RUNTIME_TARGET KatanaRecomp::runtime_core)\n"
           << "  elseif(TARGET katana_runtime_core)\n"
           << "    set(KATANA_GENERATED_RUNTIME_TARGET katana_runtime_core)\n"
