...
std::string cmake_project(const std::vector<std::filesystem::path>& sources) {
    ...
    output << ")\n"
           ...
           << "# final ThinLTO backend. The central dispatcher is already one locally\n"
           << "# optimized translation unit whose indirect AOT calls cannot profit from\n"
           << "# cross-unit inlining; keeping it out removes a large serial LTO backend.\n"
           << "# Primary 1ST_READ units and up to 64 measured overlays retain full IPO.\n"
