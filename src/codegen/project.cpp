...
std::string cmake_project(const std::vector<std::filesystem::path>& sources) {
    ...
    output << "cmake_minimum_required(VERSION 3.25)\n"
           ...
           << "    configure_file(\n"
           << "      \"${KATANA_RUNTIME_ROOT}/include/katana/abi_contract.hpp.in\"\n"
           << "      \"${CMAKE_CURRENT_BINARY_DIR}/generated/include/katana/"
              "abi_contract.hpp\"\n"
           << "      @ONLY\n"
           << "    )\n"
    ...
    output << ")\n"
           ...
           << "set(KATANA_GENERATED_COLD_AOT_SOURCES \"\")\n"
           << "set(KATANA_GENERATED_CENTRAL_DISPATCH_SOURCES \"\")\n"
           ...
           << "  endif()\n"
           << "  if(KATANA_GENERATED_SOURCE STREQUAL \"code/native-port-dispatch.cpp\")\n"
           << "    list(APPEND KATANA_GENERATED_CENTRAL_DISPATCH_SOURCES\n"
           << "      \"${KATANA_GENERATED_SOURCE}\")\n"
           << "  endif()\n"
           << "  if(KATANA_GENERATED_SOURCE MATCHES \"^code/unit-v[^/]+\\\\.cpp$\")\n"
           ...
           << "# Generated AOT units call across partitions only through the central\n"
           << "# dispatcher. Cold units therefore keep local /O2 but skip the expensive\n"
           << "# final ThinLTO backend; cross-unit inlining cannot cross that indirect\n"
           << "# boundary. The central dispatcher and declarative shards are likewise\n"
           << "# locally optimized. Only up to 64 explicitly measured hot AOT units\n"
           << "# retain IPO, independently of primary-image or overlay ownership.\n"
           ...
           << "    ${KATANA_GENERATED_CENTRAL_DISPATCH_SOURCES}\n"
           << "    ${KATANA_GENERATED_DECLARATIVE_SOURCES}\n"
           << "    ${KATANA_GENERATED_COLD_AOT_SOURCES})\n"
