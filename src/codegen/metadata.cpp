...
std::string_view provenance_name(const BlockProvenance value) {
    switch (value) {
    case BlockProvenance::ImageSegment: return "image-segment";
    case BlockProvenance::RomToRamCopy: return "rom-to-ram-copy";
    case BlockProvenance::FallbackDecode: return "fallback-decode";
    case BlockProvenance::RuntimeWrite: return "runtime-write";
    }
...
std::string serialize_block_metadata(
    const std::span<const BlockMetadata> blocks,
    ...
    for (std::size_t index = 0u; index < ordered.size(); ++index) {
        ...
        if (!semantic_identifier(block.source_segment) || block.byte_size == 0u ||
            ...
            throw std::invalid_argument("Blockmetadaten enthalten unvollstaendige Groessen oder Provenienz.");
        }
        ...
        std::sort(block.direct_successors.begin(), block.direct_successors.end(),
    ...
    output << "{\"schema_version\":" << block_metadata_schema_version
           ...
           << ",\"backend\":\"" << backend_name << "\",\"backend_abi\":"
           << backend_abi << ",\"blocks\":[";
    for (std::size_t index = 0u; index < ordered.size(); ++index) {
        ...
        output << "{\"virtual_address\":\"" << hex32(block.address.virtual_address)
               ...
               << ",\"provenance\":\"" << provenance_name(block.provenance)
               << "\",\"guest_opcodes\":[";
...
std::vector<ProjectArtifact> make_separated_codegen_artifacts(
