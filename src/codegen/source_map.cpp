...
bool portable_relative_path(const std::filesystem::path& path) {
    ...
    return std::none_of(
        path.begin(), path.end(), [](const auto& component) { return component == ".."; });
}
...
bool location_less(const AddressSourceLocation& left, const AddressSourceLocation& right) {
    ...
    for (const auto& unit : generated_units) {
        ...
        while (offset <= unit.content.size()) {
            ...
            if (found != std::string_view::npos) {
                ...
                const auto parsed =
                    std::from_chars(digits.data(), digits.data() + digits.size(), address, 16);
                ...
                if (*byte_offset >
                    std::numeric_limits<std::uint64_t>::max() - segment->file_offset) {
...
std::span<const AddressSourceLocation>
find_source_locations(const std::span<const AddressSourceLocation> locations,
                      const std::uint32_t guest_address) noexcept {
    const auto begin = std::lower_bound(locations.begin(),
                                        locations.end(),
                                        guest_address,
...
std::string serialize_address_source_map(const std::span<const AddressSourceLocation> locations) {
    ...
    output << ",\"source_map_version\":" << source_map_schema_version << ",\"locations\":[";
