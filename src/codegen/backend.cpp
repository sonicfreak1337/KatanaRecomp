...
BackendEmission generate_program(const Backend& backend, const BackendRequest& request) {
    ...
    if (backend.interface_abi_version() != request.requirements.interface_abi_version) {
        throw std::invalid_argument(
            "Codegen-Backend " + std::string(backend.name()) + " besitzt Interface-ABI " +
            std::to_string(backend.interface_abi_version()) + ", erforderlich ist " +
    ...
    if (backend.runtime_abi_version() != request.requirements.runtime_abi_version) {
        throw std::invalid_argument(
            "Codegen-Backend " + std::string(backend.name()) + " erwartet Runtime-ABI " +
            std::to_string(backend.runtime_abi_version()) + ", angefordert ist " +
            std::to_string(request.requirements.runtime_abi_version) + ".");
    }
    const auto missing_capabilities = request.requirements.capabilities & ~backend.capabilities();
    ...
    if (entry == request.functions.end()) {
        throw std::invalid_argument("Die Einstiegsfunktion ist im IR-Programm nicht vorhanden.");
    }
