...
int main() {
    ...
    require(cpu.r[3] == 1u, "CMP/EQ Register oder der erste BF-Pfad ist falsch.");

    require(cpu.r[4] == 7u,
            "Mindestens eine Statusoperation oder ein bedingter Sprung ist falsch.");

    require(cpu.t, "SETT muss das T-Bit am Programmende gesetzt haben.");

    require(cpu.pc == 0u, "RTS muss den initialen PR-Wert in PC uebernehmen.");

    require(cpu.r[0] == 5u, "Der CMP/EQ-Immediate-Test verlor den R0-Wert.");

    std::cout << "Generierte T-Bit- und Vergleichssemantik wurde erfolgreich ausgefuehrt.\n";

    std::cout << "r3 = " << std::dec << cpu.r[3] << '\n'
              << "r4 = " << cpu.r[4] << '\n'
              << "t  = " << (cpu.t ? 1 : 0) << '\n';
