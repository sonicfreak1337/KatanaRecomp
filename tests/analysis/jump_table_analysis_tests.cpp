...
int main() {
    ...
    require(
        !rejected.entries[2].accepted
            && rejected.entries[2].reason == "odd-address",
    ...
    boundaries.add_segment({
        ".code", 0x3000u, 0u, 8u, katana::io::SegmentKind::Code, {true, false, true},
        {0x09u, 0x00u, 0x0Bu, 0x00u}
    });
    boundaries.add_segment({
        ".data", 0x4000u, 4u, 28u, katana::io::SegmentKind::Data, {true, false, false},
        {
            0x00u, 0x30u, 0x00u, 0x00u,
            0x04u, 0x30u, 0x00u, 0x00u,
            0x02u, 0x30u, 0x00u, 0x00u,
            0x00u, 0x40u, 0x00u, 0x00u,
            0x01u, 0x30u, 0x00u, 0x00u,
    ...
    for (std::size_t index = 0u; index < expected_reasons.size(); ++index) {
        ...
        require(actual == expected_reasons[index], "Committed-Code-Grenzfall wurde falsch klassifiziert.");
    }

    boundaries.add_segment({
        ".top", 0xFFFFFFFCu, 32u, 4u, katana::io::SegmentKind::Data, {true, false, false},
        {0x00u, 0x30u, 0x00u, 0x00u}
    });
    const auto top_one = katana::analysis::analyze_jump_table(
        boundaries, 0x3000u, 0xFFFFFFFCu, 1u
    );
    require(top_one.resolved, "Ein Eintrag bei 0xFFFFFFFC wurde als arithmetischer Ueberlauf abgelehnt.");
    const auto top_two = katana::analysis::analyze_jump_table(
        boundaries, 0x3000u, 0xFFFFFFFCu, 2u
    );
    require(top_two.reason == "table-range-invalid", "Ueberlauf ueber den Adressraum wurde nicht abgelehnt.");
    const auto too_many = katana::analysis::analyze_jump_table(
        boundaries, 0x3000u, 0x4000u, 4097u
    );
    require(too_many.reason == "entry-count-out-of-range", "Maximalgrenze 4096 ist instabil.");
    const auto misaligned = katana::analysis::analyze_jump_table(
        boundaries, 0x3000u, 0x4002u, 1u
    );
    require(misaligned.reason == "table-range-invalid", "Unausgerichtete Tabellenbasis wurde akzeptiert.");

    boundaries.add_segment({
        ".sparse-table", 0x6000u, 36u, 8u, katana::io::SegmentKind::Data,
        {true, false, false}, {0x00u, 0x30u, 0x00u, 0x00u}
    });
    const auto sparse = katana::analysis::analyze_jump_table(
        boundaries, 0x3000u, 0x6000u, 2u
    );
    require(
        !sparse.resolved && sparse.entries.size() == 2u
            && sparse.entries[0].accepted
