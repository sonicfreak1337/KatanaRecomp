...
int main() {
    ...
    require(overrides.function_entries.size() == 2u, "Funktionshinweise fehlen.");
    require(
        overrides.function_entries[0] == 0x1000u && overrides.function_entries[1] == 0x1020u,
        "Funktionshinweise wurden nicht deterministisch sortiert."
    );
    require(
        overrides.jumps.size() == 1u && overrides.jumps[0].target == 0x1030u,
        "Sprungoverride wurde falsch gelesen."
    );
    require(
        overrides.jump_tables.size() == 1u && overrides.jump_tables[0].entry_count == 3u,
