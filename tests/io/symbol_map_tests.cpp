...
int main() {
    ...
    require(start != nullptr && start->address == 0x8C010000u && start->size == 4u, "Funktionssymbol ist falsch.");
    require(start->kind == SymbolKind::Function && start->binding == SymbolBinding::Global, "Symbolart oder Bindung ist falsch.");
    require(image.find_symbol("global_data")->kind == SymbolKind::Object, "Datensymbol ist falsch klassifiziert.");

    save(path, "8C010000 WRONG bad\n");
    ExecutableImage invalid;
    auto error = failure(path, invalid);
    require(error.find(path.string()) != std::string::npos && error.find("Zeile 1") != std::string::npos, "Map-Fehler nennt Datei und Zeile nicht.");

    save(path, "8C010000 FUNC same\n8C010004 FUNC same\n");
    ExecutableImage duplicate;
    error = failure(path, duplicate);
    require(error.find("Doppelter Symbolname") != std::string::npos && error.find("Zeile 2") != std::string::npos, "Doppeltes Symbol ist nicht diagnostisch.");
