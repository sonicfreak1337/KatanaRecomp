...
int main() {
    ...
    const auto fixture = std::filesystem::path(KATANA_SOURCE_DIR) /
    ...
    require(segment.memory_size == segment.bytes.size() && !segment.bytes.empty(), "Raw-Datei- und Speichergroesse stimmen nicht ueberein.");
    require(segment.kind == SegmentKind::Code, "Raw-Code wurde falsch klassifiziert.");
    require(segment.permissions.readable && segment.permissions.executable && !segment.permissions.writable, "Raw-Standardberechtigungen sind falsch.");
    require(image.entry_points().size() == 1u && image.entry_points()[0] == 0x8C010004u, "Der konfigurierte Einstiegspunkt fehlt.");

    const auto missing = std::filesystem::path(KATANA_SOURCE_DIR) / "tests" / "fixtures" / "does-not-exist.bin";
    const auto missing_error = require_failure(
        [&] { static_cast<void>(load_raw_binary(missing)); },
        "Eine fehlende Raw-Datei wurde akzeptiert."
    );
    require(missing_error.find(missing.string()) != std::string::npos, "Der Loaderfehler nennt die Quelldatei nicht.");

    const auto empty = std::filesystem::current_path() / "katana-empty-raw-fixture.bin";
    {
        std::ofstream output(empty, std::ios::binary);
    }
    const auto empty_error = require_failure(
        [&] { static_cast<void>(load_raw_binary(empty)); },
        "Eine leere Raw-Datei wurde akzeptiert."
    );
    std::filesystem::remove(empty);
    require(empty_error.find("Offset 0") != std::string::npos, "Der Leerdateifehler nennt den Dateioffset nicht.");

    RawBinaryLoadOptions wrap_options;
    wrap_options.base_address = 0xFFFFFFFFu;
    const auto wrap_error = require_failure(
        [&] { static_cast<void>(load_raw_binary(fixture, wrap_options)); },
        "Ein Raw-Image ausserhalb des Adressraums wurde akzeptiert."
    );
    require(wrap_error.find("32-Bit-Adressraum") != std::string::npos, "Der Adressraumfehler nennt seine Ursache nicht.");
