...
int main() {
    ...
    require(image.entry_points().size() == 1u, "Doppelte Einstiegspunkte wurden nicht normalisiert.");
    ...
    require(code->permissions.readable && code->permissions.executable && !code->permissions.writable, "Codeberechtigungen sind falsch.");
    require(code->file_offset == 0x40u && code->bytes.size() == 4u, "Dateilayout ging verloren.");
    require(code->byte_offset(0x8C010003u) == 3u, "Virtuelle Adresse wurde falsch in Dateidaten abgebildet.");
    require(!code->byte_offset(0x8C010004u).has_value(), "Zero-Fill-Bereich wurde faelschlich als Dateidaten gemeldet.");
    ...
    require_throws<std::invalid_argument>(
        [] { ExecutableImage invalid; invalid.add_segment({"", 0u, 0u, 1u, SegmentKind::Unknown, {}, {0u}}); },
        "Ein namenloses Segment wurde akzeptiert."
    );
    require_throws<std::invalid_argument>(
        [] { ExecutableImage invalid; invalid.add_segment({"short", 0u, 0u, 1u, SegmentKind::Data, {}, {1u, 2u}}); },
        "Dateidaten groesser als der Speicherbereich wurden akzeptiert."
    );
    require_throws<std::out_of_range>(
        [] { ExecutableImage invalid; invalid.add_segment({"wrap", 0xFFFFFFFFu, 0u, 2u, SegmentKind::Data, {}, {}}); },
