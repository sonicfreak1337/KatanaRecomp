...
int main() {
    ...
    require(duplicate_rejected,
        "Doppelte Funktionseinstiege erlauben mehrdeutige Ausgabe.");

    require(text.starts_with("katana-ir-v2\nfunction 0x8C010000\n"),
        "Textausgabe besitzt keinen stabilen Kopf oder keine Sortierung.");
    require(text.find("widths result=i32 input=none immediate=i8") !=
            std::string::npos &&
        text.find("delay role=owner counterpart=0x8C010002") !=
            std::string::npos &&
    ...
    require(json.ends_with("]}\n") &&
        ...
        json.find("\"delay_slot\":{\"role\":\"owner\"") !=
            std::string::npos,
