...
bool has_issue(const std::vector<katana::ir::VerificationIssue>& issues,
...
int main() {
    ...
    require(has_issue(katana::ir::verify_function(stale_calls), "Callee-Metadaten"),
            "Veraltete Call-Metadaten werden akzeptiert.");

    auto wrong_successor = program.front();
    wrong_successor.blocks.front().successors.push_back(wrong_successor.entry_address);
    require(has_issue(katana::ir::verify_function(wrong_successor), "Terminaloperation"),
            "CFG-Nachfolger entgegen der Terminaloperation werden akzeptiert.");

    auto missing_effective = program.front();
    auto& effective_instruction = missing_effective.blocks.front().instructions.front();
    effective_instruction.operation = katana::ir::Operation::LoadLongPcRelative;
    effective_instruction.original_operation = effective_instruction.operation;
    effective_instruction.widths =
        katana::ir::operation_operand_widths(effective_instruction.operation);
    effective_instruction.memory_effects =
        katana::ir::instruction_memory_effects(effective_instruction.operation);
    require(has_issue(katana::ir::verify_function(missing_effective), "effektive Adresse"),
    ...
    special_instruction.widths =
        katana::ir::operation_operand_widths(special_instruction.operation);
    special_instruction.memory_effects =
        katana::ir::instruction_memory_effects(special_instruction.operation);
    require(has_issue(katana::ir::verify_function(missing_special), "kein Spezialregister"),
    ...
    require(!katana::ir::verify_function(invalid_width).empty(),
            "Widerspruechliche Operandbreite wird nicht abgelehnt.");

    auto orphan_slot = program.front();
    orphan_slot.blocks.front().instructions.front().delay_slot = {katana::ir::DelaySlotRole::Slot,
                                                                  0x8C00FFFEu};
    require(!katana::ir::verify_function(orphan_slot).empty(),
    ...
    require(has_issue(katana::ir::verify_function(wrong_distance), "nicht gegenseitig verknuepft"),
    ...
    require(
        has_issue(katana::ir::verify_function(false_owner), "keinen verzoegerten Originalopcode"),
    ...
    require(has_issue(katana::ir::verify_function(missing_owner), "keine Owner-Rolle"),
    ...
    require(has_issue(katana::ir::verify_function(control_in_slot),
    ...
    require(katana::ir::verify_function(illegal_slot).empty(),
    ...
    try {
        ...
        static_cast<void>(
            katana::codegen::emit_cpp_program(invalid_program, invalid_width.entry_address));
    } catch (const std::invalid_argument& error) {
        codegen_rejected = std::string(error.what()).find("Operandbreiten") != std::string::npos;
    }
