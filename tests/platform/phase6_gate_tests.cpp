...
struct FixtureDirectory {
    std::filesystem::path path =
        std::filesystem::current_path() / "katana-phase6-gate-fixture";
    FixtureDirectory() {
...
std::vector<std::uint8_t> make_boot_track() {
    ...
    std::copy(hardware.begin(), hardware.end(), bytes.begin() +
        static_cast<std::ptrdiff_t>(payload_offset(0u, 0x00u)));
    std::copy(boot_file.begin(), boot_file.end(), bytes.begin() +
        static_cast<std::ptrdiff_t>(payload_offset(0u, 0x60u)));

    const auto pvd = payload_offset(16u);
    bytes[pvd] = 1u;
    std::copy_n("CD001", 5u, bytes.begin() + static_cast<std::ptrdiff_t>(pvd + 1u));
    bytes[pvd + 6u] = 1u;
    record(bytes, pvd + 156u, data_lba + 20u, payload_size,
        std::string(1u, '\0'), true);

    auto directory = payload_offset(20u);
    directory += record(bytes, directory, data_lba + 20u, payload_size,
        std::string(1u, '\0'), true);
    directory += record(bytes, directory, data_lba + 20u, payload_size,
...
int main() {
    ...
    require(
        disc.data_track_lba == data_lba && disc.extent_lba_bias == 0u &&
        ...
        "Dreamcast-GDI-Boot waehlt nicht den letzten Datentrack oder verliert absolute ISO-Extents."
    );
    const auto first = run_phase6_gate(
        fixture.path / "disc.gdi",
        execute_synthetic_block,
        1u
    );
    const auto second = run_phase6_gate(
        fixture.path / "disc.gdi",
        execute_synthetic_block,
        1u
    );
    ...
    require(
        first.checkpoint == "SA_PHASE6_MAIN_EXECUTION_STARTED" &&
    ...
    require(
        serialized == serialize_phase6_gate_report(second),
        "Zwei identische Phase-6-Laeufe liefern unterschiedliche Kernmetriken."
    );
    require(
        serialized.find(fixture.path.string()) == std::string::npos &&
