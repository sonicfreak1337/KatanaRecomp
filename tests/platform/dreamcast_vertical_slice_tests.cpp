...
int main() {
    ...
    input_frame.pressed_buttons =
        static_cast<std::uint16_t>(static_cast<std::uint16_t>(ControllerButton::A) |
                                   static_cast<std::uint16_t>(ControllerButton::Start));
    auto input = std::make_shared<ReplayInputBackend>(std::vector<ControllerState>{input_frame});
    ...
    DreamcastMediaClock clock(
        ...
        [&](const VideoTick&) {
            boot_observed =
                cpu.pc == boot.entry_point && cpu.memory.read_u16(boot.entry_point) == 0x0009u;
            const auto response =
                maple.exchange(0u, 0u, {MapleCommand::GetCondition, {0x01000000u}});
