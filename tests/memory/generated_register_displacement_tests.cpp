...
void verify_common_result(
    const katana_generated::CpuState& cpu
) {
    require(
        cpu.r[0] == 0xFFFF8001u,
        "Byte- oder Word-Displacement-Load hat R0 falsch gesetzt."
    );

    require(
        cpu.memory.read_u16(0x0000013Eu) == 0xC3D4u,
        "MOV.W Displacement-Store ist nicht Little Endian."
    );

    require(
        cpu.memory.read_u32(0x000001BCu) == 0x89ABCDEFu &&
        cpu.r[3] == 0x89ABCDEFu &&
        cpu.r[4] == 0x00000180u,
...
void run_normal_case() {
    ...
    require(
        cpu.memory.read_u8(0x0000010Fu) == 0xD4u &&
        cpu.r[1] == 0x00000100u &&
        cpu.r[2] == 0x00000120u,
...
int main() {
    ...
    std::cout
        << "KR-1402 End-to-End-Semantik wurde erfolgreich ausgefuehrt.\n";
