...
constexpr std::array<std::uint8_t, 12> fixture_bytes = {
    0x02, 0x91, // mov.w @(4, pc), r1 -> 0x108
    0x03, 0xD2, // mov.l @(12, pc), r2 -> 0x110 (PC aligned)
    0x09, 0x00, // nop
    0x04, 0xC7, // mova @(16, pc), r0 -> 0x118 (PC aligned)
    0x0B, 0x00, // rts
...
void append_synthetic_function(
...
const katana::ir::Instruction* find_source_instruction(
...
int main(const int argc, char* argv[]) {
    ...
    require(
        word.kind == InstructionKind::MovWordLoadPcRelative &&
        word.destination_register == 3u &&
        word.displacement == 510 &&
        word.text == "mov.w @(510, pc), r3",
        "PC-relativer MOV.W-Load wurde falsch dekodiert."
    );
    require(
        longword.kind == InstructionKind::MovLongLoadPcRelative &&
        longword.destination_register == 4u &&
        longword.displacement == 1020 &&
        longword.text == "mov.l @(1020, pc), r4",
        "PC-relativer MOV.L-Load wurde falsch dekodiert."
    );
    require(
        mova.kind == InstructionKind::MoveAddressPcRelative &&
        mova.destination_register == 0u &&
        mova.displacement == 1020 &&
    ...
    require(
        source.find("read_s16(0x00000108u)") != std::string::npos &&
        source.find("read_u32(0x00000110u)") != std::string::npos &&
        source.find("cpu.r[0] = 0x00000118u;") != std::string::npos,
        "Der Codegenerator verwendet falsche PC-relative Adressen."
    );

    std::cout
        << "Alle KR-1405 Decoder-, IR- und Codegen-Tests erfolgreich.\n";
    return EXIT_SUCCESS;
}
