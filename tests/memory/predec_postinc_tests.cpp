...
int main() {
    ...
    require(store_b.kind == InstructionKind::MovByteStorePreDecrement &&
    ...
    require(store_w.kind == InstructionKind::MovWordStorePreDecrement &&
    ...
    require(store_l.kind == InstructionKind::MovLongStorePreDecrement &&
    ...
    require(load_b.kind == InstructionKind::MovByteLoadPostIncrement &&
                load_b.source_register == 7u && load_b.destination_register == 8u &&
                load_b.text == "mov.b @r7+, r8",
            "MOV.B Post-Increment wurde falsch dekodiert.");

    require(load_w.kind == InstructionKind::MovWordLoadPostIncrement &&
                load_w.source_register == 9u && load_w.destination_register == 10u &&
                load_w.text == "mov.w @r9+, r10",
            "MOV.W Post-Increment wurde falsch dekodiert.");

    require(load_l.kind == InstructionKind::MovLongLoadPostIncrement &&
                load_l.source_register == 11u && load_l.destination_register == 12u &&
                load_l.text == "mov.l @r11+, r12",
            "MOV.L Post-Increment wurde falsch dekodiert.");

    require(same_store.source_register == same_store.destination_register &&
                same_store.text == "mov.b r13, @-r13",
            "Identischer Pre-Decrement-Registerfall wurde falsch dekodiert.");

    require(same_load.source_register == same_load.destination_register &&
                same_load.text == "mov.b @r14+, r14",
            "Identischer Post-Increment-Registerfall wurde falsch dekodiert.");

    constexpr std::array<std::uint8_t, 20> bytes = {0x14, 0x22, 0x35, 0x24, 0x56, 0x26, 0x74,
                                                    0x68, 0x95, 0x6A, 0xB6, 0x6C, 0xD4, 0x2D,
                                                    0xE4, 0x6E, 0x0B, 0x00, 0x09, 0x00};
    ...
    require(program.size() == 1u, "Der KR-1401-Unit-Test muss genau eine Funktion besitzen.");

    require(count_operation(program, Operation::StoreBytePreDecrement) == 2u,
            "StoreBytePreDecrement wurde nicht zweimal abgesenkt.");

    require(count_operation(program, Operation::StoreWordPreDecrement) == 1u &&
                count_operation(program, Operation::StoreLongPreDecrement) == 1u,
            "Word- oder Long-Pre-Decrement wurde falsch abgesenkt.");

    require(count_operation(program, Operation::LoadByteSignedPostIncrement) == 2u,
            "LoadByteSignedPostIncrement wurde nicht zweimal abgesenkt.");

    require(count_operation(program, Operation::LoadWordSignedPostIncrement) == 1u &&
                count_operation(program, Operation::LoadLongPostIncrement) == 1u,
    ...
    require(source.find("const std::uint32_t address = cpu.r[2] - 1u;") != std::string::npos,
            "Byte-Pre-Decrement berechnet die Adresse nicht vor dem Store.");

    require(source.find("const std::uint32_t value = cpu.r[13];") != std::string::npos,
            "Der identische Pre-Decrement-Fall sichert den alten Quellwert nicht.");

    require(source.find("const bool same_register = true;") != std::string::npos &&
                source.find("const bool same_register = false;") != std::string::npos,
