...
constexpr std::array<std::uint8_t, 16> fixture_bytes = {
    0x1F, 0x80, // mov.b r0, @(15, r1)
    0x2F, 0x81, // mov.w r0, @(30, r2)
    0x3F, 0x14, // mov.l r3, @(60, r4)
    0x5F, 0x84, // mov.b @(15, r5), r0
    0x6F, 0x85, // mov.w @(30, r6), r0
    0x7F, 0x58, // mov.l @(60, r7), r8
    0x0B, 0x00, // rts
...
const katana::ir::Instruction* find_operation(
...
int main(const int argc, char* argv[]) {
    ...
    require(
        argc == 1,
        "Unerwartete Argumente fuer den KR-1402-Test."
    );
    ...
    require(
        store_b.kind == InstructionKind::MovByteStoreDisplacement &&
        store_b.source_register == 0u &&
        store_b.destination_register == 1u &&
        store_b.displacement == 15 &&
        store_b.text == "mov.b r0, @(15, r1)",
        "MOV.B Displacement-Store wurde falsch dekodiert."
    );

    require(
        store_w.kind == InstructionKind::MovWordStoreDisplacement &&
        store_w.source_register == 0u &&
        store_w.destination_register == 2u &&
        store_w.displacement == 30 &&
        store_w.text == "mov.w r0, @(30, r2)",
        "MOV.W Displacement-Store wurde falsch skaliert."
    );

    require(
        store_l.kind == InstructionKind::MovLongStoreDisplacement &&
        store_l.source_register == 3u &&
        store_l.destination_register == 4u &&
        store_l.displacement == 60 &&
        store_l.text == "mov.l r3, @(60, r4)",
        "MOV.L Displacement-Store wurde falsch dekodiert."
    );

    require(
        load_b.kind == InstructionKind::MovByteLoadDisplacement &&
        load_b.source_register == 5u &&
        load_b.destination_register == 0u &&
        load_b.displacement == 15 &&
        load_b.text == "mov.b @(15, r5), r0",
        "MOV.B Displacement-Load wurde falsch dekodiert."
    );

    require(
        load_w.kind == InstructionKind::MovWordLoadDisplacement &&
        load_w.source_register == 6u &&
        load_w.destination_register == 0u &&
        load_w.displacement == 30 &&
        load_w.text == "mov.w @(30, r6), r0",
        "MOV.W Displacement-Load wurde falsch skaliert."
    );

    require(
        load_l.kind == InstructionKind::MovLongLoadDisplacement &&
        load_l.source_register == 7u &&
        load_l.destination_register == 8u &&
        load_l.displacement == 60 &&
        load_l.text == "mov.l @(60, r7), r8",
        "MOV.L Displacement-Load wurde falsch dekodiert."
    );

    const auto zero_displacement = katana::sh4::decode(0x8030u);
    require(
        zero_displacement.displacement == 0,
    ...
    require(
        program.size() == 1u,
        "Der KR-1402-Test muss genau eine Funktion erzeugen."
    );

    for (const auto operation : {
        Operation::StoreByteDisplacement,
        ...
        require(
            instruction != nullptr,
            "Eine Register-Displacement-Operation fehlt in der IR."
        );
    }
    ...
    require(
        source.find("cpu.r[1] + 15u") != std::string::npos &&
    ...
    require(
        source.find("read_s8(cpu.r[5] + 15u)") != std::string::npos &&
    ...
    std::cout
        << "Alle KR-1402 Decoder-, IR- und Codegen-Tests erfolgreich.\n";
