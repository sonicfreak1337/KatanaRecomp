...
int main() {
    ...
    require(opcode_rules_overlap(OpcodeRuleView{"breit", 0xF000u, 0x6000u},
                                 OpcodeRuleView{"eng", 0xF00Fu, 0x6003u}),
            "Eine echte Teilmengen-Kollision wurde nicht erkannt.");
    require(!opcode_rules_overlap(OpcodeRuleView{"mov", 0xF00Fu, 0x6003u},
    ...
    if (!collisions.empty()) {
        ...
        require(false,
                "Decoderregeln kollidieren: " + first.first_rule + " / " + first.second_rule + ".");
    }
