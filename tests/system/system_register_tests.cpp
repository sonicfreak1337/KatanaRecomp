...
katana::ir::Instruction transfer(
...
std::vector<katana::ir::Function> build_program() {
    ...
    constexpr std::array registers = {
        IrRegister::Mach, IrRegister::Macl, IrRegister::Pr,
        IrRegister::Fpul, IrRegister::Fpscr, IrRegister::Sr,
        IrRegister::Gbr, IrRegister::Vbr, IrRegister::Ssr,
        IrRegister::Spc, IrRegister::Dbr, IrRegister::Bank0,
        IrRegister::Bank1, IrRegister::Bank2, IrRegister::Bank3,
        IrRegister::Bank4, IrRegister::Bank5, IrRegister::Bank6,
        IrRegister::Bank7
    };
    ...
    append_function(
        program,
        0x230u,
        {transfer(0x230u, Operation::StoreSpecialRegister, IrRegister::Sgr, 1u)}
    );
    append_function(
        program,
        0x240u,
...
void test_decoder_matrix() {
    ...
    constexpr std::array system_offsets = {
        0x00u, 0x10u, 0x20u, 0x50u, 0x60u
    };

    for (const auto& family : system_families) {
        for (const auto offset : system_offsets) {
            const auto decoded = katana::sh4::decode(
                static_cast<std::uint16_t>(family.suffix + offset + 0x0F00u)
            );
            require(
                decoded.kind == family.kind && !decoded.is_privileged,
    ...
    constexpr std::array control_direct_store = {
        0x0002u, 0x0012u, 0x0022u, 0x0032u, 0x0042u,
        0x003Au, 0x00FAu, 0x0082u, 0x0092u, 0x00A2u,
        0x00B2u, 0x00C2u, 0x00D2u, 0x00E2u, 0x00F2u
    };
    constexpr std::array control_memory_store = {
        0x4003u, 0x4013u, 0x4023u, 0x4033u, 0x4043u,
        0x4032u, 0x40F2u, 0x4083u, 0x4093u, 0x40A3u,
        0x40B3u, 0x40C3u, 0x40D3u, 0x40E3u, 0x40F3u
    };
    constexpr std::array control_direct_load = {
        0x400Eu, 0x401Eu, 0x402Eu, 0x403Eu, 0x404Eu,
        0x40FAu, 0x408Eu, 0x409Eu, 0x40AEu, 0x40BEu,
        0x40CEu, 0x40DEu, 0x40EEu, 0x40FEu
    };
    constexpr std::array control_memory_load = {
        0x4007u, 0x4017u, 0x4027u, 0x4037u, 0x4047u,
        0x40F6u, 0x4087u, 0x4097u, 0x40A7u, 0x40B7u,
        0x40C7u, 0x40D7u, 0x40E7u, 0x40F7u
    };

    const auto check_control = [](const auto& opcodes, const Kind kind) {
        for (const auto opcode : opcodes) {
            const auto decoded = katana::sh4::decode(
                static_cast<std::uint16_t>(opcode | 0x0700u)
            );
            const bool is_gbr = (opcode & 0x00FFu) == 0x12u ||
                (opcode & 0x00FFu) == 0x13u ||
                (opcode & 0x00FFu) == 0x1Eu ||
                (opcode & 0x00FFu) == 0x17u;
            require(
                decoded.kind == kind && decoded.is_privileged != is_gbr,
    ...
    require(
        katana::sh4::decode(0x075Au).text == "sts fpul, r7" &&
...
void test_lowering() {
    constexpr std::array<std::uint8_t, 20> bytes = {
        0x0A, 0x40, 0x0A, 0x01, 0x22, 0x42, 0x26, 0x43,
        0x0E, 0x44, 0x02, 0x05, 0x13, 0x46, 0x17, 0x47,
        0x0B, 0x00, 0x09, 0x00
    };
    ...
    require(
        counts == std::array<std::size_t, 4>{2u, 2u, 2u, 2u} && privileged == 2u,
...
int main(const int argc, char* argv[]) {
    ...
    const auto source = katana::codegen::emit_cpp_program(build_program(), 0x100u);
    require(
        source.find("cpu.write_sr(value);") != std::string::npos &&
