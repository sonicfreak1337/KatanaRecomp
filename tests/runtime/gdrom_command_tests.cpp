...
int main() {
    ...
    require(capacity.status == GdRomStatus::Good &&
                capacity.data == std::vector<std::uint8_t>({0u, 0u, 0u, 2u, 0u, 0u, 8u, 0u}),
    ...
    require(read.status == GdRomStatus::Good && read.transferred_sectors == 1u &&
                read.data.size() == 2048u && read.data.front() == 0x5Au &&
                read.data.back() == 0x5Au,
    ...
    auto empty =
        std::make_shared<MemoryDiscSource>(std::span<const std::uint8_t>{}, "synthetic:empty");
