...
BlockExit block_a(CpuState&, BlockExecutionContext&) { return {}; }
BlockExit block_b(CpuState&, BlockExecutionContext&) { return {}; }
...
int main() {
    try {
        ...
        require(table.lookup(0x8C001000u, base)->function == block_a, "Deterministischer Lookup schlug fehl.");
        require(table.lookup(0xAC001000u, base)->runtime_registered, "Laufzeitprovenienz ging verloren.");
        ...
        require(throws_with([&] {
            table.register_runtime({0x8C001004u, 0x0C002000u, 8u, BlockEndKind::Call, base, block_b, "overlap", false});
        }, "rom-reset", "overlap"), "Ueberlappung nennt nicht beide Provenienzen.");

        auto other_variant = base;
        ++other_variant.mmu_generation;
        table.register_runtime({0x8C001000u, 0x0D001000u, 8u, BlockEndKind::Return, other_variant, block_b, "tlb-remap", false});
