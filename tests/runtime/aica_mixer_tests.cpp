...
int main() {
    ...
    const std::vector<AicaVoice> voices = {{center_samples, aica_unity_gain, aica_pan_center},
    ...
    const std::vector<AicaVoice> clipping = {{loud, aica_unity_gain, aica_pan_center},
    ...
    require(throws<std::invalid_argument>([&] {
                static_cast<void>(mixer.mix(
                    std::vector<AicaVoice>{{center_samples, aica_unity_gain + 1u, 0}}, 1u));
            }),
