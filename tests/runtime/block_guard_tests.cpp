...
int main() {
    try {
        ...
        require(first.physical_page == 0x0C002000u && !space.translate(0x1234u, TranslationAccess::Read).no_mmu_fastpath,
        ...
        require(remapped.physical_page == 0x0D004000u && remapped.mmu_generation != first.mmu_generation,
        ...
        try { static_cast<void>(space.translate(0x3000u, TranslationAccess::Instruction)); }
        catch (const TranslationError& error) {
            instruction_error = error.cause() == ExceptionCause::AddressErrorRead &&
                error.access() == TranslationAccess::Instruction && error.address() == 0x3000u;
        }
        require(instruction_error, "Ungueltige Instruktionsuebersetzung erzeugt keine strukturierte SH-4-Ausnahme.");
        ...
        require(!(single == double_precision), "FPSCR PR/FR/RM verwendet eine inkompatible Blockvariante wieder.");
