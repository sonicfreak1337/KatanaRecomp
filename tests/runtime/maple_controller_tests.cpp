...
int main() {
    ...
    const auto bit = [](const ControllerButton button) { return static_cast<std::uint16_t>(button); };
    std::vector<ControllerState> frames(2u);
    frames[0].pressed_buttons = static_cast<std::uint16_t>(bit(ControllerButton::A) | bit(ControllerButton::Start));
    ...
    require((second.payload[1] & bit(ControllerButton::DpadLeft)) == 0u && controller->sampled_frames() == 2u,
        "Deterministisches Replay schreitet nicht frameweise fort.");
    require(throws<std::out_of_range>([&] { static_cast<void>(bus.exchange(0u, 0u, {MapleCommand::GetCondition, {}})); }),
