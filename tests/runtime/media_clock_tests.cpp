...
int main() {
    ...
    require(completed.status == SchedulerAdvanceStatus::ReachedTarget &&
                clock.video_tick_count() == 6u && clock.audio_tick_count() == 3u &&
                clock.emitted_audio_frames() == 12u && video_backend.submitted_frames() == 6u &&
                audio_backend.submitted_buffers() == 3u && audio_backend.submitted_frames() == 12u,
    ...
    require(clock.video_tick_count() == 6u && clock.audio_tick_count() == 3u &&
    ...
    clock.reset();
    require(!clock.running() && clock.video_tick_count() == 0u && clock.audio_tick_count() == 0u &&
                clock.emitted_audio_frames() == 0u,
    ...
    DreamcastMediaClock fractional(
        fractional_scheduler, MediaClockConfig{10u, 3u, 1u, 100u}, [&](const VideoTick& tick) {
            fractional_video_cycles.push_back(tick.guest_cycle);
        });
    fractional.start();
    static_cast<void>(fractional_scheduler.advance_to(10u, 3u));
    require(fractional_video_cycles == std::vector<std::uint64_t>{3u, 6u, 10u},
            "Rationale Frame-Kadenz akkumuliert Rundungsdrift.");

    EventScheduler failing_scheduler;
    DreamcastMediaClock failing(
        failing_scheduler, MediaClockConfig{60u, 60u, 60u, 1u}, [](const VideoTick&) {
            throw std::runtime_error("sichtbarer Video-Callbackfehler");
        });
    failing.start();
    ...
    require(throws<std::invalid_argument>([&] {
                ...
                throws<std::invalid_argument>([&] {
                    DreamcastMediaClock invalid(scheduler,
                                                MediaClockConfig{1u, 60u, 44'100u, 735u});
                }) &&
                throws<std::invalid_argument>([&] {
    ...
    DreamcastMediaClock callback_stop(callback_stop_scheduler,
    ...
    require(!callback_stop.running() && callback_stop.video_tick_count() == 1u &&
    ...
    DreamcastMediaClock callback_restart(
        callback_restart_scheduler, MediaClockConfig{60u, 60u, 1u, 100u}, [&](const VideoTick&) {
            ++restart_callbacks;
    ...
    require(callback_restart.running() && callback_restart_scheduler.pending_event_count() == 2u &&
