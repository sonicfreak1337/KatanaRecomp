...
int main() {
    try {
        ...
        bool aborted = false;
        try {
            static_cast<void>(aborting.enter(cpu, context, opcode));
        } catch (const ControlledFallbackError& error) {
            const std::string text = error.what();
            aborted = text.find("8c001000") != std::string::npos &&
                      text.find("ffff") != std::string::npos;
        }
        ...
        const auto stopped = diagnose.enter(cpu,
                                            context,
                                            {FallbackReason::UnresolvedControlFlow,
                                             0x8C002000u,
                                             std::nullopt,
                                             0x8C00F000u,
                                             0x8C002000u});
        ...
        ControlledFallback interpreter(FallbackPolicy::Interpreter,
                                       [](CpuState& state,
                                          BlockExecutionContext& boundary,
        ...
        ControlledFallback wrong_boundary(
            FallbackPolicy::UserHook,
            [](CpuState&, BlockExecutionContext&, const ControlledFallbackRequest& request) {
                return request.block_boundary + 2u;
            });
        bool rejected = false;
        try {
            static_cast<void>(wrong_boundary.enter(cpu, context, opcode));
