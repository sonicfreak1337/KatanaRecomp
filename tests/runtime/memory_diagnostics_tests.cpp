...
int main() {
    ...
    require(
        strict_bus.alignment_policy() == MemoryAlignmentPolicy::Strict,
        "Der Speicherbus verwendet nicht standardmaessig strikte Ausrichtung."
    );
    require(
        has_memory_error(
            [&strict_bus] {
                static_cast<void>(strict_bus.read_u16(0x00001001u));
            },
    ...
    require(
        has_memory_error(
            [&strict_bus] {
                strict_bus.write_u32(0x00001002u, 0xFFFFFFFFu);
            },
            MemoryAccessErrorReason::Misaligned,
    ...
    Memory permissive_bus(
        8u,
        MemoryAlignmentPolicy::Permissive
    );
    permissive_bus.write_u32(1u, 0x89ABCDEFu);
    require(
        permissive_bus.read_u32(1u) == 0x89ABCDEFu &&
        permissive_bus.read_u8(1u) == 0xEFu &&
    ...
    require(
        has_memory_error(
            [&permissive_bus] {
                static_cast<void>(permissive_bus.read_u32(1u));
            },
            MemoryAccessErrorReason::Misaligned,
            MemoryAccessOperation::Read,
            1u,
    ...
    require(
        has_memory_error(
            [&strict_bus] {
                static_cast<void>(strict_bus.read_u8(0x00003000u));
            },
    ...
    cross_region_bus.map_region(
        "short-region",
        0x00002000u,
        std::make_shared<LinearMemoryDevice>(6u)
    );
    require(
        has_memory_error(
            [&cross_region_bus] {
                static_cast<void>(
                    cross_region_bus.read_u32(0x00002004u)
                );
            },
    ...
    require(
        has_memory_error(
            [&read_only_bus] {
                read_only_bus.write_u16(0x00004000u, 0x1234u);
            },
    ...
    require(
        has_memory_error(
            [&overflow_bus] {
                static_cast<void>(
                    overflow_bus.read_u16(0xFFFFFFFFu)
                );
            },
    ...
    observed_bus.set_trace_handler(
        [&trace_events](const MemoryAccessEvent& event) {
            trace_events.push_back(event);
        }
    );
    require(
        observed_bus.has_trace_handler(),
        "Der globale Trace-Handler wird nicht als aktiv gemeldet."
    );

    const auto read_watchpoint = observed_bus.add_watchpoint(
        0x00005002u,
        4u,
    ...
    const auto write_watchpoint = observed_bus.add_watchpoint(
        0x00006002u,
        2u,
    ...
    require(
        ...
        trace_events[0].region_name == "observed-primary" &&
        read_watch_events.empty() &&
        write_watch_events.empty(),
        "Trace oder zugriffsgefilterte Watchpoints melden einen Write falsch."
    );

    require(
        observed_bus.read_u32(0x00005000u) == 0x11223344u,
        "Der beobachtete Read liefert einen falschen Wert."
    );
    require(
        trace_events.size() == 2u &&
        read_watch_events.size() == 1u &&
    ...
    require(
        trace_events.size() == 3u &&
        write_watch_events.size() == 1u &&
        ...
        observed_ram->read_u8(2u) == 0xEFu &&
        observed_ram->read_u8(3u) == 0xBEu,
    ...
    require(
        has_memory_error(
            [&observed_bus] {
                static_cast<void>(
                    observed_bus.read_u16(0x00005001u)
                );
            },
            MemoryAccessErrorReason::Misaligned,
            MemoryAccessOperation::Read,
    ...
    require(
        read_watch_events.size() == 1u,
        "Ein entfernter Watchpoint wird weiterhin aufgerufen."
    );

    observed_bus.clear_watchpoints();
    observed_bus.clear_trace_handler();
    require(
        observed_bus.watchpoint_count() == 0u &&
        !observed_bus.has_trace_handler(),
    ...
    require(
        throws<std::invalid_argument>([&observed_bus] {
            static_cast<void>(observed_bus.add_watchpoint(
                0x00005000u,
                0u,
    ...
    std::cout
        << "Ausrichtung, Speicherfehler und Watchpoints erfolgreich.\n";
    return EXIT_SUCCESS;
}