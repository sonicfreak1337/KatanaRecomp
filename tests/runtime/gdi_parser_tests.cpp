...
int main() {
    ...
    require(parsed.descriptor_name == "synthetic.gdi" && parsed.tracks.size() == 2u &&
        parsed.tracks[0].type == GdiTrackType::Audio && parsed.tracks[0].sector_count == 2u &&
        parsed.tracks[0].descriptor_line == 2u && parsed.tracks[1].lba == 450u &&
        parsed.tracks[1].sector_count == 3u && parsed.tracks[1].resolved_path.parent_path() == fixture.path,
