...
namespace {
BlockExit host_block(CpuState&, BlockExecutionContext&) { return {}; }
...
int main() {
    try {
        ...
        for (const auto address : {0x8C001000u, 0x8C002000u, 0x8C003000u}) {
            table.register_static({address, canonical_physical_address(address), 4u,
                BlockEndKind::Return, variant, host_block, "block-" + std::to_string(address), false});
        }
        ...
        require(dispatcher.dispatch(cpu, context, variant,
            {BlockEndKind::Fallthrough, source, {}, fallthrough}).target_block != nullptr,
            "Fallthrough besitzt keinen Ausfuehrungspfad.");
        require(dispatcher.dispatch(cpu, context, variant,
            {BlockEndKind::StaticBranch, source, {taken}, std::nullopt}).exit.target == taken,
            "Statischer Block waehlt sein Ziel nicht.");
        require(dispatcher.dispatch(cpu, context, variant,
            {BlockEndKind::ConditionalBranch, source, {taken}, fallthrough}, true).exit.target == taken &&
            dispatcher.dispatch(cpu, context, variant,
            {BlockEndKind::ConditionalBranch, source, {taken}, fallthrough}, false).exit.target == fallthrough,
            "Bedingter Block waehlt nicht exakt einen dokumentierten Nachfolger.");

        require(dispatcher.dispatch(cpu, context, variant,
            {BlockEndKind::DynamicBranch, source, {}, std::nullopt}, false, 0x8C003000u).target_block != nullptr,
            "Dynamischer Sprung besitzt keinen generischen Lookup.");
        static_cast<void>(dispatcher.dispatch(cpu, context, variant,
            {BlockEndKind::Call, source, {}, std::nullopt}, false, 0x8C003000u));
        require(cpu.pr == source.virtual_address + 4u, "Dynamischer Call setzt PR falsch.");
        cpu.pr = 0x8C002000u;
        const auto returned = dispatcher.dispatch(cpu, context, variant,
            {BlockEndKind::Return, source, {}, std::nullopt});
        ...
        require(!dispatcher.dispatch(cpu, context, variant,
            {BlockEndKind::Exception, source, {}, std::nullopt}).target_block &&
            !dispatcher.dispatch(cpu, context, variant,
            {BlockEndKind::InterruptSafepoint, source, {}, std::nullopt}).target_block,
