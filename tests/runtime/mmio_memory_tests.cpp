...
struct Access {
    ...
    katana::runtime::MemoryAccessWidth width = katana::runtime::MemoryAccessWidth::Byte;
};
...
int main() {
    ...
    const auto read_handler = [&registers, &accesses](const std::uint32_t offset,
                                                      const MemoryAccessWidth width) {
        const auto count = width_bytes(width);
        std::uint32_t value = 0u;
        for (std::size_t index = 0u; index < count; ++index) {
            value |= static_cast<std::uint32_t>(registers[offset + index]) << (index * 8u);
        }
        accesses.push_back(Access{false, offset, value, width});
        return value;
    };

    const auto write_handler = [&registers, &accesses](const std::uint32_t offset,
                                                       const std::uint32_t value,
                                                       const MemoryAccessWidth width) {
        accesses.push_back(Access{true, offset, value, width});
        const auto count = width_bytes(width);
        for (std::size_t index = 0u; index < count; ++index) {
            registers[offset + index] = static_cast<std::uint8_t>((value >> (index * 8u)) & 0xFFu);
        }
    };

    auto mmio = std::make_shared<MmioMemoryDevice>(registers.size(), read_handler, write_handler);
    ...
    require(accesses.size() == 3u && accesses[0].write && accesses[0].offset == 1u &&
                accesses[0].value == 0xA5u && accesses[0].width == MemoryAccessWidth::Byte &&
                accesses[1].write && accesses[1].offset == 2u && accesses[1].value == 0xBEEFu &&
                accesses[1].width == MemoryAccessWidth::Halfword && accesses[2].write &&
                accesses[2].offset == 4u && accesses[2].value == 0x89ABCDEFu &&
                accesses[2].width == MemoryAccessWidth::Word,
            "MMIO-Schreibzugriffe erreichen den Handler nicht exakt einmal mit Breite.");

    accesses.clear();
    require(bus.read_u8(0x205F0001u) == 0xA5u && bus.read_u16(0x205F0002u) == 0xBEEFu &&
                bus.read_u32(0x205F0004u) == 0x89ABCDEFu,
            "MMIO-Lesehandler liefert ueber Aliase falsche Werte.");
    require(accesses.size() == 3u && !accesses[0].write && accesses[0].offset == 1u &&
                accesses[0].width == MemoryAccessWidth::Byte && !accesses[1].write &&
                accesses[1].offset == 2u && accesses[1].width == MemoryAccessWidth::Halfword &&
                !accesses[2].write && accesses[2].offset == 4u &&
                accesses[2].width == MemoryAccessWidth::Word,
    ...
    registers[10u] = 0xBBu;
    registers[11u] = 0xAAu;
    require(mmio->read_u8(8u) == 0xDDu && mmio->read_u16(8u) == 0xCCDDu &&
    ...
    require(byte_only->byte_writes == 4u && bus.read_u32(0x00001000u) == 0x11223344u,
    ...
    auto read_only_mmio = std::make_shared<MmioMemoryDevice>(
        4u,
        [](const std::uint32_t, const MemoryAccessWidth) { return 0x12345678u; },
        [&forbidden_writes](const std::uint32_t, const std::uint32_t, const MemoryAccessWidth) {
            ++forbidden_writes;
        });
    bus.map_region("read-only-mmio", 0x00002000u, read_only_mmio, MemoryRegionAccess::ReadOnly);
    require(throws<std::runtime_error>([&bus] { bus.write_u32(0x00002000u, 0xFFFFFFFFu); }) &&
                forbidden_writes == 0u,
    ...
    require(throws<std::runtime_error>([&bus] { static_cast<void>(bus.read_u8(0x00003000u)); }),
    ...
    bus.map_region("handler-read-only-mmio", 0x00004000u, read_only_handler_mmio);
    require(throws<std::runtime_error>([&bus] { bus.write_u8(0x00004000u, 1u); }),
            "Ein fehlender MMIO-Schreibhandler wird nicht sichtbar gemeldet.");

    const auto callback_count_before_error = accesses.size();
    require(throws<std::out_of_range>([&mmio] { static_cast<void>(mmio->read_u32(30u)); }) &&
    ...
    require(throws<std::invalid_argument>([] {
                static_cast<void>(MmioMemoryDevice(
                    4u, katana::runtime::MmioReadHandler{}, katana::runtime::MmioWriteHandler{}));
            }),
