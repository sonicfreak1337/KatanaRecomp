...
int main() {
    try {
        ...
        CpuState cpu;
        static_cast<void>(cpu.memory.add_watchpoint(0x20u, 1u, MemoryWatchpointAccess::Write,
            [&](const auto&) { watchpoint = true; }));
        PreciseInterpreterBoundary boundary(safepoints,
        ...
        cpu.vbr = 0x8C000000u;
        const auto fault = boundary.execute(cpu, {
            "memory-fault", 0x8C002002u, 2u, 0x8C002004u, 1u, 0x8C002000u
        });
        require(fault.exception && cpu.last_exception_cause == ExceptionCause::BusErrorRead &&
        ...
        cpu.last_exception_cause = ExceptionCause::None;
        const auto illegal = boundary.execute(cpu, {
            "unknown-opcode", 0x8C003002u, 0xFFFFu, 0x8C003004u, 1u, 0x8C003000u
        });
        require(illegal.exception && cpu.last_exception_cause == ExceptionCause::SlotIllegalInstruction &&
        ...
        try {
            static_cast<void>(boundary.execute(cpu, {
                "manifest-denied", 0x8C004000u, 1u, 0x8C004002u, 1u, std::nullopt, false
            }));
        } catch (const std::runtime_error&) { forbidden = true; }
