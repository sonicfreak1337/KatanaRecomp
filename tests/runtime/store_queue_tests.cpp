...
int main() {
    try {
        ...
        for (std::uint32_t offset = 0u; offset < 32u; offset += 4u) {
            queues.write_p4(0xE0000000u + offset, 0x03020100u + offset * 0x01010101u, MemoryAccessWidth::Word);
            queues.write_p4(0xE2000000u + offset, 0x83828180u + offset * 0x01010101u, MemoryAccessWidth::Word);
        }
        require(queues.prefetch(0xE0001000u) && queues.prefetch(0xE2002000u) && transfers.size() == 2u,
        ...
        require(transfers[0].queue == 0u && transfers[0].target_address == 0x0C001000u &&
                transfers[0].bytes == queues.queue(0u) &&
                transfers[1].queue == 1u && transfers[1].target_address == 0x12002000u &&
        ...
        memory.map_region("ta", 0x12000000u, std::make_shared<MmioMemoryDevice>(
            0x10000u,
            [](std::uint32_t, MemoryAccessWidth) { return 0u; },
            [&](std::uint32_t offset, std::uint32_t, MemoryAccessWidth width) {
                require(width == MemoryAccessWidth::Byte, "TA-SQ-Transfer umgeht den MMIO-Breitenvertrag.");
        ...
        bool rejected = false;
        try { queues.set_operand_cache_ram_enabled(true); }
        catch (const std::runtime_error&) { rejected = true; }
        require(rejected, "Nicht modellierter CCR-Operand-Cache-RAM-LLE-Pfad wird nicht sichtbar abgelehnt.");
