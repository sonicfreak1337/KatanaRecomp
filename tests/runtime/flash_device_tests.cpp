...
void program(katana::runtime::Memory& bus, const std::uint32_t address, const std::uint8_t value) {
    unlock(bus); bus.write_u8(0x00200000u + katana::runtime::dreamcast_flash_unlock_address_1, 0xA0u); bus.write_u8(address, value);
}
}
...
int main() {
    ...
    require(bus.read_u8(0x80201234u) == 0x50u && source[0x1234u] == 0xF0u && flash->source_byte(0x1234u) == 0xF0u,
    ...
    unlock(bus); bus.write_u8(0x00200000u + dreamcast_flash_unlock_address_1, 0x80u); unlock(bus); bus.write_u8(0x00201234u, 0x30u);
    ...
    unlock(bus);
    require(throws<std::runtime_error>([&] { bus.write_u8(0x00200000u + dreamcast_flash_unlock_address_1, 0x90u); }),
    ...
    require(bus.read_u8(0x00200020u) == 0x7Fu, "Reset nach Fehler stellt den Lesemodus nicht wieder her.");
    program(bus, 0x00200021u, 0xF0u);
    require(bus.read_u8(0x00200021u) == 0xF0u, "Programmdaten 0xF0 werden faelschlich als Reset interpretiert.");
