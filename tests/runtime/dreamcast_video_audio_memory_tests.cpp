...
std::uint32_t expected_vram_32bit_linear_offset(
    const std::uint32_t offset
) {
    constexpr std::uint32_t bytes_per_word = 4u;

    const auto bank =
        offset < katana::runtime::dreamcast_vram_bank_size ? 0u : 1u;
    const auto bank_offset =
        offset - bank * katana::runtime::dreamcast_vram_bank_size;
...
[[noreturn]] void fail_vram_offset(
...
void verify_vram_offset_mapping_exhaustively() {
    ...
    for (
        ...
        raw_offset < katana::runtime::dreamcast_vram_size;
        ++raw_offset
    ) {
        const auto offset = static_cast<std::uint32_t>(raw_offset);
        const auto actual =
            katana::runtime::dreamcast_vram_32bit_to_linear_offset(
                offset
            );
        const auto expected =
            expected_vram_32bit_linear_offset(offset);

        if (
            actual != expected ||
            actual >= katana::runtime::dreamcast_vram_size
        ) {
            fail_vram_offset(
                "Die exhaustive 32-Bit-VRAM-Abbildung ist falsch.",
                offset,
                actual,
                expected
            );
        ...
        if (seen[actual] != 0u) {
            fail_vram_offset(
                "Die 32-Bit-VRAM-Abbildung ist nicht bijektiv.",
                offset,
                actual,
                expected
            );
...
void verify_vram_bus_boundaries(
    katana::runtime::Memory& bus,
        ...
        for (const auto word_index : word_indices) {
            for (std::uint32_t byte_index = 0u; byte_index < 4u; ++byte_index) {
                const auto path_offset =
                    bank * katana::runtime::dreamcast_vram_bank_size +
                    word_index * 4u +
                    byte_index;
                const auto linear_offset =
                    expected_vram_32bit_linear_offset(path_offset);
                ...
                if (vram->read_u8(linear_offset) != value) {
                    fail_vram_offset(
                        "Byteposition oder Bank des 32-Bit-VRAM-Pfads ist falsch.",
                        path_offset,
    ...
    require(
        vram->read_u16(
            expected_vram_32bit_linear_offset(0x003FFFFEu)
        ) == 0xA1B2u,
    ...
    require(
        vram->read_u16(
            expected_vram_32bit_linear_offset(0x00400000u)
        ) == 0xC3D4u,
    ...
    require(
        vram->read_u16(
            expected_vram_32bit_linear_offset(0x00400002u)
        ) == 0xE5F6u,
    ...
    require(
        vram->read_u32(
            expected_vram_32bit_linear_offset(0x003FFFFCu)
        ) == 0x11223344u,
        ...
        for (const auto sequence_start : sequence_starts) {
            for (std::uint32_t index = 0u; index < 16u; ++index) {
                ...
                require(
                    ...
                    "Fortlaufende 32-Bit-Woerter unterscheiden sich zwischen interleavter und linearer VRAM-Sicht."
                );
            }
        }
    }

    constexpr std::array<std::uint32_t, 12> alias_offsets = {
        0x00000000u,
        0x00000001u,
        0x00000002u,
        0x00000003u,
    ...
    for (const auto path_offset : alias_offsets) {
        const auto value = static_cast<std::uint8_t>(
            (path_offset ^ (path_offset >> 8u) ^ 0xA5u) & 0xFFu
        );
        bus.write_u8(path_32bit_base + path_offset, value);

        for (
            const auto segment_base :
            katana::runtime::dreamcast_direct_segment_bases
        ) {
            for (
                ...
                require(
                    bus.read_u8(
                        direct_alias(segment_base, physical_base) +
                        path_offset
                    ) == value,
