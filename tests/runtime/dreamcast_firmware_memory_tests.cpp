...
int main() {
    ...
    require(
        bios->size() == dreamcast_bios_size &&
        flash->size() == dreamcast_flash_size,
        "BIOS oder Flash besitzt nicht die erwartete Groesse."
    );
    require(
        dreamcast_bios_alias_count == 7u &&
        dreamcast_flash_alias_count == 7u &&
        bus.region_count() == 14u,
        "BIOS- oder Flash-Aliaszahl stimmt nicht."
    );

    for (const auto segment_base : dreamcast_direct_segment_bases) {
        const auto bios_base = direct_alias(
            segment_base,
            dreamcast_bios_physical_base
        );
        const auto flash_base = direct_alias(
            segment_base,
            dreamcast_flash_physical_base
        );

        require(
            bus.contains(bios_base, dreamcast_bios_size),
            "Ein direkter BIOS-Alias fehlt."
        );
        require(
            bus.contains(flash_base, dreamcast_flash_size),
            "Ein direkter Flash-Alias fehlt."
        );
        require(
            bus.read_u8(bios_base) == 0x12u &&
            bus.read_u8(bios_base + 0x00012345u) == 0x34u &&
            bus.read_u8(
                bios_base +
                static_cast<std::uint32_t>(dreamcast_bios_size - 1u)
            ) == 0x56u,
    ...
    require(
        bus.read_u32(0x60200100u) == 0x89ABCDEFu &&
        flash->read_u8(0x00000100u) == 0xEFu &&
    ...
    for (std::size_t index = 0u; index < bus.region_count(); ++index) {
        ...
        if (region.name.rfind("dreamcast-bios-", 0u) == 0u) {
            saw_bios = true;
            require(
                region.access == MemoryRegionAccess::ReadOnly,
        ...
        if (region.name.rfind("dreamcast-flash-", 0u) == 0u) {
            saw_flash = true;
            require(
                region.access == MemoryRegionAccess::ReadWrite,
    ...
    require(
        loaded_flash_bus.read_u8(0x00200000u) == 0xA1u &&
    ...
    require(
        throws<std::invalid_argument>([&invalid_bios_bus, &short_bios] {
            static_cast<void>(
                map_dreamcast_bios(invalid_bios_bus, short_bios)
            );
        }) &&
    ...
    require(
        throws<std::invalid_argument>([&invalid_flash_bus, &short_flash] {
            static_cast<void>(
                map_dreamcast_flash(invalid_flash_bus, short_flash)
            );
        }) &&
    ...
    require(
        throws<std::invalid_argument>([&conflicting_bios_bus] {
    ...
    require(
        throws<std::invalid_argument>([&conflicting_flash_bus] {
    ...
    require(
        full_bus.region_count() ==
            ...
            dreamcast_bios_alias_count +
            dreamcast_flash_alias_count,
