...
katana::runtime::BlockExit backend_a(katana::runtime::CpuState& cpu,
    ...
    return katana::runtime::make_block_exit(cpu,
                                            context,
...
void backend_b(const katana::runtime::CpuState& cpu,
               const katana::runtime::BlockExecutionContext& context) {
    require(cpu.r[0] == 0x01020304u && cpu.r_bank[7] == 0x11223344u && cpu.fr[3] == 0x7FC00000u &&
                cpu.xf[15] == 0x3F800000u && cpu.pr == 0x8C001234u &&
                cpu.read_sr() == 0x600000F1u && cpu.read_fpscr() == 0x002C0001u &&
                cpu.ssr == 0x400000F0u && cpu.spc == 0x8C002000u && cpu.sleeping &&
                cpu.trap_pending &&
                cpu.last_exception_cause == katana::runtime::ExceptionCause::AddressErrorRead &&
...
int main() {
    ...
    require(first_exit.source.virtual_address != first_exit.source.physical_address &&
    ...
    constexpr std::array target_kinds = {BlockEndKind::Fallthrough,
                                         BlockEndKind::StaticBranch,
    ...
    constexpr std::array terminal_kinds = {BlockEndKind::Exception,
    ...
    require(
        throws_invalid([&] { validate_block_entry(cpu, context, entry); }) && throws_invalid([&] {
            static_cast<void>(make_block_exit(cpu, context, BlockEndKind::Call, entry.address));
        }),
