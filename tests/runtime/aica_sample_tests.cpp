...
int main() {
    ...
    require(pcm16.decode(std::vector<std::uint8_t>{0x34u, 0x12u, 0x00u, 0x80u}, 2u) ==
                std::vector<std::int16_t>({0x1234, -32768}),
            "PCM16 ist nicht signed little-endian.");
    AicaSampleDecoder pcm8(AicaSampleFormat::Pcm8);
    require(pcm8.decode(std::vector<std::uint8_t>{0x7Fu, 0x80u, 0xFFu}, 3u) ==
                std::vector<std::int16_t>({32512, -32768, -256}),
            "PCM8 wird nicht korrekt vorzeichenerweitert.");
    ...
    require(first == std::vector<std::int16_t>({15, 253}) && adpcm.predictor() == 253 &&
                adpcm.step() == 304,
