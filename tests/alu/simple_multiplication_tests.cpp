...
int main() {
    ...
    require(mul_l.kind == InstructionKind::MultiplyLong && mul_l.source_register == 1u &&
                mul_l.destination_register == 2u && mul_l.text == "mul.l r1, r2",
            "MUL.L wurde falsch dekodiert.");

    require(muls_w.kind == InstructionKind::MultiplySignedWord && muls_w.source_register == 3u &&
                muls_w.destination_register == 4u && muls_w.text == "muls.w r3, r4",
            "MULS.W wurde falsch dekodiert.");

    require(mulu_w.kind == InstructionKind::MultiplyUnsignedWord && mulu_w.source_register == 5u &&
                mulu_w.destination_register == 6u && mulu_w.text == "mulu.w r5, r6",
    ...
    constexpr std::array<std::uint8_t, 38> bytes = {
        ...
        0x00, 0x09, 0x00, 0x17, 0x02, 0x0B, 0x00, 0x09, 0x00, 0x09, 0x00, 0x3F, 0x24,
        0x0B, 0x00, 0x09, 0x00, 0x09, 0x00, 0x5E, 0x26, 0x0B, 0x00, 0x09, 0x00};
    ...
    require(count_operation(program, Operation::MultiplyLong) == 1u,
            "MUL.L wurde nicht genau einmal in IR abgesenkt.");

    require(count_operation(program, Operation::MultiplySignedWord) == 1u,
            "MULS.W wurde nicht genau einmal in IR abgesenkt.");

    require(count_operation(program, Operation::MultiplyUnsignedWord) == 1u,
    ...
    require(source.find("const std::uint64_t product =") != std::string::npos,
            "MUL.L fehlt im generierten C++.");

    require(source.find("static_cast<std::int32_t>(source_raw) - 0x00010000") != std::string::npos,
            "MULS.W besitzt keine portable Vorzeichenerweiterung.");

    require(source.find("& 0x0000FFFFu) *") != std::string::npos,
    ...
    std::cout << "Alle KR-1301 Decoder-, IR- und Codegen-Tests erfolgreich.\n";
