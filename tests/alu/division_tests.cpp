...
int main() {
    ...
    require(
        div0u.kind == InstructionKind::DivideInitializeUnsigned &&
        div0u.text == "div0u",
        "DIV0U wurde falsch dekodiert."
    );

    require(
        div0s.kind == InstructionKind::DivideInitializeSigned &&
        div0s.source_register == 3u &&
        div0s.destination_register == 4u &&
        div0s.text == "div0s r3, r4",
        "DIV0S wurde falsch dekodiert."
    );

    require(
        div1.kind == InstructionKind::DivideStep &&
        div1.source_register == 5u &&
        div1.destination_register == 6u &&
        div1.text == "div1 r5, r6",
    ...
    require(
        count_operation(
            program,
            Operation::DivideInitializeUnsigned
        ) == 1u,
    ...
    require(
        count_operation(
            program,
            Operation::DivideInitializeSigned
        ) == 1u,
        "DIV0S wurde nicht genau einmal in IR abgesenkt."
    );

    require(
        count_operation(program, Operation::DivideStep) == 1u,
