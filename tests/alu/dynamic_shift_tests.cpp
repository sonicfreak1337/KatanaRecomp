...
int main() {
    ...
    require(shad.kind == InstructionKind::ShiftArithmeticDynamic && shad.source_register == 3u &&
                shad.destination_register == 4u && shad.text == "shad r3, r4",
            "SHAD wurde falsch dekodiert.");

    require(shld.kind == InstructionKind::ShiftLogicalDynamic && shld.source_register == 5u &&
                shld.destination_register == 6u && shld.text == "shld r5, r6",
            "SHLD wurde falsch dekodiert.");

    constexpr std::array<std::uint8_t, 8> bytes = {0x3C, 0x44, 0x5D, 0x46, 0x0B, 0x00, 0x09, 0x00};
    ...
    require(program.size() == 1u, "Der KR-1204-Test muss genau eine Funktion besitzen.");
    ...
    require(operations.size() == 4u, "Der KR-1204-Test besitzt eine falsche IR-Groesse.");

    require(operations[0] == Operation::ShiftArithmeticDynamic,
            "SHAD wurde falsch in IR abgesenkt.");

    require(operations[1] == Operation::ShiftLogicalDynamic, "SHLD wurde falsch in IR abgesenkt.");

    const auto source = katana::codegen::emit_cpp_program(program, 0x8C010000u);

    require(source.find("((~count) & 0x1Fu) + 1u") != std::string::npos,
            "Negative dynamische Shiftzaehler fehlen im generierten C++.");

    require(source.find("amount == 32u ? 0u : value >> amount") != std::string::npos,
            "SHLD-Sonderfall fuer negative Vielfache von 32 fehlt.");

    require(source.find("? 0xFFFFFFFFu") != std::string::npos,
    ...
    std::cout << "Alle KR-1204 Decoder-, IR- und Codegen-Tests erfolgreich.\n";
