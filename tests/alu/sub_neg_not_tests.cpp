...
int main() {
    ...
    require(
        sub.kind == InstructionKind::SubRegister,
        "SUB wurde nicht erkannt."
    );
    require(
        sub.destination_register == 1u &&
        sub.source_register == 2u,
        "SUB verwendet falsche Register."
    );
    require(
        sub.text == "sub r2, r1",
        "SUB besitzt falschen Disassembly-Text."
    );

    const auto neg = katana::sh4::decode(0x631Bu);

    require(
        neg.kind == InstructionKind::NegateRegister,
        "NEG wurde nicht erkannt."
    );
    require(
        neg.destination_register == 3u &&
        neg.source_register == 1u,
        "NEG verwendet falsche Register."
    );
    require(
        neg.text == "neg r1, r3",
        "NEG besitzt falschen Disassembly-Text."
    );

    const auto bit_not = katana::sh4::decode(0x6427u);

    require(
        bit_not.kind == InstructionKind::NotRegister,
        "NOT wurde nicht erkannt."
    );
    require(
        bit_not.destination_register == 4u &&
        bit_not.source_register == 2u,
        "NOT verwendet falsche Register."
    );
    require(
        bit_not.text == "not r2, r4",
    ...
    require(
        program.size() == 1u,
        "Der ALU-Test muss genau eine Funktion besitzen."
    );
    ...
    require(
        instructions[2].operation == Operation::SubRegister,
        "SUB wurde nicht korrekt in IR abgesenkt."
    );
    require(
        instructions[3].operation == Operation::NegateRegister,
    ...
    require(
        source.find("cpu.r[3] = 0u - cpu.r[1];") != std::string::npos,
    ...
    std::cout
        << "Alle KR-1101 Decoder-, IR- und Codegen-Tests erfolgreich.\n";
