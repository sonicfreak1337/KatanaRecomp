...
int main() {
    ...
    require(
        shll2.kind == InstructionKind::ShiftLogicalLeftTwo &&
        shll2.destination_register == 1u &&
        shll2.text == "shll2 r1",
        "SHLL2 wurde falsch dekodiert."
    );

    require(
        shll8.kind == InstructionKind::ShiftLogicalLeftEight &&
        shll8.destination_register == 2u &&
        shll8.text == "shll8 r2",
        "SHLL8 wurde falsch dekodiert."
    );

    require(
        shll16.kind == InstructionKind::ShiftLogicalLeftSixteen &&
        shll16.destination_register == 3u &&
        shll16.text == "shll16 r3",
        "SHLL16 wurde falsch dekodiert."
    );

    require(
        shlr2.kind == InstructionKind::ShiftLogicalRightTwo &&
        shlr2.destination_register == 4u &&
        shlr2.text == "shlr2 r4",
        "SHLR2 wurde falsch dekodiert."
    );

    require(
        shlr8.kind == InstructionKind::ShiftLogicalRightEight &&
        shlr8.destination_register == 5u &&
        shlr8.text == "shlr8 r5",
        "SHLR8 wurde falsch dekodiert."
    );

    require(
        shlr16.kind == InstructionKind::ShiftLogicalRightSixteen &&
        shlr16.destination_register == 6u &&
        shlr16.text == "shlr16 r6",
    ...
    require(
        program.size() == 1u,
        "Der KR-1202-Test muss genau eine Funktion besitzen."
    );
    ...
    require(
        operations.size() == 8u,
        "Der KR-1202-Test besitzt eine falsche IR-Groesse."
    );

    require(
        operations[0] == Operation::ShiftLogicalLeftTwo,
        "SHLL2 wurde falsch in IR abgesenkt."
    );
    require(
        operations[1] == Operation::ShiftLogicalLeftEight,
        "SHLL8 wurde falsch in IR abgesenkt."
    );
    require(
        operations[2] == Operation::ShiftLogicalLeftSixteen,
        "SHLL16 wurde falsch in IR abgesenkt."
    );
    require(
        operations[3] == Operation::ShiftLogicalRightTwo,
        "SHLR2 wurde falsch in IR abgesenkt."
    );
    require(
        operations[4] == Operation::ShiftLogicalRightEight,
        "SHLR8 wurde falsch in IR abgesenkt."
    );
    require(
        operations[5] == Operation::ShiftLogicalRightSixteen,
    ...
    require(
        source.find("] <<= 2u;") != std::string::npos &&
    ...
    std::cout
        << "Alle KR-1202 Decoder-, IR- und Codegen-Tests erfolgreich.\n";
