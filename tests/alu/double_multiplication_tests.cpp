...
int main() {
    ...
    require(dmulu.kind == InstructionKind::DoubleMultiplyUnsignedLong &&
                dmulu.source_register == 1u && dmulu.destination_register == 2u &&
                dmulu.text == "dmulu.l r1, r2",
            "DMULU.L wurde falsch dekodiert.");

    require(dmuls.kind == InstructionKind::DoubleMultiplySignedLong &&
                dmuls.source_register == 3u && dmuls.destination_register == 4u &&
                dmuls.text == "dmuls.l r3, r4",
            "DMULS.L wurde falsch dekodiert.");

    constexpr std::array<std::uint8_t, 30> bytes = {
        0x06, 0xB0, 0x09, 0x00, 0x08, 0xB0, 0x09, 0x00, 0x0B, 0x00, 0x09, 0x00, 0x09, 0x00, 0x09,
        0x00, 0x15, 0x32, 0x0B, 0x00, 0x09, 0x00, 0x09, 0x00, 0x3D, 0x34, 0x0B, 0x00, 0x09, 0x00};
    ...
    require(count_operation(program, Operation::DoubleMultiplyUnsignedLong) == 1u,
            "DMULU.L wurde nicht genau einmal in IR abgesenkt.");

    require(count_operation(program, Operation::DoubleMultiplySignedLong) == 1u,
    ...
    require(source.find("cpu.mach = static_cast<std::uint32_t>(product >> 32u);") !=
                std::string::npos,
            "Die oberen 32 Produktbits werden nicht nach MACH geschrieben.");

    require(source.find("static_cast<std::int64_t>(source_raw) - 0x100000000ll") !=
                std::string::npos,
            "DMULS.L besitzt keine portable 32-Bit-Vorzeichenerweiterung.");

    require(source.find("static_cast<std::uint64_t>(signed_product)") != std::string::npos,
    ...
    std::cout << "Alle KR-1302 Decoder-, IR- und Codegen-Tests erfolgreich.\n";
