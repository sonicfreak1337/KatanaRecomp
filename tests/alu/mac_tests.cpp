...
int main() {
    ...
    require(macw.kind == InstructionKind::MultiplyAccumulateWord && macw.source_register == 1u &&
                macw.destination_register == 2u && macw.text == "mac.w @r1+, @r2+",
            "MAC.W wurde falsch dekodiert.");

    require(macl.kind == InstructionKind::MultiplyAccumulateLong && macl.source_register == 3u &&
                macl.destination_register == 4u && macl.text == "mac.l @r3+, @r4+",
            "MAC.L wurde falsch dekodiert.");

    require(clrs.kind == InstructionKind::ClearS && clrs.text == "clrs",
            "CLRS wurde falsch dekodiert.");

    require(sets.kind == InstructionKind::SetS && sets.text == "sets",
            "SETS wurde falsch dekodiert.");

    constexpr std::array<std::uint8_t, 56> bytes = {
        0x58, 0x00, 0x48, 0x00, 0x08, 0xB0, 0x09, 0x00, 0x0A, 0xB0, 0x09, 0x00, 0x0C, 0xB0,
        0x09, 0x00, 0x0E, 0xB0, 0x09, 0x00, 0x0B, 0x00, 0x09, 0x00, 0x1F, 0x42, 0x0B, 0x00,
        0x09, 0x00, 0x09, 0x00, 0x3F, 0x04, 0x0B, 0x00, 0x09, 0x00, 0x09, 0x00, 0x5F, 0x45,
        0x0B, 0x00, 0x09, 0x00, 0x09, 0x00, 0x6F, 0x06, 0x0B, 0x00, 0x09, 0x00, 0x09, 0x00};
    ...
    require(count_operation(program, Operation::MultiplyAccumulateWord) == 2u,
            "MAC.W wurde nicht genau zweimal in IR abgesenkt.");

    require(count_operation(program, Operation::MultiplyAccumulateLong) == 2u,
            "MAC.L wurde nicht genau zweimal in IR abgesenkt.");

    require(count_operation(program, Operation::SetS) == 1u,
            "SETS wurde nicht genau einmal in IR abgesenkt.");

    require(count_operation(program, Operation::ClearS) == 1u,
