...
int main() {
    ...
    require(
        addc.kind == InstructionKind::AddWithCarry &&
        addc.text == "addc r2, r1",
        "ADDC wurde falsch dekodiert."
    );

    require(
        addv.kind == InstructionKind::AddWithOverflow &&
        addv.text == "addv r2, r1",
        "ADDV wurde falsch dekodiert."
    );

    require(
        subc.kind == InstructionKind::SubWithCarry &&
        subc.text == "subc r2, r1",
        "SUBC wurde falsch dekodiert."
    );

    require(
        subv.kind == InstructionKind::SubWithOverflow &&
        subv.text == "subv r2, r1",
        "SUBV wurde falsch dekodiert."
    );

    require(
        negc.kind == InstructionKind::NegateWithCarry &&
        negc.destination_register == 3u &&
        negc.source_register == 1u &&
        negc.text == "negc r1, r3",
    ...
    require(
        program.size() == 1u,
        "Der KR-1104-Test muss genau eine Funktion besitzen."
    );
    ...
    require(
        ...
        "Der KR-1104-IR-Block besitzt eine falsche Groesse."
    );

    require(
        instructions[0].operation == Operation::AddWithCarry,
        "ADDC wurde falsch in IR abgesenkt."
    );
    require(
        instructions[1].operation == Operation::AddWithOverflow,
        "ADDV wurde falsch in IR abgesenkt."
    );
    require(
        instructions[2].operation == Operation::SubWithCarry,
        "SUBC wurde falsch in IR abgesenkt."
    );
    require(
        instructions[3].operation == Operation::SubWithOverflow,
        "SUBV wurde falsch in IR abgesenkt."
    );
    require(
        instructions[4].operation == Operation::NegateWithCarry,
    ...
    require(
        source.find("const std::uint64_t carry_in") !=
            std::string::npos &&
        source.find("cpu.t = result > 0xFFFFFFFFull;") !=
            std::string::npos,
        "ADDC-Semantik fehlt im generierten C++."
    );

    require(
        source.find("const std::uint64_t borrow_in") !=
            std::string::npos &&
        source.find("cpu.t = minuend < subtrahend;") !=
            std::string::npos,
        "SUBC-Semantik fehlt im generierten C++."
    );

    require(
        source.find("cpu.t = subtrahend != 0ull;") !=
            std::string::npos,
        "NEGC-Semantik fehlt im generierten C++."
    );

    require(
        source.find("~(lhs ^ rhs) & (lhs ^ result)") !=
            std::string::npos,
    ...
    std::cout
        << "Alle KR-1104 Decoder-, IR- und Codegen-Tests erfolgreich.\n";
