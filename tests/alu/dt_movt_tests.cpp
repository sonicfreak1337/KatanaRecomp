...
int main() {
    ...
    require(
        dt.kind == InstructionKind::DecrementAndTest &&
        dt.destination_register == 3u &&
        dt.text == "dt r3",
        "DT wurde falsch dekodiert."
    );

    require(
        movt.kind == InstructionKind::MoveT &&
        movt.destination_register == 5u &&
        movt.text == "movt r5",
    ...
    require(
        program.size() == 1u,
        "Der KR-1106-Test muss genau eine Funktion besitzen."
    );
    ...
    require(
        instructions.size() == 5u,
        "Der KR-1106-IR-Block besitzt eine falsche Groesse."
    );

    require(
        instructions[0].operation == Operation::DecrementAndTest,
        "DT wurde falsch in IR abgesenkt."
    );

    require(
        instructions[1].operation == Operation::MoveT,
        "MOVT wurde falsch in IR abgesenkt."
    );

    require(
        instructions[2].operation == Operation::DecrementAndTest,
