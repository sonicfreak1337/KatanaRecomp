...
void run_case(const char* name,
    ...
    require(cpu.r[2] == expected_logical, std::string(name) + ": SHLD-Ergebnis ist falsch.");

    require(cpu.r[4] == expected_arithmetic, std::string(name) + ": SHAD-Ergebnis ist falsch.");

    require(cpu.r[8] == (initial_t ? 1u : 0u) && cpu.r[9] == (initial_t ? 1u : 0u),
            std::string(name) + ": T-Marker wurden veraendert.");

    require(cpu.t == initial_t, std::string(name) + ": T-Bit wurde veraendert.");

    require(cpu.r[1] == logical_count && cpu.r[3] == arithmetic_count,
            std::string(name) + ": Quellregister wurden veraendert.");

    require(cpu.pc == 0u, std::string(name) + ": RTS-Ergebnis ist falsch.");
...
int main() {
    run_case("positiv +4", 4u, 0x12345678u, 0x23456780u, 4u, 0x81234567u, 0x12345670u, true);
    ...
    run_case("grosser Zaehler +36 und -36",
             36u,
             0x12345678u,
             0x23456780u,
             0xFFFFFFDCu,
    ...
    run_case("Grenze -32 mit negativem SHAD-Wert",
             0xFFFFFFE0u,
             0x89ABCDEFu,
             0x00000000u,
             0xFFFFFFE0u,
    ...
    run_case("Grenze -32 mit positivem SHAD-Wert",
             0x80000000u,
             0xFFFFFFFFu,
             0x00000000u,
             0x80000000u,
    ...
    std::cout << "KR-1204 End-to-End-Semantik wurde erfolgreich ausgefuehrt.\n";
