...
int main() {
    ...
    require(and_register.kind == InstructionKind::AndRegister, "AND Register wurde nicht erkannt.");
    require(and_register.destination_register == 1u && and_register.source_register == 2u,
            "AND Register verwendet falsche Register.");
    require(and_register.text == "and r2, r1", "AND Register besitzt falschen Disassembly-Text.");

    const auto xor_register = katana::sh4::decode(0x265Au);

    require(xor_register.kind == InstructionKind::XorRegister, "XOR Register wurde nicht erkannt.");
    require(xor_register.destination_register == 6u && xor_register.source_register == 5u,
            "XOR Register verwendet falsche Register.");

    const auto or_register = katana::sh4::decode(0x243Bu);

    require(or_register.kind == InstructionKind::OrRegister, "OR Register wurde nicht erkannt.");
    require(or_register.destination_register == 4u && or_register.source_register == 3u,
            "OR Register verwendet falsche Register.");

    const auto and_immediate = katana::sh4::decode(0xC90Fu);

    require(and_immediate.kind == InstructionKind::AndImmediate,
            "AND Immediate wurde nicht erkannt.");
    require(and_immediate.destination_register == 0u && and_immediate.immediate == 15,
            "AND Immediate wurde falsch dekodiert.");

    const auto xor_immediate = katana::sh4::decode(0xCAFFu);

    require(xor_immediate.kind == InstructionKind::XorImmediate,
            "XOR Immediate wurde nicht erkannt.");
    require(xor_immediate.immediate == 255,
            "XOR Immediate muss ohne Vorzeichenerweiterung dekodiert werden.");

    const auto or_immediate = katana::sh4::decode(0xCB80u);

    require(or_immediate.kind == InstructionKind::OrImmediate, "OR Immediate wurde nicht erkannt.");
    ...
    require(program.size() == 1u, "Der KR-1102-Test muss genau eine Funktion besitzen.");

    const auto& instructions = program[0].blocks[0].instructions;

    require(instructions.size() == 13u, "Der IR-Block besitzt eine falsche Instruktionsanzahl.");

    require(instructions[2].operation == Operation::AndRegister,
            "AND Register wurde nicht korrekt in IR abgesenkt.");
    require(instructions[4].operation == Operation::OrRegister,
            "OR Register wurde nicht korrekt in IR abgesenkt.");
    require(instructions[6].operation == Operation::XorRegister,
            "XOR Register wurde nicht korrekt in IR abgesenkt.");
    require(instructions[8].operation == Operation::AndImmediate,
            "AND Immediate wurde nicht korrekt in IR abgesenkt.");
    require(instructions[9].operation == Operation::OrImmediate,
            "OR Immediate wurde nicht korrekt in IR abgesenkt.");
    require(instructions[10].operation == Operation::XorImmediate,
    ...
    require(source.find("cpu.r[3] |= cpu.r[2];") != std::string::npos,
            "OR Register fehlt im generierten C++.");
    require(source.find("cpu.r[4] ^= cpu.r[2];") != std::string::npos,
            "XOR Register fehlt im generierten C++.");
    require(source.find("cpu.r[0] &= static_cast<std::uint32_t>(15);") != std::string::npos,
            "AND Immediate fehlt im generierten C++.");
    require(source.find("cpu.r[0] |= static_cast<std::uint32_t>(128);") != std::string::npos,
            "OR Immediate fehlt im generierten C++.");
    require(source.find("cpu.r[0] ^= static_cast<std::uint32_t>(255);") != std::string::npos,
            "XOR Immediate fehlt im generierten C++.");

    std::cout << "Alle KR-1102 Decoder-, IR- und Codegen-Tests erfolgreich.\n";
