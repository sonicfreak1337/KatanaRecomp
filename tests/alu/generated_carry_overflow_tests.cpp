...
int main() {
    ...
    require(
        overflow_cpu.r[1] == 0u,
        "ADDC muss 0xFFFFFFFF + 0 + T zu 0 mit Carry berechnen."
    );

    require(
        overflow_cpu.r[2] == 0u,
        "ADDC darf das Quellregister nicht veraendern."
    );

    require(
        overflow_cpu.r[3] == 0xFFFFFFFFu,
        "SUBC muss 0 - 0 - T mit Borrow berechnen."
    );

    require(
        overflow_cpu.r[6] == 0xFFFFFFFFu,
        "NEGC muss 0 - 1 - 0 berechnen."
    );

    require(
        overflow_cpu.r[11] == 0x80000000u,
        "ADDV-Grenzwert ist falsch."
    );

    require(
        overflow_cpu.r[13] == 0x7FFFFFFFu,
        "SUBV-Grenzwert ist falsch."
    );
    ...
    require(
        overflow_cpu.t,
        "SUBV muss beim signed Overflow T setzen."
    );
    ...
    require(
        normal_cpu.r[1] == 3u,
        "ADDC ohne Carry-Out muss 1 + 1 + T zu 3 berechnen."
    );

    require(
        normal_cpu.r[3] == 3u,
        "SUBC ohne Borrow-Out muss 5 - 1 - T zu 3 berechnen."
    );

    require(
        normal_cpu.r[6] == 0u,
        "NEGC von 0 ohne Borrow-In muss 0 ergeben."
    );

    require(
        normal_cpu.r[11] == 2u,
        "ADDV ohne Overflow muss 1 + 1 zu 2 berechnen."
    );

    require(
        normal_cpu.r[13] == 4u,
        "SUBV ohne Overflow muss 5 - 1 zu 4 berechnen."
    );
    ...
    require(
        !normal_cpu.t,
        "SUBV ohne signed Overflow muss T loeschen."
    );

    std::cout
        << "KR-1104 End-to-End-Semantik wurde erfolgreich ausgefuehrt.\n";
