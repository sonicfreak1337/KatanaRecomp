...
void execute_ir_reference(
    ...
    switch (opcode) {
        case 0xE001u: cpu.r[0] = 1u; break;
        case 0x7001u: ++cpu.r[0]; break;
        case 0x2102u: memory[0].value = static_cast<std::uint8_t>(cpu.r[0]); break;
        case 0xC001u:
            mmio.push_back({
            ...
            break;
        default: throw std::runtime_error("IR-Referenz erhielt unbekannten Testopcode.");
    }
}

void execute_generated_cpp(
    katana::runtime::CpuState& cpu,
    std::vector<DifferentialMemoryByte>& memory,
    std::vector<DifferentialMmioObservation>& mmio,
    const std::uint16_t opcode
    ...
    if (opcode == 0xE001u) {
        ...
        mmio.emplace_back(DifferentialMmioObservation{
            katana::runtime::MemoryAccessOperation::Write,
            0x005F6800u,
            katana::runtime::MemoryAccessWidth::Word,
            cpu.r[0]
...
bool execute_fallback_instruction(
    katana::runtime::CpuState& cpu,
    std::vector<DifferentialMemoryByte>& memory,
    std::vector<DifferentialMmioObservation>& mmio,
    ...
    switch (request.opcode) {
        case 0xE001u: cpu.r[0] = 1u; break;
        case 0x7001u: cpu.r[0] += 1u; break;
        case 0x2102u: memory.at(0).value = static_cast<std::uint8_t>(cpu.r[0]); break;
        case 0xC001u:
            mmio.push_back({
                katana::runtime::MemoryAccessOperation::Write,
                0x005F6800u,
...
DifferentialTrace execute_direct_path(
    const DifferentialExecutionPath path,
...
std::array<DifferentialRunner, 3u> runners(const bool corrupt_generated = false) {
    return {{
        {DifferentialExecutionPath::IrReference, [](const auto& program) {
            return execute_direct_path(DifferentialExecutionPath::IrReference, program, false);
        }},
        {DifferentialExecutionPath::GeneratedCpp, [=](const auto& program) {
        ...
        {DifferentialExecutionPath::InterpreterFallback, [](const auto& program) {
            return execute_fallback_path(program);
        }}
    }};
}
...
int main() {
    try {
        ...
        const auto matching = katana::testing::run_differential_execution(
            program,
            matching_runners
        );
        ...
        require(
            broken.first_difference->actual_path == DifferentialExecutionPath::GeneratedCpp &&
        ...
        const auto counterexample = katana::testing::format_differential_counterexample_json(broken);
        require(
            counterexample.find("katana-differential-counterexample") != std::string::npos &&
        ...
        for (const auto& item : corpus) categories.insert(item.corpus);
        require(
        ...
        try {
            static_cast<void>(katana::testing::run_differential_execution(
                program,
                duplicate_runners
            ));
