...
class RecordingBackend : public katana::codegen::Backend {
  public:
    ...
    [[nodiscard]] katana::codegen::BackendCapabilities capabilities() const noexcept override {
        return katana::codegen::capability(katana::codegen::BackendCapability::StructuredSections);
    }

    [[nodiscard]] katana::codegen::BackendEmission
    emit(const katana::codegen::BackendRequest& request) const override {
...
class IncompleteBackend final : public RecordingBackend {
  public:
    [[nodiscard]] katana::codegen::BackendEmission
    emit(const katana::codegen::BackendRequest&) const override {
        return {"declarations\n", {}, {}};
    }
};
...
int main() {
    ...
    require(backend.observed_entry == entry && backend.observed_functions == 1u,
            "Backend erhaelt nicht das validierte IR-Programm und den Einstieg.");
    require(emission.joined_text() == "declarations\nfunctions\nmetadata\n",
            "Backendabschnitte verlieren ihre deterministische Reihenfolge.");
    require(
        throws<std::invalid_argument>([&] {
            const katana::codegen::BackendRequest empty{std::span<const katana::ir::Function>{},
                                                        entry};
            static_cast<void>(katana::codegen::generate_program(backend, empty));
        }) &&
            throws<std::invalid_argument>([&] {
                static_cast<void>(
                    katana::codegen::generate_program(backend, {functions, entry + 4u}));
            }) &&
            throws<std::invalid_argument>([&] {
